...
/// the source-generated <see cref="LauncherJsonContext"/> (AOT-safe). All I/O is best-effort.
/// </summary>
public sealed class EnvStateStore : IEnvStateStore {
	...
	public EnvState Load() {
		try {
			if (File.Exists(_path)) {
				...
				return JsonSerializer.Deserialize(json, LauncherJsonContext.Default.EnvState) ?? new EnvState();
			}
		}
		catch (Exception) {
	...
	public void Save(EnvState state) {
		try {
			...
			string json = JsonSerializer.Serialize(state, LauncherJsonContext.Default.EnvState);
