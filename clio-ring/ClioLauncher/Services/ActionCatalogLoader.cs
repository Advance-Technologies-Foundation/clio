...
/// <see cref="LauncherJsonContext"/> (AOT-safe) then enforces the closed-envelope contract:
...
public sealed class ActionCatalogLoader : IActionCatalogLoader {
	...
	public ActionCatalog Load(string? path = null) {
		...
		try {
			...
			catalog = JsonSerializer.Deserialize(json, LauncherJsonContext.Default.ActionCatalog);
		}
	...
	private static void ValidateKindBlocks(LauncherAction action, string source) {
	...
	private static void ValidateParameters(LauncherAction action, string source) {
	...
	private static void Require(bool present, LauncherAction action, string block, string source) {
		if (!present) {
	...
	private static void Forbid(bool present, LauncherAction action, string blocks, string source) {
		if (present) {
			throw new ActionCatalogException(
