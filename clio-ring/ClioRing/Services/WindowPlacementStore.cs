...
public sealed class WindowPlacementStore : IWindowPlacementStore {
	...
	public WindowPlacement? Load() {
		try {
			if (File.Exists(_path)) {
				...
				return JsonSerializer.Deserialize(json, RingJsonContext.Default.WindowPlacement);
			}
		}
		catch (Exception) {
			// ignore
	...
	public void Save(WindowPlacement placement) {
		try {
			...
			File.WriteAllText(_path, JsonSerializer.Serialize(placement, RingJsonContext.Default.WindowPlacement));
		}
		catch (Exception) {
			// best-effort
		}
