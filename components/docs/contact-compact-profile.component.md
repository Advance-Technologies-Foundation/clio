# How to Add a Contact Compact Profile (`crt.ContactCompactProfile`) to a Freedom UI Page

> Audience: code agent inserting a `crt.ContactCompactProfile` into a Creatio Freedom UI page schema.
> A `crt.ContactCompactProfile` is an editable inline card for a contact record; it shows and allows editing of the contact's full name parts (first / middle / last), photo, birth date, country, city, and time zone. Requires the `CrtCustomer360App` package.
...
`crt.ContactCompactProfile` loads the contact record internally via `referenceColumn` (bound to the primary record ID). No separate `modelConfigDiff` or `viewModelConfigDiff` is needed for the basic recipe — the create command auto-wires `referenceColumn` to `$Id` on the primary datasource.
...
`referenceColumn` is bound to the `$-prefix` attribute that holds the Contact record ID (typically `"$Id"` on a Contact form page). Omit `layoutConfig` or use the grid-shaped form if the parent is a `crt.GridContainer`.
...
`crt.ContactCompactProfile` are in `ComponentRegistry.json` under `componentType: "crt.ContactCompactProfile"`. This guide
...
The `firstName`, `middleName`, `lastName`, `displayName`, `birthDate`, `country`, `city`, `timeZone` inputs
accept `$-prefix` attribute bindings when individual fields need to be driven from the viewModel:
