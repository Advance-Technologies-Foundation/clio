...
1. **Явний `target-version`** в args інструменту (overrides все).
2. **`environment-name`** → probe `GetSysInfo` → cache per environment.
3. **Cliogate недоступний / probe failed** → fallback to **`latest known`** (з явним маркером).
4. **Без env і без version** → `latest known` з тим же маркером.

У response завжди повертати:

```json
{
  "resolvedTargetVersion": "8.1.5",
...
2. **Fallback-семантика, коли версія не відома.** «Latest known» (поточна пропозиція) vs. «unrestricted» (повернути все, включно з deprecated) vs. помилка. Кожна — інший AI UX.
