...
## OQ-01 verdict (Blocking Prerequisite) — PARTIALLY RESOLVED: token leg feasible via ctor; cookie leg unverified
...
### OQ-04 — Per-request context transport (FR-04): `IHttpContextAccessor` behind a singleton seam
...
- **Trade-offs / risks:**
  - **RISK #1 (exec-context flow):** the MCP AspNetCore defaults (`EnableLegacySse=false`, `PerSessionExecutionContext=false`) make handler-time `IHttpContextAccessor` **plausible**, so this is lower-risk than a "POST/SSE split." The residual risk is that clio changes those defaults or the SDK does not invoke tools on the request's context in practice — if so, the seam returns null and re-targets the SDK's per-request DI scope. Mitigated by plan step 1 + the step-15e transport-default assertion.
  - **FR-06 concurrency assumption:** AsyncLocal capture isolation is safe only if concurrent invocations run on independent async flows; if not, capture falls back to a per-invocation object from the child container. Proven/disproven in step 1.
