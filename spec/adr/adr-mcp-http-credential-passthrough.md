...
- **Do NOT** put `[FeatureToggle]` on `McpHttpServerCommandOptions` — that hides the shipped `mcp-http` verb entirely and regresses FR-10/AC-10.
- **AMENDED (v1 as shipped — story-11 reverted 2026-07-10):** the second `IsFeatureEnabled("mcp-http-credential-passthrough")` incubation gate described below was **removed** and is NOT in the code. v1 is **single-gated by the platform API-key gate** (fail-closed: no key configured ⇒ passthrough disabled, exactly the pre-passthrough behavior; a public/non-loopback bind with auth off refuses to start). This was reviewed as safe — the API-key gate alone is the fail-closed control — and is the accepted v1 design. The paragraph below is retained for historical context; treat the API-key gate as the sole runtime gate.
