...
9. **A process parameter can carry its own value** (an initial/default), using the same `ProcessSchemaParameterValue` source model as element mappings (#5). Only a **constant** (`Source=ConstValue`) is honored today; the descriptor keeps the mapping-style `{value|expression|processParameter}` shape so non-constant defaults are a later, non-breaking addition.
10. **Parameter identity is the UId.** Every reference (mappings, formulas, signature/describe) resolves by UId (#5), so updates **mutate the parameter in place** (preserving the UId) — caption, description, code (Name), direction, lookup `referenceSchema` and the constant value all change with no reference rewriting and no dangling refs. A **data-type change is the exception**: it can invalidate the stored value, orphan a Lookup's `referenceSchema`, and break the type expectations of elements that map from the parameter (the references survive by UId, but the type beneath them changes), so it needs value/reference re-validation and is **deferred** — a type *change* is rejected with a clear error (a `type` equal to the current type is a no-op).
