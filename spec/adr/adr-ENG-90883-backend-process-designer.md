...
10. **Parameter identity is the UId.** Every reference (mappings, formulas, signature/describe) resolves by UId (#5), so updates **mutate the parameter in place** (preserving the UId) — caption, code (Name), direction, lookup `referenceSchema` and the constant value all change with no reference rewriting and no dangling refs.
