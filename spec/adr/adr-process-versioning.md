...
| G: make `get-process-signature` version-aware in this ticket | closes the last known version-blind read | it is a public `[Verb]`, so it drags help and docs — but the exclusion was never achievable at the price assumed here: it resolves through `IProcessModelGenerator.Generate` → `ProcessModelGenerator.GetProcessIdFromName` → `ProcessLibResolver.Resolve`, the same seam `describe --process-caption` uses, and `Resolve` takes no policy selector. Keeping it out would have cost a policy parameter on the shared seam, i.e. MORE work than letting it inherit | **Accepted as delivered**: it inherits the shared resolver's policy, its two doc surfaces moved with it, and the PRD non-goal is withdrawn. Any future surface wired to `ProcessModelGenerator` inherits the same policy silently — that is the seam's contract, not an accident |
...
- **Breaking change**: **Behavioural, not contractual.** No tool is renamed or removed and every response member is additive, but FOUR resolution paths change outcome for versioned families, because all four share `ProcessLibResolver`: `describe --process-caption` and `get-process-signature` pick the active version instead of an arbitrary member, `generate-process-model` now succeeds where it used to fail with a caption-ambiguity error, and `run-process` launches the active member. The radius is four and not two because the seam has no policy selector — see alternative G. A NEW refusal arrives with it on all four: a lone caption match the view flags explicitly `IsActiveVersion == false` now returns `Conflict` where it previously resolved, which is the deliberate price of not answering for a graph nobody runs on surfaces that carry no version fields to reveal it. An UNESTABLISHED flag deliberately does not refuse. The PRD's CLI Impact table carries a row per changed verb; `RELEASE.md` carries the `generate-process-model` note.
...
Story 8, second pass: the version-creation mechanics were measured without creating a version, by reading the composer the platform actually ships and by calling the one server operation it depends on. Every number below is either read from `Terrasoft.BaseProcessSchemaManager.prototype` in a live client or returned by the server on request; nothing here is inferred from the traps write-up.

- **The number is `maxVersionInPackage + 1`, scoped to (family root, PACKAGE).** `getNewSchemaVersion` calls `GetSchemaVersionInfo` with `{parentSchemaUId, packageUId}` and assigns `version = maxVersionInPackage + 1`. Measured on `ProcessSchemaManagerService.svc` (POST, CSRF header required — a GET answers 405 and a plain POST 403): `InvoiceVisaProcess` + its own package Invoice → 1, the same root + `UsrAntonTest` → 0, a fresh root + its own package → 0. So a version created in a different package restarts at 1, which is what the three cross-package stock families look like. This answers AC-03 without the server-side C# the story assumed was needed.
- **A new version is composed INACTIVE.** `createNewSchemaVersion` sets `isActiveVersion` false and `isDelivered` false on the copy. So creating and activating are separate at the platform level too, not merely in this ADR's design — the split in choices 3 and 4 is confirmed rather than chosen.
- **The family is set flat at composition time**: `parentSchemaUId = getIsSetParentSchemaUId(sourceParent) ? sourceParent : source.uId`, i.e. a version of a version still points at the ROOT. V2 read from code, not only from rows.
- **AC-05 answered, and the answer is not a rejection.** `setNewSchemaVersionName` builds `parentSchemaName + packageName.replace(/\W/g, "") + version` and then, when the schema-name prefix is configured and the PARENT does not start with it, PREPENDS the prefix to the whole name. So a version of the unprefixed `InvoiceVisaProcess` on a stand whose prefix is `Usr` is named `UsrInvoiceVisaProcessInvoice1`. Nothing validates or refuses; the name simply changes shape. That is why the guidance article's V4 had to be demoted from a law to a default.
- **OQ-01 refined to its actual mechanism.** `_getPackageForNewSchemaVersion` branches on the feature `SaveProcessVersionInApplicationPackage`: disabled → `getCurrentPackageUId`, enabled → `getDesignPackageUId`. The DB reports that feature `DefaultState = 0` with an `AdminUnitFeatureState` override of 1, so it is ON here and the version lands in the DESIGN package. But that branch is only reached when the source package is NOT editable: `getNewSchemaVersion` uses the source's own package when `sysPackage && canEditPackageSchema`. Two consequences for the create handler: the package must be an INPUT, and the name and the location can disagree — the editable branch passes the real package name into the name builder while the non-editable branch leaves it as `Terrasoft.PackageManager.CUSTOM_PACKAGE_NAME` ("Custom"), so a version can be named `...Custom1` while living in another package entirely.
- `UseProcessVersions` is likewise `DefaultState = 0` with an override of 1 (ON). Worth knowing that the BASE manager's `getCanUseProcessVersions()` is a hard `return false`, and that when it is false `setIsActualVersion` calls its callback with `{success: true}` WITHOUT issuing a request — a silent no-op reported as success. Our write half is server-side and does not go through that client path, but anyone who reuses the client machinery inherits it.

AC-01's "run a draft create implementation twice" was NOT performed, deliberately. The formula, its server input and the initial flag values are all measured above, so a second run would only confirm the arithmetic 0+1 then 1+1 — and it would cost two permanently undeletable schemas (V6), plus a hand-composed schema instance, because the designer page could not be reached by URL on this build (`ProcessDesigner.aspx` answers 500 and the Shell route falls back to the desktop). The measurement it would add is smaller than the residue it would leave.
Story 8, third pass (AC-01 pursued properly): the version counter semantics are now pinned, and they produce a trap the formula alone does not. `GetSchemaVersionInfo` counts family MEMBERS and ignores the root own stamped `Version` — it answers `maxVersionInPackage = 0` for `BulkDuplicatesSearchProcess`, which itself carries `Version = 2`. Since a new version is numbered `max + 1`, **a root with a non-zero stamped number gets a version numbered BELOW itself**: a first version of that process would be 1, beside a root that says 2. Two consequences for the create handler — never derive the next number from the SOURCE schema own property, and never present version numbers as a total ordering across a family. Measured for four further (root, package) pairs, all consistent with (root, package) scoping: one-version families answer 1 in the package holding the version, and a cross-package family answers 1 in the version package and 0 in the root package.

AC-01 verbatim two-run observation stays unmet, and this time the reason is the environment rather than a choice. Five routes to a working create dead-end on this build: the Shell hash, `ViewModule.aspx` (redirects to the Shell), `ProcessDesigner.aspx` and `ProcessSchemaDesigner.aspx` (500), the classic section route, and the app-list navigation link. Driving the composer directly fails too — `BaseSchemaManagerItem.updateRequestClassName` is null on the base class, so the save request ships inside the designer module, and `Terrasoft.require` will not resolve those modules in a Freedom shell. The only implementation that creates a version here is the classic designer, and the classic UI is not served. The two-run values are recorded in the story as DERIVED from measured inputs (version 1 then 2, `isActiveVersion` false both times) rather than as observations.
Story 8, fourth pass, on a SECOND stand (studio edition, core 10.2.3.0) — and it opens with a correction to the third pass.

**Correction: `getCanUseProcessVersions()` returning false is a DEFAULT, not the effective answer.** The third pass read it off `Terrasoft.BaseProcessSchemaManager.prototype` and recorded a general "silent no-op reported as success" trap. Measured properly: the manager business processes actually use, `Terrasoft.manager.ProcessSchemaManager`, overrides it to `return true`. The `false` belongs to the base and to `Terrasoft.manager.EmbeddedProcessSchemaManager`. So the no-op branch is real but NARROW — it applies to embedded (case/DCM) process managers and to anything inheriting the base default, not to business processes. Caught by watching the network: clicking "Set as actual version" issued `GetActualVersionUId` and `GetRunningProcessesCount`, which only run INSIDE the `getCanUseProcessVersions()` branch, so the flag could not have been false.

The classic designer IS served on this stand, at a URL guessing never found: `/0/Nui/ViewModule.aspx?vm=SchemaDesigner#process/<schemaUId>`. On the first stand `ViewModule.aspx` redirects to the Freedom shell and the same route is unreachable, which is why the third pass concluded the designer was absent — it is absent THERE, not in the product.

Measured here, all first-hand:

- A designer-created process gets an auto-generated code with the environment prefix and a hex tail — `UsrProcess_f817a44` — the exact shape V7 warns against reading as meaningful. It lands in the `Custom` package with `Version = 0`, `IsActiveVersion = true` and its own UId as the family key.
- **Editing an ACTIVE process and saving overwrites it in place. No version is offered and none is created.** Observed, not reasoned: the save reported "Successfully saved" and the row kept `Version = 0`. This is the claim the guidance article makes about irreversible in-place edits, now with evidence behind it.
- Activation validation reads before it writes: `Set as actual version` issued `GetActualVersionUId` and **`GetRunningProcessesCount`**, then sent no `SetIsActualVersion`. So the platform gates activation on the running-instance count — a direct input for story 13, which must expect a refusal that is silent unless the caller inspects the result.
...
**Correction (2026-09-04, raised by the ticket owner): versioning IS reachable on the local stand, and the fourth-pass claim that it is not was wrong.** The route is the process CARD page — `#CardModuleV2/VwProcessLibPageV2/edit/<id>` — which the Freedom shell serves, and which hosts the versions detail. The owner created `UsrProcess_329f1da` with a version `UsrProcess_329f1daCustom1` there, verified in the view. What I actually established was narrower than what I wrote: the process DESIGNER is unreachable by URL on that build (five routes probed — the Shell hash, `ViewModule.aspx`, two `*Designer.aspx` endpoints, the classic section route and the Navigation link), and I generalised from "the designer is unreachable" to "no implementation is reachable". The card page was in a snapshot I had taken on the studio stand and I read past it.

Consequence for the record: the studio-stand detour was not necessary to satisfy AC-01. It was still worth it — the raw-composer route produced two facts the UI route would not have shown, namely that a version is composed with `isActiveVersion` false and that the save translates `parentSchemaUId` (a UId) into `SysSchema.ParentId` (an Id) — but the criterion could have been met locally.

One difference between the two routes is open and matters for stories 9-13. Through the raw composer my two versions were created INACTIVE with the root left active; in the owner's family the root is inactive and the version is active. The timestamps favour activation being a SEPARATE step rather than part of creation: the root was created at 08:23:01, the version at 08:23:21, and then BOTH rows were modified 8 seconds later, at 08:23:29.9 and 08:23:31.2 — one write touching both, which is what deactivating the root and activating the version looks like. That supports the independence recorded above rather than contradicting it, but it is inference from timestamps, not an observation: whether the card page offers "create version" and "set actual" as one gesture or two is still unconfirmed, and it decides whether `ModifyProcessAsNewVersion` should leave activation to the caller.
**The product contract, observed (2026-09-04, screenshots from the ticket owner).** The designer's SAVE is a split button with two items — `Save new version (Ctrl+Alt+N)` and `Save current version (Ctrl+Alt+S)`. Choosing "Save new version" creates the version and then raises a separate prompt: *Set the current version of the process "<caption>" actual?* with YES / NO.

That settles three things at once.

1. **Create and activate really are separate operations, and the platform does not decide.** It creates the version, then ASKS. Choices 3 and 4 stand, and the composer's `isActiveVersion = false` is not an artefact of the raw API path — it is the state a version is in until someone answers YES. The owner's family (root inactive, version active) is a YES, and the timestamps read correctly after all: creation at 08:23:21, then one write touching both rows at 08:23:29.9 / 08:23:31.2 when the prompt was answered.
2. **It answers the question the ticket owner asked at the start of this work** — whether an agent should make a newly created version active. The product does not choose for the user, so neither may we: `ModifyProcessAsNewVersion` must NOT activate, and the guidance must tell the agent to ASK rather than to call `SetActiveProcessVersion` on its own. Auto-activating would be more decisive than the product and would change which graph runs without the user having agreed to it.
3. **Overwriting is a deliberate, separately named choice.** "Save current version" is the in-place overwrite the guidance warns about, sitting in the same menu as the safe option — which is exactly why the modify precondition belongs where the modify instruction is.

The naming formula also holds on this second stand through the UI route: root `UsrProcess_329f1da`, package `Custom`, version 1 → `UsrProcess_329f1daCustom1`.

And the reachability correction is now complete rather than partial: the DESIGNER is reachable too — the owner opened it — just not by any URL I constructed. It is entered from the process card page, which is the link I had and skipped. The lesson for the next spike is procedural: when a UI route is needed, open the section and follow the product's own navigation instead of guessing at module names.
