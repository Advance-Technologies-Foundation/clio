...
| OQ-06 | **Is in-place update of `executionMode` / `ignoreErrors`** on an already-multi-instance element supported, or must the caller de-convert and re-convert? | In-place is the obvious ergonomics; requiring a cycle makes every mode change destructive, and it depends on OQ-01 being YES | **Support in place.** Omitted on update means "left as is, never reset" |
