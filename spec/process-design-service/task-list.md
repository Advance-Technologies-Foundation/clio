...
**Status:** PARTIALLY implemented server-side — custom message only (ENG-92706, `cli-process-builder` branch `feature/ENG-92706-send-email-element`, on top of the ENG-95064 body foundation): dedicated `sendEmail` element + `email` block covering mode (auto/manual), sender (mailbox id or email), To/Cc/Bcc recipients as dynamic element parameters (constant / process parameter / expression+referenceSchema), subject, importance, ignoreErrors, and the manual-mode performer (user/manager/role); `describe-process` read-back; `setElement` in-place reconfigure (recipients match-or-append — an identical entry is a no-op, a new address appends, none can be removed). Serialization capture-verified (`docs/send-email-element-capture.md` there) and runtime-verified. **Two of the ticket's acceptance criteria are NOT met, and the ticket has NOT been amended** — AC2 ("…OR an email template can be selected with its parameters synced") stands verbatim and is not struck through, and AC1 still lists a "contact or entity column" recipient source. Do not read this row as the element being done. **Outstanding / follow-ups:** email templates — AC2, deferred to a follow-up sub-task, nothing implemented; entity-column recipients as first-class sources (ENG-91844 — the raw `expression` escape hatch is the workaround today, not the criterion); body macro authoring (ENG-95064); activity-connection dynamic parameters (ENG-91845 — its `setConnections` accepts `EmailTemplateUserTask` and gates on `CreateActivity == true` **OR** manual send mode; this element writes manual mode but never `CreateActivity`, so an auto-mode element is refused and a manual-mode one is not).
...
**Status:** Implemented (ENG-92717) — `changeAccessRights` element type + `accessRights` block in build/`setElement`, describe read-back, serialization pinned byte-for-byte against designer specimens (capture doc in the package repo); MCP surface in this change. Verified live (2026-09-01, stand run): build, modify, describe and RUNTIME — 35/35 manual cases pass, including a real grant, a surgical revoke, a selectedEmployees run, and the designer-card captures that closed byte parity in BOTH directions (the card renders what the service writes, and the service decodes what the card writes). The package repo capture doc (docs/change-access-rights-element-capture.md, "Designer-card parity") is the single source of truth for that verification. Land WITH the CrtProcessBuilder rebundle PR: clio.mcp.e2e scenarios (create with `accessRights` + describe read-back; setElement replace/`[]` clear; one server-refusal path) and the ClioRing compatibility statement (additive description text only) — the bundled 1.3.1.1 predates the element, so the E2E cannot pass earlier. The clio-knowledge deliverable ships from its own branch with a `libraryVersion` bump: a new `process-access-rights` article owning the block shape, plus `process-modeling`/`data-elements` pointers and a pinning fixture. RESOLVED (story owner, 2026-09-02): `create-business-process` is now `Destructive = true`. Three review rounds argued it should flip, because the tool can author a `changeAccessRights` element with `remove` entries (or `level: restrict`) on a `signalStart` process, i.e. a permission revoke that later fires autonomously. CORRECTION to an earlier version of this note: it claimed flipping the flag would trip the ClioRing compatibility gate. That was asserted without checking and is NOT true — `clio-ring/` references no process-designer tool and `McpToolCompatibilityCatalog.cs` has no `business-process` entry at all (it pins legacy-name aliases, not tool annotations), so no Ring consumer reads this flag. What survives is the narrower argument that the TOOL creates a schema, which is additive, while the revoke happens when the process later runs — the same reason `modify-business-process` is `Destructive = true` and this one is not. The deciding point: `Destructive` is the machine-readable field an MCP host reads to decide whether a call needs human approval, and a host set to auto-run non-destructive tools would otherwise install a `signalStart` + `changeAccessRights` process — one that revokes record permissions without further human action — unattended. Description prose reaches the model, not that logic. The cost is accepted deliberately: hosts now prompt on EVERY create-business-process call, including harmless ones. The confirmation requirement also opens both tool `[Description]`s, which every client reads on the ordinary tools/call path, with the prompts carrying the elaborated version. `addElement` deliberately does NOT document `accessRights`: the server applies only `email`/`performer` blocks there (like `readData`/`changeData`, configure via `setElement` later in the same operations array). No docs-to-attributes drift test guards `docs/McpCapabilityMap.md`; recorded here as an accepted follow-up. RELEASE ORDERING (a constraint, not just a deliverable): the tool descriptions point agents at `get-guidance name=process-modeling`, and the shape they describe is owned by the new `process-access-rights` article, so this change MUST NOT ship ahead of the clio-knowledge release that publishes it — that article plus the widened buildable-slice wording must land with a `libraryVersion` bump no later than the clio release carrying these descriptions, or the two mandated agent surfaces contradict each other for a release window. `validate-process-graph` E2E coverage is ADDED in this change, not deferred: `clio.mcp.e2e/ValidateProcessGraphToolE2ETests.cs` already carried the sendEmail classification precedent, and `ValidateProcessGraph_Should_ClassifyBuildTokens_AsKnownTypes` clones it over `readData`/`changeData`/`changeAccessRights`. It skips (not fails) without a reachable sandbox, per that fixture's documented tier contract. What remains deferred to the rebundle PR is only the SANDBOX-GATED half: the accessRights build/describe round trips, which need a stand carrying the new CrtProcessBuilder. Also due with that bundle: raise the `[RequiresPackage]` version floor on `CreateBusinessProcessOptions` and `ModifyBusinessProcessOptions` to the first version shipping this element, the way the email block (1.2.0.1) and the performer block did — the declarative precondition is the repo's first-class guard for a package too old to understand a block, and `AccessRightsBlockExpectation` is the behavioural backstop, not a replacement for it.
