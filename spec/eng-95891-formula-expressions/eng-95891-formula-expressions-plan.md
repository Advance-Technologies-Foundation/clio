...
- [ ] Probes P1–P6 run; P5/P6 verified on a stand by a human. *P1–P4 are automated. The stand was updated and the stored-level cases re-run on 2026-09-02 at 1.4.0.37 — 6 of 6 pass, recorded in `…-manual-test-run-2026-09-02.md` with a manifest. What remains is the BROWSER pass the prompt defers: designer rendering (conditional connectors drawn, no gateway element added) and runtime execution. The stand was then moved to the shipped 1.4.0.38 and the MCP E2E tier passes 69/69 against it. What is left needs a human at a screen.*
...
- [x] Package rebundled with a **raised** version (1.4.0.40); SHA-256, `ModifiedOnUtc` and producing-commit pins
  updated by the rebundle script, and the shipped bytes verified by unpacking rather than by version number
