...
- TeamCity **не серіалізує** білди `Team_Atf_ClioMcpE2eTests` для різних комітів однієї гілки: скрипт черги
  ...
  **Уточнення після 2026-09-10, остаточне:** одночасність сама по собі НЕ валить білд —
  16003887 відпрацював усі 48m54s паралельно з чужим білдом тієї ж конфігурації і став зеленим
  (а той чужий упав). Доведена причина раннього падіння лише одна: помилка компіляції під net8.0
  (16002823, 4.6 хв, наодинці). Причина 16000226 і 16003535 лишилась невстановленою — статус
  коміту не несе ані кроку, ані виводу компілятора, а лог TeamCity видно лише з корпоративної
  мережі. Практично: спершу виключити компіляцію обох TFM, далі читати лог, а не гадати за
  тривалістю. Що ще працювало на стенді, показує `.../commits/<sha>/statuses` (він містить
  `pending`-записи), а не `/status`.
...
Вибірка 2026-09-09 13:52 … 2026-09-10 11:26, лише **успішні** прогони:

| Група | n | min | max | середнє |
|---|---:|---:|---:|---:|
| `claude/repo-update-z18z41` (ці зміни) | 8 | 47.0 | 50.1 | **48.0 хв** |
...
них (52.0). На 34 успішних білдах — жодного винятку.
...
| Stage 4 — винесення 37 тестів харнеса | `4ed9ee04` | [16003299](https://teamcity-rnd.bpmonline.com/buildConfiguration/Team_Atf_ClioMcpE2eTests/16003299) | **success** | **47m 40s** (pending `07:43:34Z` → success `08:31:14Z`). П'ятий зелений прогін поспіль; тир `NoEnvironment` — 449 тестів замість 485, 37 перенесених виконуються за 1 с в unit-лейні | статус не несе кількості |
| мерж master + правки review + фікс лічильників | `c8b4b87` | [16003887](https://teamcity-rnd.bpmonline.com/buildConfiguration/Team_Atf_ClioMcpE2eTests/16003887) | **success** | **48m 54s** (`09:42:29Z` → `10:31:23Z`). Шостий зелений прогін. Трохи довший за попередні ~47 хв, бо весь час ділив стенд із чужим білдом 16003859 (той упав). Локально на цій же голові тир `NoEnvironment` — 451 тест, 446 passed, 2 контейнерні failures, 3 skip, 10m56s | статус не несе кількості |
| фінальні документи (виміри) | `b8b073d` | [16004065](https://teamcity-rnd.bpmonline.com/buildConfiguration/Team_Atf_ClioMcpE2eTests/16004065) | **success** | **47m 00s** (`10:39:41Z` → `11:26:44Z`). Сьомий зелений прогін і найшвидший із усіх | статус не несе кількості |
