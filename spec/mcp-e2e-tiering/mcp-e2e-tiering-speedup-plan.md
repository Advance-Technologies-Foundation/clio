...
- TeamCity **не серіалізує** білди `Team_Atf_ClioMcpE2eTests` для різних комітів однієї гілки: скрипт черги
  ...
  **Уточнення після 2026-09-10:** раннє падіння має ДВІ причини, і тривалість їх не розрізняє —
  накладання білдів (16000226 за 2.5 хв, 16003535 за 7.5 хв) і помилка компіляції під net8.0
  (16002823 за 4.6 хв, наодинці). Розрізняти треба за тим, чи працював інший білд: ендпойнт
  `.../commits/<sha>/statuses` показує `pending`-записи і реальний час старту, а `/status` — ні.
