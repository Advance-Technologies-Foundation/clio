...
| 6 | Винести 71 суто-unit тест із `clio.mcp.e2e` до `clio.tests` | 71 | ~0 хв, але вони перестають залежати від стенду і дають фідбек за хвилини замість ~45 хв | середній (треба рухати harness) |
...
Окремий, не-часовий висновок, який може виявитись важливішим за секунди: **сюїта таки виконується
на кожному відповідному PR**, і саме тому її тривалість — не внутрішня справа проєкту, а затримка
на шляху до merge. Механіка (перевірено на PR #1399):

```
pull_request (paths: clio/**, clio.mcp.e2e/**, cliogate/**, …)
   └─ .github/workflows/teamcity-mcp-e2e.yml  → черга Team_Atf_ClioMcpE2eTests
        └─ TeamCity Commit Status Publisher  → commit status "CLIO MCP e2e tests (ATF)"
             └─ висить у merge-box як pending на ~45 хв
```

Отже 71 тест, що вже сьогодні є unit-тестами і лежить у цьому проєкті, **виконується всередині
45-хвилинного білду з розгортанням Creatio**. Це аргумент за їх перенесення сильніший за економію
часу: мілісекундний асерт не має залежати від доступності стенду.
...
### 2.1 Як сюїта потрапляє на PR (перевірено на PR #1399)

Це треба зафіксувати явно, бо від цього залежить, чи взагалі має значення тривалість сюїти.

| Ланка | Артефакт | Перевірено |
|---|---|---|
| Тригер | `.github/workflows/teamcity-mcp-e2e.yml` (ENG-92669), `on: pull_request` для `clio/**`, `clio.mcp.e2e/**`, `clio.process.fixture/**`, `cliogate/**`, `Directory.Packages.props` (з винятком docs-only) | файл у репозиторії |
| Постановка в чергу | job'и `Queue TeamCity MCP e2e (attempt 1..3)` + gate `TeamCity MCP e2e trigger` на self-hosted runner'ах | check runs PR #1399 |
| Виконання | TeamCity `Team_Atf_ClioMcpE2eTests` | build [15999000](https://teamcity-rnd.bpmonline.com/buildConfiguration/Team_Atf_ClioMcpE2eTests/15999000) |
| Публікація результату | TeamCity Commit Status Publisher → **commit status** `CLIO MCP e2e tests (ATF)` на head SHA | `GET /pulls/1399/status` → `state: pending`, `context: "CLIO MCP e2e tests (ATF)"` |

Це **commit status**, а не GitHub Actions check run — тому його немає в списку `check_runs` і його легко
не помітити, аналізуючи лише `.github/workflows/`. Практичний наслідок: поки білд іде (~45 хв),
merge-box показує *«Some checks haven't completed yet»*, і PR з увімкненим auto-merge стоїть.

**Одна відкрита розбіжність, яку має підтвердити адмін репозиторію.** Коментар у заголовку
`teamcity-mcp-e2e.yml` (рядки 14–16) стверджує: *«Phase 1 is advisory (non-blocking) — the status is
informational and is NOT a required check»*, і в merge-box бейдж `Required` стоїть біля `Unit Tests`,
`Analyzer Tests`, `ClioGate Tests`, але не біля `CLIO MCP e2e tests (ATF)`. Прочитати branch
protection / rulesets з цього аналізу неможливо. Хай там як, для цього звіту різниця не принципова:
формально обов'язковий чи ні, статус з'являється в PR і тримає його в стані «не всі перевірки
завершені», тож 44m 20s — це затримка на шляху до merge, а не фонова робота.
...
Це тести, які вже сьогодні є unit-тестами — але лежать у проєкті, що гейтить PR через
стенд-залежний білд (§2.1), а не через швидкий unit-лейн.
...
**Чому це варто рухати.** Ці 71 тест виконуються на PR — але всередині ~45-хвилинного білду з
розгортанням Creatio (див. §2.1). Наслідки: (1) розробник дізнається про зламаний парсер XML через
~45 хв замість ~2 хв; (2) коли стенд недоступний або білд червоний з іншої причини, ці перевірки
не дають сигналу взагалі; (3) вони подовжують саме той білд, який є вузьким місцем.
...
- Аудит не оцінював `clio-ring` та вимоги ClioRing-сумісності до конкретних тестів — за `AGENTS.md`
  ...
  сумісності.
- **Статус перевірки в branch protection не встановлений** (§2.1): чи є `CLIO MCP e2e tests (ATF)`
  формально required — читається лише з налаштувань репозиторію, недоступних із цього аналізу.
  Коментар у workflow каже «advisory», бейджі в merge-box це не спростовують — але коментар писався
  на Phase 1 і міг застаріти.
- Фільтр кроку `Run_MCP_e2e_tests` у TeamCity відомий лише з коментаря в
  `Support/Configuration/McpE2ECategories.cs` (`TestCategory!=McpE2E.ProcessDesigner`). Він
  **виключальний**, тож тести з `[Category("Unit")]` у цьому проєкті в білд потрапляють. Сама
  конфігурація job'а лежить поза репозиторієм і не перевірялась.
