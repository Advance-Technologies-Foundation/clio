...
| 6 | done | R-1…R-4 and R-8a hold for the shipped cohort; the stdio-only gate is the enforcement (TC-E-601…604, TC-U-601) | **R-10** (G-1 carries forward — this is the first stage where call rate is real), **R-12** (holds, untested negatives) | R-5, R-6, R-9 |
| 7 | done | **R-5** (sticky scope key — the sticky registry keys on the resolved tenant, never on the caller's spelling), **R-6** (target normalisation, `SessionTargetNormalizer`, T-5's over-normalisation trap closed), **R-9** (sticky lifetime — deadline timer plus exit watcher), **R-10's G-1** (`CLIO_MCP_WORKER_CONCURRENCY` makes the cap operator-configurable, and the sticky ceiling keeps sticky work from starving per-call work) | the T-4 keyed-digest condition still stands: sticky keys are what would spread a fingerprint, and nothing tests the classification | — |
| 8 | done | **R-7** (the drain runs on the terminal-stage lease too — ADR §3.4 — so a deploy child cannot block on a full standard-error pipe and be misreported as lost), **R-11** (the per-message stdout bound is applied at the shared transport seam, so the streaming deploy path inherits it rather than opting in) | — | R-5/R-6 while the gate holds |
