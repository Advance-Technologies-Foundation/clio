...
| FR-07 | Uninstall pipeline steps (from `CreatioUninstaller`): Read configuration → Stop IIS site → Delete IIS site + app pool → Drop database → Delete application files → **Unregister environment (final, only after cleanup succeeds)**. Config-read failure is a **visible FAILED step + safe abort before IIS is stopped** (never silent-skip-but-report-success); AppPool profile cleanup is an **explicit skipped/not-supported** step shown only if a profile exists | Must |
