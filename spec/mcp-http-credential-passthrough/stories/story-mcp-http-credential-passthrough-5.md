...
- Notes:
  ...
  - **REVIEW FIX (2026-07-09, ENG-93208 batch, test gap T2):** the HTTP middleware pipeline had NO unit coverage — the gap that let B1 ship. `EnforcePlatformApiKeyGate` and `CaptureCredentialContext` were promoted `private static` → `internal static` (no behavior change; the methods were already extracted, not inline lambdas) and are now covered by `clio.tests/Command/McpServer/CredentialPassthroughMiddlewareTests.cs` against `DefaultHttpContext`: no-key-configured → passthrough false + header ignored + proceeds; key-configured-but-no-header → false + proceeds; key + header + matching Bearer → true + context captured; key + header + wrong key → 401 short-circuit + secret-free body; gate-before-capture ordering asserted as the item-key dependency contract (capture inert unless `PassthroughEnabledItemKey` is true).
