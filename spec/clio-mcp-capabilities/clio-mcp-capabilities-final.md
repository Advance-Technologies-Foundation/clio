...
> Готовий до публікації блурб для анонсу Speaking Corner.

**Clio MCP: capabilities**
*Що вміє AI‑агент на Creatio — і як це зроблено, щоб працювало з LLM.*

Clio — це CLI і MCP‑сервер для Creatio. Через MCP будь‑який AI‑агент отримує доступ до **120+ інструментів**
і веде повний цикл розробки — від підняття середовища до готової Freedom UI сторінки.

**🔵 Блок 1 — Функціональні можливості.** Пройдемо шлях від ідеї до готового застосунку: розглянемо
основні функції роботи з додатком, змінимо об'єктну модель і сторінку, додамо компонент із хендлером
і створимо бізнес‑правило.

**🟣 Блок 2 — Технологічні ньюанси.** Як віддати LLM 120+ інструментів через MCP і не забруднити
контекст. Один MCP для різних версій Creatio — як влаштоване версіонування каталогу компонентів.
І чому **guidance — це API**: агенту віддаємо не лише кнопки, а й те, як ними правильно користуватись.

🎬 *Буде жива демка: агент будує застосунок прямо з чату.*
