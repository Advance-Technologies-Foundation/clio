...
| D5 | A column outside a non-empty `readData.columns` list is refused. | `ReadDataUserTask.AddESQColumns` fetches only the listed columns; the reference would read null with no error. |
