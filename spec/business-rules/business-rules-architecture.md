...
flowchart LR
    subgraph Clio["clio"]
        ...
        subgraph Core["Business Rule Core"]
            subgraph EntityCore["Entity business rules"]
                ...
                EntityReadCommand["GetEntityBusinessRulesCommand"]
                EntityReadService["EntityBusinessRuleReadService"]
            ...
            subgraph SharedCore["Shared business-rule infrastructure"]
                ...
                ReverseConverter["BusinessRuleMetadataReverseConverter"]
                EsqDecompiler["EsqFilterDecompiler"]
            end

            subgraph ReadMcp["mcp (read)"]
                EntityReadTool["GetEntityBusinessRulesTool"]
            end
    ...
    EntityReadTool -->|"options"| EntityReadCommand
    EntityReadCommand -->|"package + entity schema"| EntityReadService
    EntityReadService -->|"schema name"| EntityAttributeProvider
    EntityReadService -->|"load add-on"| AddonService
    EntityReadService -->|"stored rule metadata"| ReverseConverter
    ReverseConverter -->|"apply-static-filter ESQ envelope"| EsqDecompiler
...
Reading is the reverse of creation and shares the create-side resolution and add-on loading:

1. `GetEntityBusinessRulesCommand` receives `{ environment-name, package-name, entity-schema-name }`.
2. `EntityBusinessRuleReadService` resolves the package UId and entity schema (reusing the create-side
   package resolver and attribute provider) and loads the current add-on schema via
   `BusinessRuleAddonService` / `AddonSchemaDesignerService.svc`.
3. It parses `schema.metaData.rules[]`, normalizes resource keys to build a `{uId}.Caption` map, and
   groups the auto-generated `Autogenerated_*` child rules under their `parentUId`.
4. For each non-child rule, `BusinessRuleMetadataReverseConverter` reconstructs the friendly
   `{ caption, condition, actions }` — the mirror of `BusinessRuleMetadataConverter`. Formula
   `set-values` are recovered by stripping the `#{EntitySchema}Record.<path>#` wrapper from the stored
   expression; `apply-static-filter` actions are decompiled by `EsqFilterDecompiler`, the deterministic
   inverse of `LocalEsqFilterBuilder` (lookup display names are read from the names embedded in the
   stored ESQ envelope, so no live Creatio queries are needed).
5. Rules are returned in stored order as the byte-identical create contract (no `name`/`enabled`),
   ready to feed back into create. Any rule that cannot be reconstructed fails the read with a
   rule-named error (no raw/partial output).

Design rationale: [read-business-rules-adr.md](./read-business-rules-adr.md).
...
- Read `apply-static-filter` rules with a full `EsqFilterDecompiler` (the deterministic inverse of `LocalEsqFilterBuilder`) rather than persisting friendly source at create time, keeping the create path unchanged and reading UI-authored rules losslessly. Lookup display names are recovered from the envelope itself, so read needs no live Creatio queries.
- Keep `BusinessRuleMetadataReverseConverter` isolated as the mirror of `BusinessRuleMetadataConverter`, so the persistence model does not leak into the read contract.
...
- Capability doc (create): [create-entity-business-rule-spec.md](C:/Projects/clio/spec/business-rules/create-entity-business-rule-spec.md)
- Capability doc (read): [read-entity-business-rule-spec.md](C:/Projects/clio/spec/business-rules/read-entity-business-rule-spec.md)
- Read ADR: [read-business-rules-adr.md](C:/Projects/clio/spec/business-rules/read-business-rules-adr.md)
