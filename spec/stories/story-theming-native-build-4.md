...
> `BaseTool`**, per R-02: constructor-injects `IThemeCssBuilder` + `IThemeTemplateProvider`). Both gated by
> **`[FeatureToggle("theming")]`** on the **options class AND the tool type** (R-12 toggle-only, **dark
> until the surface is complete + go-live approved**). Safety flags `ReadOnly=true/Destructive=false/Idempotent=true/
> OpenWorld=false` (env effects — R-17). DI in BindingsModule + Program wiring (**full-suite trigger**) +
> **mandatory `clio.mcp.e2e`** (NOT in CI — manual).
...
piped into `create-theme-by-environment`'s `css-content` (MCP) — without shelling out to Node, composing with
both flows like a pure pipe (D1)
...
- [ ] **AC-01 (R-02, flat tool)** — Given `BuildThemeTool`, when inspected, then it is a `[McpServerToolType]`
  that **does NOT derive from `BaseTool`** and **constructor-injects** `IThemeCssBuilder` + `IThemeTemplateProvider`
  directly (assert via reflection on base type + ctor params) — the `ComponentInfoTool` shape, **not** the
  `BaseTool`/`IToolCommandResolver` pattern (`BaseTool.ResolveFromCallContainer` throws for any
  non-`EnvironmentOptions` type, so `BaseTool` is wrong here). **No `BaseTool.cs` edit.** It returns
  `BuildThemeResult { success, css, descriptor, warnings?, error? }`. (R-02; TC-U-11.)
- [ ] **AC-02 (arg mapping + safety flags)** — Given the tool, when invoked, then `primary`, `secondary`,
  `accent`, `success`, `error`, `css-class-name`, `heading-font`, `body-font`, `font-weights`, and optional
  `version`/`environment-name` map onto the build input; it reads the bundled template via `IThemeTemplateProvider`,
  calls `IThemeCssBuilder.Build`, and returns `BuildThemeResult`. The `[McpServerTool]` flags are
  **`ReadOnly=true, Destructive=false, Idempotent=true, OpenWorld=false`** and the `[Description]` routes the
  agent to `get-guidance theming`. (D1, R-03; TC-U-12.)
- [ ] **AC-03 (R-17 env-effect semantics)** — Given the safety flags, when documented, then `ReadOnly`/
  `Idempotent` describe **environment** effects; reading the bundled template and the optional
  `--environment-name` version probe are read-only and **outside the flag's scope** (mirror the `get-component-info`
  wording). The tool touches an environment **only** to *resolve the template version* (R-03), never to mutate.
  (R-17; TC-U-12.)
...
- [ ] **AC-06 (CLI workspace-dir mode, R-17/OQ-03)** — Given `--output <dir>` (workspace-dir mode), when the
  command runs, then it writes **both** `theme.css` (built) and `theme.json` (filled from the bundled
  `theme.json.tpl` — Story 3, accepting `--id`/`--caption`; auto-UUID / css-class-name when omitted). The MCP no-code flow **never** gets `theme.json` (the
  asymmetry per D5/OQ-03 — `create-theme` takes id/caption/cssClassName as separate args). (D1, OQ-03, R-17;
  TC-U-31, TC-I-01.)
...
(`ComponentInfoTool` shape — constructor-injects `IThemeCssBuilder` + `IThemeTemplateProvider`, **NOT** `BaseTool`,
no command/resolver machinery); returns `BuildThemeResult { success, css, descriptor, warnings?, error? }`; flags
`ReadOnly=true/Destructive=false/Idempotent=true/OpenWorld=false`; `[Description]` → `get-guidance theming`;
optional `version`/`environment-name` (R-03).

**Key file (create): `clio/Command/McpServer/Tools/` model `BuildThemeResult`** — `record { bool success;
string css; string descriptor; IReadOnlyList<string> warnings; string error; }` (mirrors `ComponentInfoTool`'s structured result + `ListThemesResult`; `warnings` follows `ApplicationDataForgeResult.Warnings`).
...
CI, manual): the real `clio mcp-server` advertises `build-theme` with the four flags + the `get-guidance
theming` description when the toggle is enabled in the harness (bundled template); it is
**absent** when the toggle is off; invoking with a valid `primary` + `css-class-name` returns
`BuildThemeResult { success:true, css:<valid theme.css> }`.
...
|------|-------------|------|
| Unit `[Category("Unit")]` `[Property("Module","McpServer")]` | **TC-U-11** flat `ComponentInfoTool`-style tool shape (NOT `BaseTool`; ctor-injects `IThemeCssBuilder`+`IThemeTemplateProvider`; no `BaseTool.cs` edit); **TC-U-12** arg mapping → `Build`, safety flags `true/false/true/false`, description → guidance, R-17 bundled-read-outside-flag note | `clio.tests/Command/McpServer/BuildThemeToolTests.cs` |
...
| E2E `[Category("E2E")]` (manual, NOT in CI) | **TC-E2E-01** real `clio mcp-server` advertises `build-theme` with `ReadOnly=true/Destructive=false/Idempotent=true/OpenWorld=false` + `get-guidance theming` description (toggle on, bundled template); valid `theme.css` produced; **absent** when toggle off | `clio.mcp.e2e/BuildThemeToolE2ETests.cs` |
...
- [ ] `BuildThemeTool` is **flat** (`ComponentInfoTool`-style — NOT `BaseTool`), constructor-injects `IThemeCssBuilder`+`IThemeTemplateProvider`, returns `BuildThemeResult`, **no `BaseTool.cs` edit** (R-02)
...
- [ ] Safety flags `ReadOnly=true/Destructive=false/Idempotent=true/OpenWorld=false`; description → `get-guidance theming`; R-17 env-effect note documented (bundled read + `--environment-name` probe outside the flag scope)
...
- [ ] `clio.mcp.e2e` `build-theme` coverage added (toggle-on advertise + toggle-off absent + valid CSS) — flag: NOT in CI, manual
