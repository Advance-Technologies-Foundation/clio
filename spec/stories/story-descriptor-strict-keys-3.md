...
Acceptance:
- [ ] `IProcessDescriptorKeyGuard` refuses when the environment is not newer than the bundle, warns when it
      is newer or when a version cannot be read, and reads the installed version only when a key was found.
- [ ] Called from `CreateBusinessProcessService`, `ModifyBusinessProcessService` and
      `ModifyProcessAsNewVersionService` right after parsing — before the page-facts check and the POST.
- [ ] The per-block read-backs stay. TC-U-09, TC-U-10, TC-U-11, TC-U-13, TC-U-14.
