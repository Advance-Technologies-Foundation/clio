...
- **The relay owns the child's transport read loop** (rule 12). Measured on SDK 1.4.1: forwarding through `McpClientHandlers.NotificationHandlers` reordered `0..5` into `[5,4,2,3,0,1]`, and a single-consumer FIFO in the parent did **not** fix it (`[2,0,1,3,4]` on retry). The two runs disagreeing is the evidence that the SDK dispatches notification callbacks **concurrently** — a FIFO fed by racing producers preserves their disorder faithfully, so no buffering depth helps. Owning the read loop works because it takes messages off the wire serially, before SDK dispatch is involved (ADR §3.2).
