...
> **Частично устарело.** Версия пакета больше не зашита константой `BundledPackages.ProcessBuilderVersion`
> и не является полом `[RequiresPackage]`: она читается из самого архива (`IBundledPackageCatalog`), пять
> гейтов проверяют только присутствие, а «среда отстала» — отдельное правило сходимости. Требование
> четырёхсоставности осталось в силе, но относится к версии в дескрипторе. См.
