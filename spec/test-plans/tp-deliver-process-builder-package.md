...
| **The deadline branch reporting a verdict.** The MCP response deadline can fire while the install is still running, and the branch answers exit 0. If it reads as success an agent proceeds against a package that may still fail to install; if the detached run then fails, its exit code has no response to travel on. | Med | High | TC-U-14 asserts the notice states it is NOT a verdict and does not send the caller back to the installer. The stderr report of a post-deadline failure IS asserted, by `InstallProcessBuilder_Should_ReportToStdErr_When_TheInstallFailsPastTheDeadline` (captures Console.Error and requires the tool name, "FAILED" and the exit code) — this row previously called it an open residual, which invited someone either to add the test that exists or to delete the real one as unneeded. |
...
| TC-U-48 | `Execute_ShouldInstallWithAWarning_WhenTheShippedVersionCannotBeRead` + `Execute_ShouldInstall_WhenTheInstalledVersionCannotBeRead` + `Execute_ShouldInstall_WhenThePackageIsAbsentFromTheEnvironment` | the three proceed-rather-than-refuse branches. Only the first two WARN; the absent-package branch proceeds silently, and its test asserts no warning — there is nothing to warn about when there is no version to compare. The `...` shorthand used here before implied the wrong prefix for the last two, naming tests that do not exist |
...
- **Full unit suite, not the module filter.** `clio/Common/` (`BundledPackages`) and the MCP `BaseTool` /
  ...
  policy in `AGENTS.md`. Last run: 8723 passed, 0 failed, 25 skipped. (Counts in this document are a snapshot, not a pin — the suite grows; treat a mismatch as staleness here, not as a regression.)
...
3. ~~**The post-deadline stderr report** is not asserted.~~ It is: see `InstallProcessBuilder_Should_ReportToStdErr_When_TheInstallFailsPastTheDeadline`. Listed here in error; struck rather than deleted so the correction is visible.
