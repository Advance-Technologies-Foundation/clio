...
| FR-25 | Support de-conversion via `enabled: false`, subject to OQ-01. The platform has the operation (`convertToSingleInstance`, pinned by the platform's own Jasmine spec) and it is lossless for the callee's parameters. | Could |
...
|---|---------|-----------|-------|-----|
| OQ-01 | **Does de-conversion (`enabled: false`) ship in v1?** | FOR: the mechanism is cheap and bounded, and without it the only route back is `removeElement` + `addElement`, which changes the element UId and drops its flows and mappings. AGAINST: it is a destructive write an agent can issue, and the designer's own `convertToSingleInstance` is lossy in the same way — the XOR'd `Variable` twin is discarded. A product call about reversibility. | Product owner | TBD |
...
| OQ-06 | **Is in-place update of `executionMode` / `ignoreErrors`** on an already-multi-instance element supported (recommended), or must the caller de-convert and re-convert? | In-place is the obvious ergonomics; requiring a cycle makes every mode change destructive, and it depends on OQ-01 being answered YES. | Product owner | TBD |
