...
| FR-21 | Retract the 19 shipped statements that promise multi-instance is refused — 8 in clio, 11 in CrtProcessBuilder, two of them **user-visible run-time messages** — and land the clio-knowledge guidance change: seven files (`sub-process.md`, `parameters.md`, `element-catalog.md`, `process-modeling.md`, `read-data.md`, the `bundle-source.json` description, and a new `routing.md` entry), eleven statements, with a `libraryVersion` bump, plus the `curated-knowledge-names.json` re-pin on the clio side. `sequence` is derived at build time and is never hand-authored. | Must |
...
| Guidance (clio-knowledge) | A pull request in **another repository** spanning **seven files** and **eleven statements**, with a `libraryVersion` bump. `sequence` is derived at build time and never hand-authored. |
...
| OQ-02 | **What happens when the callee is retargeted on a multi-instance element?** | ~~Refuse — or reproduce the designer, which de-converts unconditionally.~~ **That second description is WRONG** and is retracted: the designer's `resetParameters` de-converts only transiently and RE-converts from the same five parameter objects. | Product owner | **DECIDED 2026-09-22 — reproduce the designer.** The retarget succeeds, the element STAYS multi-instance, the five UIds survive, the collections are re-derived. Adds scope: `resync: true` must work on such an element too, because the designer reaches the same method from its re-synchronization path |
| OQ-03 | **Is `inSync` redefined**…? | The question turned out to depend on another: a field reporting "a re-sync is owed" is only worth having if there IS one to ask for, and for a multi-instance element there was none until OQ-02 was answered. | Product owner | **DECIDED 2026-09-22 — no.** `inSync` is frozen; a separate `calleeInSync` is added to the `multiInstanceOptions` describe block. **FR-24 does not activate** and no floor is added to describe |
| OQ-04 | **Output-collection strictness: refuse, or accept-and-warn?** | Refuse: 0 of 170 shipped output item properties carry a value, the platform wipes them on every synchronization, and the designer's `getEditableParameters:634-648` leaves only the INPUT collection editable in ordinary mode. | Product owner | **DECIDED 2026-09-22 — refuse**, per the designer |
| OQ-05 | **Does `validate-process-graph` grow any rule at all?** | It cannot see multi-instance today, and any rule would be **new surface** duplicating refusals in a validator that cannot see the element's real state. The designer has no such validator either. | Product owner | **DECIDED 2026-09-22 — no rule**, per the designer |
...
| OQ-07 | **Is a collection SHAPE check in scope?** | ~~Whether the platform fails at run time or silently yields empty items is not established.~~ **Now measured, and the answer changes the question**: the shipped `SubProcess2` maps a source collection of `Id, Name` onto a contract of `LicenseProductName, LicensePackage, ExpireDate, NextExpireDay` — zero overlap, in product content that runs. Each item property carries its OWN mapping; the source supplies the iteration count, not a shape to match. A shape check would refuse shipped content. | Product owner | **DECIDED 2026-09-22 — out of v1**, per the designer, and on evidence rather than on an unknown |
