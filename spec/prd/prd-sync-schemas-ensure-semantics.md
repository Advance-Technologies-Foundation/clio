...
| FR-04 | `update-entity` reconciles the requested column set against current state per-column: for each requested column, add-if-absent / modify-if-different; a requested `remove` means "ensure absent" (already-absent is success). Columns not named in the request are left untouched. Only an explicit `modify` converges a type divergence: an `add` — explicit, or coerced from a `columns` add-batch — that names a present column of a **different** type is a **column collision** (`success: false`, `outcome: collision`, every colliding column named), never a silent type mutation of the pre-existing column. A present column of the **same** type still makes the `add` an idempotent no-op. | Must |
