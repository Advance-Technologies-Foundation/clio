...
- [ ] Goal 4 — the version-blind paths this feature owns are closed. SM-04: `describe --process-caption`, `generate-process-model`, `get-process-signature` and `run-process` each resolve to, or name, the active version — four paths, enumerated, not "all of them". Counter: an unversioned process MUST NOT start failing where it previously worked.
...
- `get-process-signature` INHERITS the shared resolver's active-version policy — this non-goal did not survive implementation, and the record is corrected rather than the code. It resolves through `IProcessModelGenerator.Generate` → `ProcessModelGenerator.GetProcessIdFromName` → `ProcessLibResolver.Resolve`, which is the same seam `describe --process-caption` goes through, so the policy reached it the moment the seam was shared. Nothing was added to make it version-aware and nothing short of an explicit policy parameter on the seam could have kept it out. Since it returns the parameter signature — the very field that differs across versions — inheriting the policy is the better answer anyway, and the shipped `[Description]`, `clio/docs/commands/get-process-signature.md` and `clio/help/en/get-process-signature.txt` all state it. SM-04 therefore counts four paths, not three; the CLI Impact table below carries its row. The follow-up this bullet asked for is not needed.
...
| **Changed behaviour of an existing CLI verb** | `get-process-signature` (`[Verb]` at `GetProcessSignatureCommand.cs`, and reached again through `PageUpdateTool`) resolves a caption to the active version, because it shares `ProcessLibResolver` through `IProcessModelGenerator`. Doc targets that moved with it: `clio/help/en/get-process-signature.txt`, `clio/docs/commands/get-process-signature.md` | Behavioural — a caption that resolved to an arbitrary family member now resolves to the active one, and a lone match the view flags explicitly NOT active now fails |
...
| # | Question | Owner | Due |
|---|---------|-------|-----|
| ~~OQ-01~~ **CLOSED 2026-09-03 (story 8)** | The package is decided in two steps and must be an INPUT to the create handler. `getNewSchemaVersion` uses the SOURCE's own package when it is editable (`sysPackage && canEditPackageSchema`); otherwise `_getPackageForNewSchemaVersion` branches on the feature `SaveProcessVersionInApplicationPackage` — disabled → current package, enabled → design package. That feature is ON here (DefaultState 0, override 1), so a version of a NON-editable source lands in the design package. Confirmed by data: 3 of 14 visible families are cross-package. Trap for the handler: in the non-editable branch the NAME still uses `CUSTOM_PACKAGE_NAME` ("Custom"), so name and location can disagree | Architect | closed |
| ~~OQ-02~~ **CLOSED 2026-09-03 (story 8)** | ON. `Feature.DefaultState = 1` for `UseNewSchemaHierarchyFolding` with no `AdminUnitFeatureState` override on the reference stand, so the manager's third ordering key is the package `HierarchyLevel`. FR-02 and FR-19 never depended on the answer — the response states its provenance instead of naming a key — and the guidance article can now describe the divergence concretely | Architect | closed |
