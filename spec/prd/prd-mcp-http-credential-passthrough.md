...
|---|-----------|--------------|
| A-01 | ENG-92865 (which this "builds on") adds — or the client layer can be extended to add — a token/cookie authentication path so `IApplicationClient`/`CreatioClient` can authenticate from a pre-obtained `accessToken`/`cookie`. Today `EnvironmentSettings`/`EnvironmentOptions` model only `Login`/`Password`/`ClientId`/`ClientSecret`; there is **no** token/cookie slot on the main client path (the only cookie-harvesting code is the separate `Common/BrowserSession` client). | If the client cannot consume a pre-obtained token/cookie, the preferred `{url, accessToken}`/`{url, cookie}` legs are infeasible and passthrough degrades to `{url, login, password}` only — undermining "log in once → reuse token." Blocking; see OQ-01. |
...
|---|---------|-----------|-------|-----|
| OQ-01 | Does ENG-92865 deliver a token/cookie auth path on `IApplicationClient`/`CreatioClient`, or must this feature add `AccessToken`/`Cookie` to `EnvironmentSettings` and the client? (Resolves A-01; see Blocking Prerequisite.) | **YES — blocking.** Current client seam cannot consume token/cookie; without this the preferred legs are infeasible. | Architect | Before ADR |
