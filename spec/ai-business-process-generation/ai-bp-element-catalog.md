...
| `addDataUserTask` | Add data / Добавить данные | Create record(s) in the background (no UI, bypasses user perms). | object, mode (Add one record / Add selection), field-value mapping, selection filters | **new record `Id`** — the `RecordId` parameter, mapped BY NAME (`describe` does not list it: the platform declares it neither `IsResult` nor `Out`); returns only the Id → chain a Read data for other fields |
...
| `changeAdminRightsUserTask` | Change access rights | Grant/revoke record permissions. BUILDABLE via the element's `accessRights` block (object + add/remove entries: operations, level, grantee role/employee/selectedEmployees) plus a record `filter` (ENG-92717). No outputs and no runtime feedback; one configuration builds green and still does nothing - `add`/`remove` both empty (a block carrying only `object` is one) - so a green build is not evidence a grant or revoke was applied. A record `filter` that is PRESENT but carries no conditions is the same runtime no-op (the "filters empty" exit) but is REFUSED at build by a current package. The total ABSENCE of a record filter is the OPPOSITE state: the runtime never enters its filter branch, so the change lands on EVERY record of the object. | object, filter, users/roles, permission type (Read/Edit/Delete), action (Grant/Revoke), level | — |
...
|---|---|---|---|---|
| `activityUserTask` | Perform task / Выполнить задачу | Create an activity (to-do/meeting) for a user. | `caption`, `ActivityCategory` (required by the designer UI only — no metadata `IsRequired` flag; runtime falls back to "To do"), `OwnerId` (the performer — the designer labels it "Who performs the task?"), `StartIn`+`StartInPeriod`, `Duration`+`DurationPeriod`, `RemindBefore`+`RemindBeforePeriod`, `ActivityPriority`, `ShowExecutionPage`, `ShowInScheduler`, `InformationOnStep`, `Recommendation` — a SUBSET of the element's 37 declared parameters (19 are "Connected to" connections, 4 are outputs/technical); the authoritative per-parameter table lives in the `process-modeling` guidance ("Element: Perform task") | yes (activity result; note: conditional flows are not buildable from clio yet, so a clio-built process cannot branch on it — flows/gateways land with ENG-91853; limitation recorded by ENG-91846) |
...
| `openEditPageUserTask` | Open edit page | Open a record page to add/edit. **Buildable via `type:"openEditPage"` (ENG-92715).** | `PageSchemaId` (a page registered on a SECTION; the object and, for a typed object, `PageTypeUId` are DERIVED from it), `EditMode` (0=add/1=edit), `RecordColumnValues` (add) / `RecordId` (edit) — mutually exclusive in STORAGE, `Recommendation` (required, single line), `InformationOnStep`, `IsMatchConditions` + `DataSourceFilters` (the runtime gates the filter on the flag), `OwnerId`/`RoleId` + `PerformerAssignmentOptions` and `ShowExecutionPage` (`Who performs the task?`, written explicitly at create). `CreateActivity` + `StartIn`/`StartInPeriod`, `Duration`/`DurationPeriod`, `RemindBefore`/`RemindBeforePeriod`, `ShowInScheduler`, `ActivityPriority` (`Log activity`; each interval is written as a value+unit PAIR, never one alone, and the priority as a BARE record Guid because its designer card reads the raw value). `GenerateDecisionsFromColumn` + `DecisionalColumnMetaPath` (the column as its UId; cleared as the literal string `"null"`, the designer's own marker) — but the CONDITIONAL flows that would route those results are not buildable yet (ENG-91853). NOT buildable: | yes (column results) |
| `autoGeneratedPageUserTask` | Auto-generated page | Dynamic form of fields + buttons. **NOT buildable.** Pick it only when there is NO record whose columns are edited — a request naming an object's columns is `openEditPage`. | page title, `Who performs`, buttons (each may emit a signal), page items | **yes** (clicked button) |
| `preconfiguredPageUserTask` | Pre-configured page | Open a custom Freedom/Classic UI page. **NOT buildable.** Pick it only when a SPECIFIC existing page is named; a wish for a nicer layout is not that signal. | which page, `Who performs`, page parameters, completing buttons | yes |
| `emailTemplateUserTask` | Send email / Отправить e-mail | Send email (auto) or open pre-filled email (manual). **Buildable via `type:"sendEmail"` (ENG-92706): custom message only.** | `Sender` (Lookup→MailboxSyncSettings), dynamic `Recipient<N>`/`CopyRecipient<N>`/`BlindCopyRecipient<N>` element parameters (To/Cc/Bcc), `Subject`, `BodyTemplateType` (0=template/1=custom), `Body`+`BodyConfig`, `Importance` (0–3), `IgnoreErrors`, `SendEmailType` (0=auto/1=manual), manual mode: performer assignment (`PerformerAssignmentOptions` user/manager/role + `OwnerId`/dynamic `RoleId`), `ShowExecutionPage` | `ActivityId`, `IsActivityCompleted` |
| `approvalUserTask` | Approval / Визирование | Create an approval, assign approver, branch on outcome. **Buildable via `type:"approval"` (ENG-92713): everything below EXCEPT branching on the outcome, which needs gateways (ENG-91853).** | approval object, record Id, approver (Employee/Manager/Role), delegate flag, notify options | **yes** (approved/rejected) |
