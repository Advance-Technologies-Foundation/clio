...
| `changeAdminRightsUserTask` | Change access rights | Grant/revoke record permissions. BUILDABLE via the element's `accessRights` block (object + add/remove entries: operations, level, grantee role/employee/selectedEmployees) plus a record `filter` (ENG-92717). No outputs and no runtime feedback; two configurations build green and still do nothing - a record `filter` that is PRESENT but carries no conditions (the runtime takes its "filters empty" exit), and `add`/`remove` both empty (a block carrying only `object` is one) - so a green build is not evidence a grant or revoke was applied. The total ABSENCE of a record filter is the OPPOSITE state: the runtime never enters its filter branch, so the change lands on EVERY record of the object. | object, filter, users/roles, permission type (Read/Edit/Delete), action (Grant/Revoke), level | — |
