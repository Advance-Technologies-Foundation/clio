...
[Binding]
public class PublishWorkspaceCommandStepDefinitions(ICLioRunner clio) : IDisposable
{
    ...
    private void OnTempCreated(object sender, FileSystemEventArgs e) => _tempDirItems.Add(e.FullPath);

    private void OnWorkDirCreated(object sender, FileSystemEventArgs e) => _workingDirItems.Add(e.FullPath);
    ...
    public void GivenIDoNotSetEnvVariableClioWorkingDirectory()
    {
        ...
        _tempWatcher = new FileSystemWatcher { Path = tempDir };
    ...
    public void GivenISetEnvVariableClioWorkingDirectoryTo(string clioWorkingDirectory)
    {
        ...
        _watcher = new FileSystemWatcher { Path = Path.Combine(_clioWorkingDirectory, "clio") };
        _watcher.Created += OnWorkDirCreated;
        _watcher.EnableRaisingEvents = true;

        _tempWatcher = new FileSystemWatcher { Path = Path.Combine(Path.GetTempPath(), "clio") };
        _tempWatcher.Created += OnTempCreated;
        _tempWatcher.EnableRaisingEvents = true;
    }
    ...
    public void WhenIExecutePublishAppWithArgs(Table table)
    {
        ...
        _clio.RunClioCommand("publish-app", cmdArgs, _clioWorkingDirectory,
            new Dictionary<string, string> { { "CLIO_WORKING_DIRECTORY", _clioWorkingDirectory } });
    }
