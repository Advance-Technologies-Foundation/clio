...
This is an isolated, executable mechanism prototype for Kirill's queue proposal. It is not a production Clio implementation or an approved architecture change. Source lives on branch `krylov/generation-queue-probe`, originally based on `089de2a75dec228564a150f6f719d9052ea3dcc3`. The first single-target slice was published at `0ab87e21c92bc00ee43ff41ee6b55a76cfd172a1`. The local [two-environment extension](MULTI-ENVIRONMENT.md) tests whether the same coordinator can serve independent targets in one shared backend.
...
There is one executing backend with at most one owned operation per target (`A` and `B`). The targets can have overlapping work; their control events are serialized. One queue contains target- and generation-tagged entries; a single supervisor event loop serializes admission, dispatch and replacement. No message broker, simultaneous old/new backend execution, or movement of running operations is needed for this fixture. Replacement drains all targets sharing the backend; it is not an update of a Creatio environment.
...
**Acceptance is not a promise to execute eventually.** The receipt now says `accepted-until-supervisor-exit`: the supervisor has accepted the request into memory. Cancellation, expiry, update failure or deferral can resolve it as `NotStarted`. Capacity exhaustion and stale/incompatible callers get explicit rejection. A stale-generation response includes the accepting generation and contract. There is no unconditional “always accept” guarantee; the clearer receipt does not add crash recovery.
...
- The shared queue capacity bounds waiting entries, excluding the at-most-two running requests. Waiting requests may have deadlines and can be cancelled; this fixture does not implement cancellation of dispatched work or per-target fairness quotas.
...
The script builds Release, runs all scenarios, runs Q2 with a deliberate unsafe fallback mutation, and Q11 with global serialization and ignored-B-cleanup mutations (all three must fail). It restores the normal rules and reruns Q2/Q11. It captures actual native exit codes, source hashes, base commit, runtime information, TRX results, client transcripts, independent effect files and process cleanup evidence under `evidence/<UTC timestamp>/`. A script success means each mutation failed at its intended oracle, not that unsafe code passed.
...
| Q11 | Independent A/B work with old backlog and held cleanup | Both start in one worker before release; B progresses while A waits; neither target's cleanup is skipped; exact target/generation/configuration/effects survive one shared replacement |
| Q12 | Only B will not drain | Shared update defers; both V2 queues become `NotStarted`; V1 A progresses while B remains held |
| Q13 | Shared worker dies with different A/B outcomes | A's published success survives, B's effect without outcome stays `Unknown`, untouched queues are `NotStarted`, no replay |
...
The work queue is bounded, but ingress and retained result history are not globally memory-bounded. Client output backpressure can delay the supervisor event loop. A client that keeps submitting stale V1 requests after cutoff is rejected; the response supplies the current contract, but automatic client adaptation is not proved. Two fixture targets have independently held in-flight operations; real Creatio I/O, parallel CPU execution, credential isolation, arbitrary target counts, nested operations and multiple workers are not modeled. These limits should remain explicit when presenting results.
