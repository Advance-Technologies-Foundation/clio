...
policy is additionally evaluated **from inside a running MCP host**: thirty seconds after the
transport starts serving, and every five minutes afterwards, the host asks the same schedule whether
a knowledge update is due and runs it when it is (ENG-99899). Without it, an installation used only
...
- The `knowledge` policy is evaluated both at CLI startup and, every five minutes, inside a running
