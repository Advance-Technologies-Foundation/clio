...
- The clio self-update is deferred while an MCP host (`mcp-server` or `mcp-http`) is resident in the
  same clio home — replacing binaries under a running host breaks it for the rest of its session.
