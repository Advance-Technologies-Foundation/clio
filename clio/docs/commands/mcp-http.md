...
As the endpoint starts accepting requests, Clio bootstraps the same built-in
...
uses the validated local checkout without a remote update check. A missing checkout is cloned in
the background, so Git latency cannot delay HTTP startup and the running host sees the knowledge
when installation completes. The source cannot be removed;
disable it with `clio disable-knowledge-source --alias creatio-curated`. A bootstrap retrieval
failure is logged as a warning and does not prevent the HTTP host from starting.
