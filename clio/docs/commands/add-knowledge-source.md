...
into Clio's visible knowledge root and updated with a fast-forward-only pull. With no Git reference, a
...
NuGet sources require their own `--trusted-key-id` and `--trusted-public-key-path`; these options
must not be supplied for Git sources. The path must be
...
NuGet sources must publish signed version 1 knowledge bundles. Git sources expose their source
manifest and content files directly in the repository checkout.
...
--trusted-key-id <id>       Required for NuGet; bundle signing-key ID
--trusted-public-key-path <absolute-path>
                            Required for NuGet; bounded local P-256 PUBLIC KEY PEM
