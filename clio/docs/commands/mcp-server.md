...
Use your MCP client to call tool-contract-get {"tool-names":["application-get-list","application-get-info","page-get","page-sync"]}.
Bootstrap an existing-app or page workflow from the authoritative contract before invoking discovery or mutation tools.

Use your MCP client to call tool-contract-get {"tool-names":["page-get","component-info","page-sync"]}.
...
- Start each MCP workflow with "tool-contract-get" so the client reads the authoritative clio MCP contract before the first discovery, inspection, or mutation call
- Preferred existing-app flow starts with tool-contract-get, then application-get-list -> application-get-info, then page or schema inspection, then page-sync / modify-entity-schema-column / schema-sync as needed
