...
enforced client-side and a per-value size cap (10 MB) applies. Every file must be a supported image
format — the logo slots accept png, jpg, jpeg, gif, bmp, webp and svg; the favicon also accepts ico.
A file with another extension is refused before anything is written to that slot.

When one image is refused (an unsupported format, or the environment rejects it) and another is
accepted, the run keeps what already succeeded:
...
--favicon                       Local image file for the browser-tab icon (FaviconImage). Pass a square icon; accepted formats: png, jpg, jpeg, gif, bmp, webp, svg, ico. Not taken from --logo.
