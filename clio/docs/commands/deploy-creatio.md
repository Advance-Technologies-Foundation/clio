...
certificate matching the host. If none is installed, deployment warns and continues over
HTTP. For dotnet deployment, this explicitly selects an HTTPS Kestrel endpoint and requires
`--cert-path` or an existing Kestrel certificate configuration; it does not fall back to HTTP.
...
--cert-path "/etc/ssl/certs/app.pem" --cert-key-path "/etc/ssl/private/app.key" \\
--zip-file "/opt/creatio-app.zip"
4b. Prefer HTTPS for local IIS (falls back to HTTP when no usable certificate exists):
...
For dotnet deployments, the generated `appsettings.json` contains the Kestrel certificate
configuration, including the password when `--cert-password` is supplied. Protect the deployed
application directory and avoid exposing the command line or configuration file to untrusted users.
