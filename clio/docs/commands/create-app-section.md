...
| `creatio-timeout` | The request was sent but Creatio produced no response within the budget. | clio automatically re-checks whether the section appeared anyway — the section transaction can still be committing when the budget expires, so the check is **retried with backoff** (not a single immediate read) before giving up, so a section that commits just after the budget expired is reported as a success rather than a spurious failure (ENG-94419). This re-check adds up to ~30s of read-only polling before a genuine timeout failure is reported. If the section still does not appear, do **not** retry blindly — the server may still be processing the insert. Wait a few minutes, run `clio list-app-sections`, and retry only if the section is still absent. |
