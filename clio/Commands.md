...
- `--type <TYPE>` (optional for modify, required for add): Column type. Supports `Guid`, `Text`, `ShortText`, `MediumText`, `LongText`, `MaxSizeText`, `Binary`, `Image`, `File`, `Blob`, `SecureText`, `Encrypted`, `Password`, `Email`, `EmailAddress`, `Integer`, `Float`, `Boolean`, `Date`, `DateTime`, `Time`, `Lookup`, plus designer-native text and decimal variants
...
- supported operation types include `Binary`, `Image`, `File`, `SecureText`, and `Email`; `Blob` is accepted as an alias for `Binary`; `Encrypted` / `Password` are accepted as aliases for `SecureText`; `EmailAddress` is accepted as an alias for `Email`
