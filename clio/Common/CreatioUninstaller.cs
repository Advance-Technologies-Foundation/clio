...
public class CreatioUninstaller : ICreatioUninstaller
{
    ...
    private void DeleteIISSite(string creatioDirectoryPath)
    {
        ...
        IISScannerHandler.UnregisteredSite site
            = AllSites.FirstOrDefault(all => all.siteBinding.path == creatioDirectoryPath);
        if (site is not null)
        {
            DeleteInstanceByNameRequest removeRequest = new()
            {
                SiteName = site.siteBinding.name
            };
    ...
    private void StopIISSite(string creatioDirectoryPath)
    {
        ...
        if (site is not null)
        {
            StopInstanceByNameRequest removeRequest = new()
            {
                SiteName = site.siteBinding.name
            };
            _mediator.Send(removeRequest);
