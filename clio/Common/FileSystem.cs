...
public class FileSystem : IFileSystem
{
    ...
    {
        Process mklinkProcess = Process.Start(new ProcessStartInfo("cmd", $"/c mklink /D \"{link}\" \"{target}\"")
        {
            CreateNoWindow = true
        });
    ...
    public string NormalizeFilePathByPlatform(string filePath)
    {
        ...
        string[] filePathItem = filePath.Split(new[]
        {
            '\\', '/'
    ...
    public string ReadAllText(string filePath) => _msFileSystem.File.ReadAllText(filePath, Utf8NoBom);
