...
public class Postgres : IPostgres
{
    ...
    public Postgres(int port, string username, string password) => _connectionString =
        $"Host=127.0.0.1;Port={port};Username={username};Password={password};Database=postgres";

    public void Init(string host, int port, string username, string password) => _connectionString =
