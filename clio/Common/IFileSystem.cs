...
public interface IFileSystem
{
    ...
    ///     Thrown when – path or pathToTarget is empty. -or- path or pathToTarget
    ///     contains invalid path characters.
    /// </exception>
    /// <exception cref="System.IO.IOException">
    ///     A file or directory already exists in the location of path. -or- An I/O error
    ///     occurred.
    /// </exception>
    ...
    /// <summary>
    ///     Checks if a file exists at the given file path. If the file exists and the delete flag is set to true, the file is
    ///     deleted.
    ...
    ///     .NET Framework and .NET Core versions older than 2.1: <paramref name="filePath" />
    ...
    ///     It uses the `GetFiles` method to retrieve all files in the directory and the `DeleteFileIfExists` method to delete
    ///     each file.
    ///     It also uses the `GetDirectories` method to retrieve all subdirectories in the directory and the
    ///     `SafeDeleteDirectory` method to delete each subdirectory.
    /// </remarks>
