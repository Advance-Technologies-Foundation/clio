...
public class SchemaBuilder : ISchemaBuilder
{
    ...
    public void AddSchema(string schemaType, string schemaName, string packagePath)
    {
        ...
        Dictionary<string, string> macrosValues = new()
        {
            ...
            }, //package maintainer otherwise Customer
            {
                "[PACKAGE_NAME]", pkgInfo.Descriptor.Name
            }, //package name or from path
            {
                "[SCHEMA_UID]", Guid.NewGuid().ToString()
            }, //Guid.NewGuid()
            {
                "[DATETIME_NOW_TICK]", modifiedOnUtc
            }, //DateTime.Now.Ticks
            {
                "[PACKAGE_UID]", pkgInfo.Descriptor.UId.ToString()
            } // UID from package descriptor
        };
