	...
	class Program
	{
		...
		private static int Main(string[] args) {
			...
			return Parser.Default.ParseArguments<ExecuteAssemblyOptions, RestartOptions, ClearRedisOptions,
					...
					RestoreNugetPkgOptions, InstallNugetPkgOptions, SetPackageVersionOptions, CheckNugetUpdateOptions, 
					GenerateModelsOptions>(args)
				.MapResult(
					...
					(GenerateModelsOptions opts) => Resolve<GenerateModelsCommand>(opts).Execute(opts),
					errs => 1);
		}
