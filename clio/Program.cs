...
namespace Clio
{
	...
	class Program
	{
		...
		private static void UnZipPackages(string zipFilePath, string packageName, EnvironmentOptions options) {
			IPackageArchiver packageArchiver = Resolve<IPackageArchiver>();
			packageArchiver.ExtractPackages(zipFilePath, true, true, false, "");
			IApplicationDownloader applicationDownloader = Resolve<IApplicationDownloader>(options);
			applicationDownloader.DownloadAutogeneratedPackages(packageName);
		}
		...
		private static int DownloadZipPackages(PullPkgOptions options) {
			try {
				...
				if (options.Unzip) {
					UnZipPackages(destPath, packageName, options);
				} else {
					UnZip(destPath);
				}
		...
		private static int Main(string[] args) {
			...
			return Parser.Default.ParseArguments<ExecuteAssemblyOptions, RestartOptions, ClearRedisOptions,
					...
					DownloadConfigurationCommandOptions>(args)
				.MapResult(
