...
public class ApplicationDownloader : IApplicationDownloader
{
    ...
    public void DownloadAutogeneratedPackages(string packageName) =>
        DownloadAutogeneratedPackages(new[]
        {
            packageName
        });
    ...
    public IEnumerable<DownloadInfo> GetDownloadInfoFromCsProj(IEnumerable<string> packagesNames)
    {
        ...
        foreach (Reference @ref in distinctRefs)
        {
            ...
            var payloadObj = new
            {
                packageName = @ref.PackageName, dllName
            };
            string packagesFolderPath = ApplicationFolderPath switch
                                        {
                                            var _ when ApplicationFolderPath.Contains("net-core") => Path.Combine(
                                                ApplicationFolderPath, "packages", @ref.PackageName, "Files", "Bin",
                                                "netstandard", dllName),
                                            var _ when ApplicationFolderPath.Contains("net-framework") => Path.Combine(
                                                ApplicationFolderPath, "packages", @ref.PackageName, "Files", "Bin",
                                                dllName),
                                            var _ => throw new ArgumentOutOfRangeException(
                                                nameof(ApplicationFolderPath), ApplicationFolderPath,
                                                "ApplicationFolderPath is not net-core or net-framework")
                                        };

            string requestData = JsonSerializer.Serialize(payloadObj);
            DownloadInfo di = new(GetUrl(ServiceUrlBuilder.KnownRoute.DownloadPackageDllFile),
