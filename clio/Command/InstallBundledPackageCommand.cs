...
/// </remarks>
public abstract class InstallBundledPackageCommand<TOptions> : Command<TOptions>
	where TOptions : InstallBundledPackageOptions {
	...
	/// </para>
	/// </remarks>
	private static readonly string OverrideHint =
	...
	// Read from the options type's own [Verb] rather than declared twice; the attribute is what the CLI
	// parser dispatches on, so it is the one name the user can actually type.
	private static string VerbName =>
		typeof(TOptions).GetCustomAttributes(typeof(VerbAttribute), false)
			.Cast<VerbAttribute>()
			.Single()
			.Name;
	...
	/// <summary>
	/// Initializes a new instance of the <see cref="InstallBundledPackageCommand{TOptions}"/> class.
	/// </summary>
	...
	protected InstallBundledPackageCommand(
	...
	private string GetPackagePath() =>
		_bundledPackageCatalog.GetArchivePath(PackageName);
	...
	private bool ShouldRefuseInstall(out string message) {
		...
		if (!string.IsNullOrWhiteSpace(shippedVersion.Suffix)) {
			...
			message =
				$"Refusing: this clio distribution declares {PackageName} "
		...
		try {
			...
			Task<PackageVersion> read = Task.Run(() =>
				_requiredPackageChecker.GetInstalledVersion(PackageName));
		...
		message =
			$"Refusing: the environment carries {PackageName} "
	...
	/// <returns>
	/// Returns 0 only when the package installed AND the package's Ping answers afterwards;
	/// otherwise, returns 1. There is no already-current branch — see the comment at the install site.
	/// </returns>
	public override int Execute(TOptions options) {
		try {
			string packagePath = GetPackagePath();
			if (!_bundledPackageCatalog.ArchiveExists(PackageName)) {
				...
				_logger.WriteError(
					$"The bundled {PackageName} package was not found at " +
			...
			if (!success) {
				_logger.WriteError(
					$"Failed to install the bundled {PackageName} package.");
				return 1;
			}
			...
			if (!WaitForPlatformRestart()) {
				_logger.WriteError(
					$"{PackageName} was installed, but the environment did not "
					+ "become ready within the timeout after the platform's post-install restart. Check the "
					+ $"instance, then verify with 'clio call-service --service-path {PingServicePath} "
					+ "-m POST -b {} -e <environment>'.");
				return 1;
			}
			...
			if (!_outcomeVerifier.IsPackageOperational(
				...
				_logger.WriteError(diagnosis ??
					$"{PackageName} was installed, but its Ping route ({PingServicePath}) " +
