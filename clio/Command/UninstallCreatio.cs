...
[Verb("uninstall-creatio", Aliases = new[]
{
    "uc"
}, HelpText = "Uninstall local instance of creatio")]
...
public class UninstallCreatioCommand : Command<UninstallCreatioCommandOptions>
{
    ...
    public override int Execute(UninstallCreatioCommandOptions options)
    {
        ...
        Action act = options switch
                     {
                         var _ when options.PhysicalPath is not null => () =>
                             _creatioUninstaller.UninstallByPath(options.PhysicalPath),
                         var _ when options.EnvironmentName is not null => () =>
                             _creatioUninstaller.UninstallByEnvironmentName(options.EnvironmentName),
                         var _ => throw new ArgumentOutOfRangeException(nameof(options),
