...
public sealed class CreateRelatedPageAddonTool(
	...
	[Description("Configure the RelatedPage add-on for an object (entity schema): which Freedom UI pages open by default and for adding records, optionally per audience and per type. " +
		...
		"A base default page is MANDATORY: always include an is-default entry with no type-column-value (the page opened for a record and the fallback when a record's type has no dedicated set) — even when adding portal or typed pages; never bind only typed or only add pages without it. " +
	...
	public CreateRelatedPageAddonResponse CreateRelatedPageAddon(
		...
		if (string.IsNullOrWhiteSpace(args?.EntitySchemaName)) {
			return new CreateRelatedPageAddonResponse { Success = false, Error = "entity-schema-name is required" };
		}
		if (string.IsNullOrWhiteSpace(args.PackageName)) {
			return new CreateRelatedPageAddonResponse { Success = false, Error = "package-name is required" };
		}
...
public sealed record CreateRelatedPageAddonArgs(
	...
	[property: Description("Related-page entries. Fully replaces the object's current configuration. Mark exactly one entry per role/type bucket with is-default=true (the page opened on a record) and one with is-add=true (the page used to add a record); the same page may serve both. A base default entry is MANDATORY — always include one with is-default=true and no type-column-value (the main record page and the fallback when a record's type has no dedicated set); never bind only typed (or only add) pages without it.")]
	[property: Required]
