...
public sealed class PageUpdateTool(
	...
	[Description("Update a Freedom UI page schema body. environment-name preferred; uri/login/password fallback only. " +
		...
		"MOBILE: a viewConfigDiff insert/set must carry its component `type` INSIDE `values` — the differ builds the element from `values` alone, so a type on the operation object is discarded and the save persists an element that never renders; this is rejected. A `crt.Button` inserted into `Scaffold`/`actions` is warned about: it saves but never renders — place buttons in a a page container's `items` with a `layoutConfig`. See get-guidance `mobile-page-modification`. " +
