...
public sealed class PageUpdateTool(
	...
	[Description("Update Freedom UI page schema body. Prefer `environment-name`; keep direct connection args only for bootstrap or emergency fallback flows. " +
		...
		"INSERTED-FIELD CONTRACT: when the body contains `operation:\"insert\"` in viewConfigDiff for a standard field component (crt.Input, crt.NumberInput, crt.Checkbox, crt.ComboBox, crt.PhoneInput, crt.EmailInput, crt.DateTimePicker, crt.WebInput, crt.RichTextEditor,crt.ColorPicker, crt.ImageInput, crt.FileInput, crt.EncryptedInput, crt.Slider), the SAME call MUST also include both (a) a viewModelConfigDiff entry declaring the control's binding attribute with a `modelConfig.path` to the entity column, and (b) the label resource — either passed in the `resources` parameter, or set to `$Resources.Strings.<columnCode>` where `<columnCode>` is the LAST segment of the binding attribute's `modelConfig.path` (auto-provide is keyed by entity column code, not by view-model attribute name; `$Resources.Strings.PDS_<columnCode>` form is NOT auto-provided). Payloads that violate this contract are rejected at validation time; the diagnostic names the offending field, attribute, and section.")]
