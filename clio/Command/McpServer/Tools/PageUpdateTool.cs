...
public sealed class PageUpdateTool(
	...
	[Description("Update a Freedom UI page schema body. environment-name preferred; uri/login/password fallback only. " +
		...
		"MOBILE: a viewConfigDiff insert/set must carry its component `type` INSIDE `values` — the differ builds the element from `values` alone, so a type on the operation object is discarded and the save persists an element that never renders; this is rejected. A `merge` whose `values` authors child elements in a container slot (an array of objects carrying a `name`, e.g. on `actions` or `items`) is also rejected — the differ strips it when the slot is already populated, so the write succeeds and nothing is created; author each child with its own `insert`. A `crt.Button` inserted into `Scaffold`/`actions` is warned about: it saves but does not appear on the mobile designer canvas — place buttons in a page container's `items` with a `layoutConfig`. See get-guidance `mobile-page-modification`. " +
