...
public sealed class PageUpdateTool(
	...
	[Description("Update Freedom UI page schema body. Prefer `environment-name`; keep direct connection args only for bootstrap or emergency fallback flows. " +
		...
		"if the body changes SCHEMA_HANDLERS call get-guidance with name `page-schema-handlers` first — NOTE: restricting which records a lookup/ComboBox offers, even by a computed or relative date/time (e.g. \"birthday tomorrow\", \"created last quarter\"), is NOT handler business logic; it is an entity business rule (apply-static-filter), so use create-entity-business-rule, not crt.InitRequest; " +
