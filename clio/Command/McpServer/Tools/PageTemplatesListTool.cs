...
public sealed class PageTemplatesListTool(
	...
	[Description("List Freedom UI page templates advertised by the target Creatio environment. Call this before create-page to discover valid `template` values. The web catalog always includes `BaseDashboardTemplate` (title `Dashboard`, groupName `DashboardPage`) — use it as the `template` for a dashboard — and `CentralAreaDesktopTemplate` (title `Desktop`, groupName `Desktop`) — use it as the `template` for a desktop. Prefer `environment-name`; keep direct connection args only for bootstrap or emergency fallback flows.")]
