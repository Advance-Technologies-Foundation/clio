...
public sealed class PageTemplatesListTool(
	...
	[Description("List Freedom UI page templates advertised by the target Creatio environment. Call this before create-page to discover valid `template` values. The platform endpoint omits `BaseDashboardTemplate` (the dashboard-page parent), so clio always adds it to the web catalog (title `Dashboard`, groupName `DashboardPage`) — use it as the `template` for a dashboard. Prefer `environment-name`; keep direct connection args only for bootstrap or emergency fallback flows.")]
