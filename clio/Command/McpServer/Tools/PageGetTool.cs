...
public sealed class PageGetTool(
	...
	[Description(
		...
		"if the task involves designing or laying out the page UI/UX (choosing a component for a concept, placing or ordering fields, grid columns/colSpan, container nesting, grouping into tabs/groups, captions/tooltips/placeholders) — or reviewing a page's design, copy, or accessibility — call get-guidance with name `ui-guidelines` first (it routes to `ui-page-layout`, `ui-accessibility`, and `ui-review-checklists`); author and review from it, not from memory, and read the existing page style first so new fields match the existing label positions and spacing.")]
