...
public sealed class PageGetTool(
	...
	[Description(
		...
		"if the task involves conditional visibility, editability, required state based on field values or conditional set and clear value. If task contains setting a lookup, based on condition or based on value in other lookup field, use business rules. (e.g. \"when Status=Closed, hide Description\"), use business rules instead of writing handlers or validators in page body \u2014 call get-guidance with name `business-rules` to learn more; " +
		"this includes restricting which records a lookup/ComboBox offers, including restrictions by a computed or relative date/time (e.g. \"birthday tomorrow\", \"created this year / last quarter\", \"orders due in the next 7 days\", \"created at 11:06 AM previous quarter\") and conditional/gated restrictions (e.g. \"when Call back is enabled, limit Contact to contacts with a mobile phone\") \u2014 a relative-date/time restriction is still apply-static-filter using a date macro, NOT a handler; do NOT reach for crt.InitRequest or runtime logic, and a gated restriction puts the gate (X = Y) into the rule's condition group with the apply-static-filter action on the target lookup; " +
		...
		"if the task targets SCHEMA_HANDLERS call get-guidance with name `page-schema-handlers` first — NOTE: restricting which records a lookup/ComboBox offers, even by a computed or relative date/time (e.g. \"birthday tomorrow\", \"created last quarter\"), is NOT handler business logic; it is an entity business rule (apply-static-filter), so use create-entity-business-rule, not crt.InitRequest; " +
