		...
		return new ToolContractDefinition(
			...
			new ToolInputSchemaContract(
				...
				EnvironmentOrExplicitConnectionFields(
					...
					Field("force", BooleanType, "Skip the external-modification (checksum) conflict check and deliberately overwrite out-of-band changes. Set true ONLY after the user explicitly confirms overwriting changes made outside this session. Not needed for a `target-package-uid` / `target-schema-uid` redirect: there the conflict check does not run at all and the response carries a warning instead of a conflict."),
