...
internal static class ToolContractCatalog {
	...
	private static ToolContractDefinition BuildInstallProcessBuilder() {
		return new ToolContractDefinition(
			...
			"Installs (or updates) the bundled CrtProcessBuilder package into a registered Creatio environment, making ProcessDesignService reachable there. The package ships as source and the TARGET compiles it during installation, so the call takes roughly 15-75 seconds. A restart does happen (the platform recycles itself on .NET Framework, the installer issues it on .NET) but you never trigger it yourself - the tool waits for the instance to answer its health check before judging the result. It verifies the outcome rather than the install call: it asks ProcessDesignService which build is SERVING and fails if that is older than the version installed, which is the only way to see a failed upgrade (the platform records the new version on ACCEPTING the archive and keeps running its last successfully built assembly, so list-packages shows the new version either way). Packages older than 1.1.0.0 have no such operation and fall back to a weaker ListUserTasks proof-of-life. Re-running against an already-current environment does not reinstall, but it is still checked the same way; pass force=true when the package is present and the service is silent.",
			new ToolInputSchemaContract(
				[EnvironmentNameFieldName],
				[
					Field(EnvironmentNameFieldName, StringType, RegisteredEnvironmentNameDescription)
