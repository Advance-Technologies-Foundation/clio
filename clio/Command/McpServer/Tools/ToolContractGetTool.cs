			...
			new ToolInputSchemaContract(
				...
				Validators: [
					...
					new ToolContractValidator("apply-filter-shape", "invalid-apply-filter-action", "rule.actions[*]",
						Context: "When rule.actions[*].type is apply-filter, provide target, targetFilterPath, source, optional sourceFilterPath, clearValue, and populateValue. Target and source must be direct lookup attributes on the root entity. targetFilterPath and sourceFilterPath resolve inside the referenced lookup schemas. apply-filter rules support exactly one action and may use an empty condition group."),
					new ToolContractValidator("apply-filter-lookup", "unsupported-apply-filter-lookup", "rule.actions[*].target",
						Context: "apply-filter only supports lookup targets and lookup sources. The final targetFilterPath and source/sourceFilterPath endpoints must resolve to the same data value type. If sourceFilterPath is provided, populateValue must be false.")
				]),
