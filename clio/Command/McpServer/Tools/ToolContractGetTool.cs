	...
	private static ToolContractDefinition BuildApplicationSectionCreate() {
		return new ToolContractDefinition(
			...
			EnvelopeOutput(
				...
				Field(ErrorFieldName, StringType, FailureMessageDescription),
				Field("error-class", StringType, "Failure classification, present on classified errors only: 'transport' (request never reached Creatio — retry is safe), 'creatio-timeout' (no response within the budget — side effects unknown, verify with list-app-sections before retrying), 'contention' (insert aborted without detail because sections were created in the same app at once — no section created; create sections one at a time, clio serializes and auto-retries once), 'server-error' (Creatio rejected the operation with a real reason — fix inputs or server state first)."),
