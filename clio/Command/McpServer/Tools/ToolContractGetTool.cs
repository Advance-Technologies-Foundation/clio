...
internal static class ToolContractCatalog {
		...
		return new ToolContractDefinition(
			...
			new ToolInputSchemaContract(
				...
				EnvironmentPackageSchemaFields(
					...
					Field("is-db-view", BooleanType, "Map to a separately provisioned SQL view without generating a table. Omit to preserve inherited metadata."),
	...
	private static ToolContractDefinition BuildRequestInfo() {
		return new ToolContractDefinition(
			...
			EnvelopeOutput(
				...
				Field("items", ArrayType, "Flat list-mode request summaries, each with requestType, an optional description, and optional deprecated/deprecationReason — filter on those at browse time so a deprecated request is ruled out without opening its detail response."),
				...
				Field("deprecated", BooleanType, "True in detail mode when the request TYPE itself is deprecated at the class level (as opposed to a single deprecated baseParameter) — honor that guidance and do not author new schemas against it; schemas that already reference it keep working. Omitted on non-deprecated requests."),
				Field("deprecationReason", StringType, "Why the request type is deprecated and, when the producer named one, the replacement request to reach for instead — honor that guidance. Present only alongside deprecated:true."),
	...
	private static ToolContractDefinition BuildPageValidate() {
		return new ToolContractDefinition(
			...
			"For mobile pages (plain JSON body starting with `{`): validates that disallowed constructs (validators, handlers, custom converters sections) are absent, and that an authored crt.IndicatorWidget can show a value (executable data providing, config.layout, config.text).",
			new ToolInputSchemaContract(
				[],
				[
