		...
		return new ToolContractDefinition(
			...
			new ToolInputSchemaContract(
				...
				EnvironmentOrExplicitConnectionFields(
					...
					Field("force", BooleanType, "Skip the external-modification (checksum) conflict check and deliberately overwrite out-of-band changes. Set true ONLY after the user explicitly confirms overwriting changes made outside this session. Note that the two schema-identity checks run BEFORE the checksum comparison and are armed from disk on a pinned save too, so a `target-package-uid` / `target-schema-uid` redirect can be refused even when the pin matches the server."),
