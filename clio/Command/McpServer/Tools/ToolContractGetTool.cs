	...
	private static ToolContractDefinition BuildEntityBusinessRuleRead() {
		return new ToolContractDefinition(
			...
			BusinessRulesReadOutput(
				" Autogenerated apply-filter child rules are folded into the parent action's clearValue/populateValue flags and are never listed as separate rules. apply-static-filter rules return the persisted ESQ envelope in rule.actions[*].esqFilter (the friendly filter is NOT reconstructed); supply a friendly filter on update to change it. Formula set-values return the persisted expression text best-effort."),
			CommonErrorContract,
			[],
			[],
			[
