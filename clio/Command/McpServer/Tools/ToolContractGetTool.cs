...
internal static class ToolContractCatalog {
	...
	private static ToolContractDefinition BuildInstallProcessBuilder() {
		return new ToolContractDefinition(
			...
			"Installs (or updates) the bundled CrtProcessBuilder package into a registered Creatio environment, making ProcessDesignService reachable there. The package ships as source and the TARGET compiles it during installation, so the call takes roughly 15-75 seconds. A restart does happen (the platform recycles itself on .NET Framework, the installer issues it on .NET) but you never trigger it yourself - the tool waits for the instance to answer its health check before judging the result. It checks the outcome rather than the install call: it queries ListUserTasks afterwards and fails when ProcessDesignService does not answer, so \"installed but never compiled\" is reported instead of looking like success. Do not over-read a success: that is conclusive on a FIRST install (nothing served before, so only a fresh build can answer), but on an UPGRADE a failed compile leaves the previously built assembly serving and answering, so success means the service works, not that your new version is the one running. It ALWAYS installs - there is no skip, and re-running is safe (one configuration build on the target). Take the gated tool's refusal as the signal to call this rather than comparing versions yourself: the gate checks the version the environment recorded against the version clio bundles.",
			new ToolInputSchemaContract(
				[EnvironmentNameFieldName],
				[
					Field(EnvironmentNameFieldName, StringType, RegisteredEnvironmentNameDescription)
