	...
	private static ToolContractDefinition BuildPageUpdate() {
		return new ToolContractDefinition(
			...
			EnvelopeOutput(
				...
				Field("appendProjection", ObjectType, "What the append merge did — or, on a dry run, WOULD do — to the page's viewConfigDiff. Present only for `mode: append`; absent for `replace`, which writes verbatim. Carries `currentOperationCount`, `incomingOperationCount`, `projectedOperationCount` (compare this against the count you expect), `addedOperationCount`, `replacedOperations`/`replacedOperationCount` (existing entries your fragment replaces in place — NOT a loss, the operation survives with your values), and `droppedOperations`/`droppedOperationCount` (existing entries the merge does not carry over: the further-duplicate exception described in `mode`). A non-zero drop count also raises a `warnings` entry naming each one, so you never have to derive a loss from the counts. The named lists are capped in length; the counts are exact. Speaks for `viewConfigDiff` ONLY: the sibling `*_DIFF` arrays append unconditionally and cannot lose an entry, but handlers are NOT covered — they dedupe by `request` and the merge drops every current handler whose `request` your fragment carries, so a current body holding one `request` twice keeps neither. Inspect the handlers section yourself when that shape is possible."),
				Field("resourcesRegistered", NumberType, "Number of registered resources."),
				Field("warnings", ArrayType, "Advisory non-fatal warnings; omitted when there are none. The save already succeeded — never retry on a warning. Covers an operation the differ will silently drop because another operation for the same component name cancels it (see `mode`), an `insert` this body replaced with a `merge`/`move`/`remove`, page-body lint findings, an existing viewConfigDiff operation an append drops rather than re-applies (see `appendProjection.droppedOperations`), and the best-effort Designer Presence push."),
				Field(ErrorFieldName, StringType, FailureMessageDescription)
			),
			CommonErrorContract,
			[
