...
public sealed class PageValidateTool(
	...
	[Description("Validates a Freedom UI page body client-side without saving to Creatio (markers, JS syntax, field/column bindings, handler/converter/validator structure for web; disallowed constructs, diff-apply, and component-type placement for mobile — `type` must sit inside `values`). " +
