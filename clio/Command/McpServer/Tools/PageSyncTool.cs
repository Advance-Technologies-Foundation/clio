...
public sealed class PageSyncTool(
	...
	[Description("Updates multiple Freedom UI page schemas in a single call. " +
	             ...
	             "On MOBILE bodies it additionally rejects a viewConfigDiff insert/set whose component `type` sits on the operation object instead of inside `values` (the differ discards it and the element never renders), and a `merge` whose `values` authors child elements in a container slot (the differ strips it and nothing is created; use one `insert` per child), and warns when a `crt.Button` is inserted into `Scaffold`/`actions` (it saves but does not appear on the mobile designer canvas — use a page container's `items` with a `layoutConfig`); see get-guidance `mobile-page-modification`. " +
