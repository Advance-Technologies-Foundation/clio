...
public sealed class WithdrawTelemetryConsentTool
{
	...
	[Description("Withdraws product telemetry consent: sets the locally stored decision to denied and deletes any not-yet-uploaded local telemetry events, so nothing more is collected or uploaded. Call this when the developer asks to stop, turn off, opt out of, or withdraw product telemetry. Forward-looking: it does not delete events already uploaded to Creatio (those expire on the server-side retention timer). Idempotent and safe to call from any state; after it succeeds get-telemetry-consent returns denied and the workflow continues without telemetry.")]
