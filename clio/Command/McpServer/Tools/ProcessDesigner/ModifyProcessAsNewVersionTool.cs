...
public class ModifyProcessAsNewVersionTool(
	...
	[McpServerTool(Name = ModifyProcessAsNewVersionToolName, ReadOnly = false, Destructive = false,
	 ...
	 Description("Edit an existing Creatio business process and save the result as a NEW VERSION of it, leaving "
		 ...
		 + "with install-process-builder. An email.template or email.templateEntity in the operations needs 1.6.2.1 "
		 + "(the Send email template message mode) — this route runs NO read-back check, so on a 1.6.1.x environment "
		 + "the template member is silently discarded and the version is saved with an element in no message mode. After a successful save the version normally stays INTERPRETED and "
