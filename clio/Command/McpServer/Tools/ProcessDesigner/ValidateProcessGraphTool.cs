...
public sealed class ValidateProcessGraphTool {
	...
	[Description("Checks a planned Creatio business-process graph against the BPMN connection rules before you build it. "
		+ "Nodes are given by data-id (e.g. startEvent/readDataUserTask/exclusiveGateway/endEvent; edges by flow-kind sequence|conditional|default - an omitted flow-kind is a plain sequence flow, an UNKNOWN one is refused rather than treated as plain; and results[], the activity-result CAPTIONS deciding a conditional branch when its predicate is a selection rather than text - pass it and R13 stops asking for a condition that branch neither needs nor can use) and are checked against rules R1-R20 (R18: a conditional flow may have at most ONE outgoing sibling that carries no condition - the platform drops one of them and runs the other beside the branch the condition chose). The graph is validated in-memory, but the tool requires the 'CrtProcessBuilder' package to be installed on the target environment (install it with install-process-builder) (named by environment-name). Returns structured findings (error/warning + ruleId). nodes is REQUIRED - a call supplying none is REFUSED, not validated as an empty graph, and this tool never reads a process from the environment (use describe-business-process for that). Call this BEFORE driving the designer. IMPORTANT: a passing graph is NOT necessarily buildable — the rules cover the full BPMN catalog (gateways, conditional/default flows, timers, sub-processes), while create-business-process / modify-business-process build only startEvent/signalStart/endEvent/userTask (incl. its readData/changeData/addData/deleteData/performTask aliases)/sendEmail/approval/openEditPage/preconfiguredPage/changeAccessRights/subProcess elements plus exclusiveGateway, parallelGateway and formulaTask, and all three flow kinds declaratively (flows[].kind with flows[].condition). Still NOT buildable: inclusiveGateway, eventBasedGateway, timer/message starts, intermediate events, scriptTask, webService and the event sub-process - and each such node now carries an UNBUILDABLE WARNING naming it, so the finding, not this list, is what tells you a node must be replaced before building. create-business-process runs these same rules before it posts and writes the ones the server does not check (R7/R9 with no default, R8, R13 off an event, R17) as advisory Pre-flight warnings; modify runs none of them. The Sub-process element IS buildable now (the callActivity data-id here, the subProcess token there); what is not is the EVENT and the EXPANDED sub-process, which share the platform class but call no other process. An element that runs the called process ONCE PER ITEM of a collection IS buildable too (subProcess.multiInstanceOptions), and this validator neither checks nor needs it: the mode is element configuration, not graph shape, so it changes no node type and no edge. Note also that this tool cannot check R16 - whether the called process begins with a Simple start event - because a planned graph carries no reference to it; the build path refuses that one, and execute-esq over VwProcessLib answers it up front through HasStartEvent. The activity-result dialect IS buildable now (flows[].results / setFlowResults), and a formula on such a connector is REFUSED by the build, so do not plan one; check the buildable slice in get-guidance name=process-modeling before promising a build; get-guidance name=process-formulas for an `expression` mapping source or a conditional-flow condition.")]
