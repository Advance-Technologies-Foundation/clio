...
public class ModifyBusinessProcessTool(
	...
	[McpServerTool(Name = ModifyBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("Edit an EXISTING business process on a Creatio environment by applying an ordered JSON array of "
		 ...
		 + "means LEAVE THEM ALONE, never the page has none — with ONE exception: changing page TO a Freedom UI "
		 + "page REQUIRES buttons in the same call (the stored buttons name the previous page's buttons, so "
		 + "the operation is refused rather than carried across; re-read get-process-page-facts for the new page "
		 + "first), and changing page to a Classic UI page is refused outright. dataSources have no removal path "
		 + "through this contract, so a page change carries the previous page's data-source parameters forward "
		 + "as leftovers — NOT inert ones: a data-source parameter naming a source the CURRENT page does not "
		 + "declare stops the step from ever completing (the completing button abandons the completion, the page "
		 + "stays open with no error and the instance never leaves 'Running'), which is why a dataSources entry "
		 + "the new page does not have is refused here rather than stored. Any setElement touching such an element also "
