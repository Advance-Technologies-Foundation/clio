...
public class ModifyBusinessProcessTool(
	...
	[McpServerTool(Name = ModifyBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("Edit an EXISTING business process on a Creatio environment by applying an ordered JSON array of operations. "
		 + "Every overwrite is irreversible; read describe's isActiveVersion first. TRUE: it replaces the running "
		 + "graph, so get an explicit yes (the edit request is not one) and offer modify-business-process-as-new-version. "
		 + "FALSE: do not modify it; re-describe by activeVersionSchemaUId, or report the standing and ask "
		 + "(process-modeling owns both branches). "
		 ...
		 + "userTaskName?, useBackgroundMode? (element-level; process-element-catalog owns where it acts), "
		...
		+ "changeData {source?, values?} reconfigures a changeData element: omit source to keep the current "
		 ...
		 + "first. For a setFlowCondition operation or an 'expression' mapping source read get-guidance "
			 ...
			 + "Requires the ProcessDesignService (CrtProcessBuilder) package; install with install-process-builder. Unless the edit adds a scriptTask, replaces its body or changes a using or the methods, the process stays INTERPRETED and the result carries the compile-not-required note: do NOT run compile-creatio, and do NOT infer a compile need from a raw `VwSysProcess` read. Such an edit warns compile-REQUIRED instead: then, after asking the user, run compile-creatio with process-name. Read get-guidance name=process-script-task first.")]
