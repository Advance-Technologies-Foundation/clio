...
public class ModifyBusinessProcessTool(
	...
	[McpServerTool(Name = ModifyBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("Edit an EXISTING business process on a Creatio environment by applying an ordered JSON array of "
		 ...
		 + "{useBackgroundMode?} — changes element-level fields IN PLACE on ANY element kind, preserving the element "
		 + "and its flows; only the fields you pass change), "
		 ...
		 + "column's own reference entity, which is the one piece of trivia you cannot guess. Refused, with the "
		 + "reason, when the user task's runtime never writes connections, when they would not take effect on this "
		 + "element (usually CreateActivity left at its false default — the refusal quotes the exact operation "
		 + "to prepend), when the column is never a connection at all (ActivityCategory, ShowInScheduler are "
		 + "refused with their own reason pointing at addMapping), when the column is not one this element can "
		 + "carry, or when an expression is not a platform macro at all (it must look like [#...#]; a bare value is "
		 + "refused — use recordId) or is a macro family that cannot hold a record reference. A column that "
		 + "exists but carries no connection-registry row IS written "
		 + "at run time yet stays invisible in the designer and to the record page's connections detail, so it "
		 + "SUCCEEDS with a caveat in the response 'warnings'), "
		 ...
		 + "carry 'warnings' — outcomes that applied but are not what you would assume; read them. "
