...
public class ModifyBusinessProcessTool(
	...
	[McpServerTool(Name = ModifyBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("Edit an EXISTING business process on a Creatio environment by applying an ordered JSON array of "
		 ...
		 + "attributed to the one operation that carried it (on CrtProcessBuilder this clio requires 1.4.0.41, the "
		 + "version where clio stopped validating formulas a second time itself; below it a refused formula still "
		 ...
		 + "The refusal names the parameter, the character index of the fault, and the expression as the "
		 ...
		 + "do not conclude the wrong formula was validated. For an unresolvable parameter reference it names "
		 + "the reference and the remedy. Must be "
		 + "one line; "
