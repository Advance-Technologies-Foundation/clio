...
public class ModifyBusinessProcessTool(
	...
	[McpServerTool(Name = ModifyBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("Edit an EXISTING business process on a Creatio environment by applying an ordered JSON array of "
		 ...
		 + "{useBackgroundMode?, readData?} — changes element-level fields IN PLACE, preserving the element and its "
		 ...
		 + "selection/order, pass columns:[] to reset to ALL columns; retargeting source to a different object "
		 + "clears the columns, sort AND record filter bound to the old entity — re-supply them (and setFilter) in "
		 + "the same batch), "
