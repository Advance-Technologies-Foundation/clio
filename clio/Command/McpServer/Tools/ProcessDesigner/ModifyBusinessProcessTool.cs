...
public class ModifyBusinessProcessTool(
	...
	[McpServerTool(Name = ModifyBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("Edit an EXISTING business process on a Creatio environment by applying an ordered JSON array of operations. "
		 ...
		 + "'elementParameter', and one source of {sourceElement, sourceElementParameter (+ sourceColumn)} "
		 ...
		 + "failedOperationIndex while one caused by a single operation reports its zero-based index (on CrtProcessBuilder this clio requires 1.6.6.25, for "
		 + "sourceColumn (1.6.6.14 before it, for subProcess.multiInstanceOptions {enabled, executionMode, "
		 + "ignoreErrors}); it subsumes 1.6.0.1's page-change reconciliation of a "
