...
public class ModifyBusinessProcessTool(
	...
	[McpServerTool(Name = ModifyBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("Edit an EXISTING business process on a Creatio environment by applying an ordered JSON array of "
		 ...
		 + "{useBackgroundMode?, readData?, changeData?, email?, openEditPage?} — changes element-level fields IN PLACE, preserving the element and its "
		 ...
		 + "openEditPage "
		 + "(Open edit page elements only, same block as create-business-process) applies PARTIALLY — only the "
		 + "fields you pass change, and defaultValues:[] empties the pre-filled set. An object-bound field "
		 + "(defaultValues, recordId, resultsByColumn) needs NO page here: it resolves against the object the "
		 + "element already stores. TWO refusals guard a destructive change: changing editMode — or changing page "
		 + "to one of ANOTHER object — requires the new configuration's payload in the same update (defaultValues "
		 + "for add, recordId for edit), because the switch clears the branch being left, exactly as the designer "
		 + "does; and an object-changing retarget is refused while object-bound configuration is stored — replace a "
		 + "stored resultsByColumn column in the same update, and clear stored completion conditions with "
		 + "clearFilter BEFORE the retarget, since setElement carries no filter field), "
