...
public class ModifyBusinessProcessTool(
	...
	[McpServerTool(Name = ModifyBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("Edit an EXISTING business process on a Creatio environment by applying an ordered JSON array of "
		 ...
		 + "e.g. Text/Integer/Guid, direction?, caption?, or referenceSchema for a Lookup to an object e.g. City, or "
		 + "typeFromElement + typeFromElementParameter to copy an element parameter's exact type), "
		 + "addMapping (with a 'mapping': target {elementName, elementParameter} or {targetProcessParameter}, and one "
		 + "source of {sourceElement, sourceElementParameter} | processParameter | "
		 + "value | expression; parameter-to-parameter mappings require compatible types). Operations apply in "
		 + "order; any failure aborts the edit (nothing is saved). "
