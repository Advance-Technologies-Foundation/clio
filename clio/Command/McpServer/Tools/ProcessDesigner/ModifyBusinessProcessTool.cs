...
public class ModifyBusinessProcessTool(
	...
	[McpServerTool(Name = ModifyBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("Edit an EXISTING business process on a Creatio environment by applying an ordered JSON array of "
		 ...
		 + "direction? (one of In / Out / Variable / Internal — those EXACT tokens; 'Input'/'Output' are rejected), "
		 + "caption?, description?, optional value (a literal constant, not a formula), or "
		 ...
		 + "{useBackgroundMode?, email?} — changes element-level fields IN PLACE, preserving the element and its "
		 + "flows; only the fields you pass change. useBackgroundMode applies to ANY element kind. email "
		 ...
		 + "column's own reference entity, which is the one piece of trivia you cannot guess. Supported on SIX user "
		 ...
		 + "on the environment; a user task whose runtime never writes connections (the CallUserTask, EmailUserTask, "
		 + "SendEmailUserTask and ReadDataUserTask schemas — NOT the sendEmail element type above, which maps to the "
		 + "supported EmailTemplateUserTask); "
		 ...
		 + "at all it says so instead, because addMapping could not create one; on a Send email element switching it "
		 + "to manual send also clears this gate, because the manual sender creates the activity unconditionally); "
		 + "a column that is never a connection "
