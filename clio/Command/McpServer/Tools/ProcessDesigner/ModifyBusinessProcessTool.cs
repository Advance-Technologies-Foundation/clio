...
public class ModifyBusinessProcessTool(
	...
	[McpServerTool(Name = ModifyBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("Edit an EXISTING business process on a Creatio environment by applying an ordered JSON array of "
		 ...
		 + "one of Text/Long text/Integer/Float/Money/Boolean/Date/Date-time/Time/Guid/Collection (other types are rejected), "
		 + "direction?, caption?, description?, optional value (a literal constant, not a formula; refused on a Collection), or "
		 + "referenceSchema for a Lookup to an object e.g. City (refused on a Collection), or typeFromElement + "
		 + "typeFromElementParameter to copy an element parameter's exact type — mirroring a COLLECTION output (e.g. a "
		 ...
		 + "addMapping (with a 'mapping': target {elementName, "
		 ...
		 + "actually reads (THIS clio bundles CrtProcessBuilder 1.4.0.42 and refuses any "
		 ...
		 + "{source?, mode?:first|collection|count|aggregation, columns?, sort?:{column, direction?:asc|desc}, "
		 + "numberOfRecords? (collection only), aggregation?:{function:sum|avg|min|max, column} (aggregation only, "
		 + "required there)} reconfigures a readData element's data configuration: omit source to keep the current "
		 + "source object, omit mode to KEEP the current mode, omit columns/sort to keep the current selection/order, "
		 + "pass columns:[] to reset to ALL columns (first only — collection REQUIRES a selection). Changing the mode is "
		 + "a real conversion: the previous mode's parameters clear, the result flag moves to the new mode's output "
		 + "(first → ResultEntity; collection → ResultEntityCollection + ResultCompositeObjectList; count → ResultCount; "
		 + "aggregation → ResultIntegerFunction | ResultFloatFunction | ResultDateTimeFunction by the column's type), "
		 + "the selection/sort survive between first and collection but clear on entering count/aggregation, and leaving "
		 + "collection empties ResultCompositeObjectList's item properties — a collection process parameter mirrored "
		 + "from the element is NOT re-mirrored, re-issue its setParameter afterwards; sort and non-empty columns are "
		 + "refused for count/aggregation (the runtime ignores both). Retargeting source to a different object is refused while "
