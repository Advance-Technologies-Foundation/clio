...
public class ModifyBusinessProcessTool(
	...
	[McpServerTool(Name = ModifyBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("BEFORE CALLING with an accessRights block: that block changes who can read, edit or delete LIVE records. Show the user the target object, the element record filter that decides WHICH records are affected, and every grantee with its operations and level - calling out level:delegate as onward re-sharing, level:restrict as the platform Deny level, which is UNVERIFIED: the same value is captioned \"NotSet\" on the record-rights detail and appears in no captured designer specimen, and a restrict entry lands in the add (GRANT) collection, restrict writes the platform Deny level and is DESTRUCTIVE, not inert: UseDenyRecordRights gates only record positioning, never whether a right row is written. Against a grantee who already holds Allow it DOWNGRADES that row to Deny, and a fresh insert writes one row per operation - the one you named at your level and the OTHER TWO at Deny - so operations:['read'] denies edit and delete as well. Prefer a remove entry to take access away, a remove entry as a revoke, and a supplied add/remove as a REPLACEMENT that drops every entry it does not restate - and get an explicit yes. The element has no output parameters, so nothing at run time will report what it did. "
		 ...
		 + "{useBackgroundMode?, readData?, changeData?, accessRights?, email?, performer?} — changes element-level fields IN PLACE, preserving the element and its "
		 ...
		 + "accessRights {object?, considerTimeInFilter?, add?, remove?} reconfigures a Change access rights element in place (entry shape as in create-business-process): omitted fields keep their values; a supplied add/remove REPLACES that whole collection — its previous grantee parameters are removed, replace is the only way to remove an entry — and [] clears it — clearing ONE is safe only while the other still holds an entry, since an element with both empty runs and changes nothing while reporting success. Because a supplied collection replaces rather than appends, a collection you build from a read-back can only be as complete as that read-back was: describe reports a stored-but-undecodable collection as an EMPTY array, so an empty add/remove there means the stored entries are UNKNOWN, not absent. Supplying a collection for such an element DROPS whatever it did not restate, including a remove entry, which BROADENS access. If a read-back reported an empty collection and you need to change it, confirm the stored entries in the process designer first. NOTE the post-edit accessRights check clio runs (dropped block, missing record filter) is triggered by an accessRights block in the operations, so an array whose only operation is setFilter or clearFilter against a Change access rights element is NOT checked - verify that one yourself with describe-business-process; omit object to keep the current target (required when none is stored yet, and a present-but-blank object is refused). On ANY object change (first configuration included) the stored record filter clears unless it already targets the incoming object — re-issue setFilter in the same batch), "
		 + "email "
