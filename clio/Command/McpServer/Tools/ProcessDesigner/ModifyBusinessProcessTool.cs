...
public class ModifyBusinessProcessTool(
	...
	[McpServerTool(Name = ModifyBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("Edit an EXISTING business process on a Creatio environment by applying an ordered JSON array of "
		 ...
		 + "{useBackgroundMode?, readData?, email?, openEditPage?} — changes element-level fields IN PLACE, preserving the element and its "
		 ...
		 + "openEditPage (openEditPage elements only, same block as create-business-process) reconfigures the "
		 + "element IN PLACE: every omitted field keeps its stored value and a supplied defaultValues array REPLACES "
		 + "the whole set - pass [] to REMOVE every pre-filled value, the one way to empty that block (omitting it "
		 + "keeps what is stored, and the runtime applies whatever stays there). TWO changes are DESTRUCTIVE and are guarded, mirroring the designer: retargeting page "
		 + "(which the designer itself warns loses every field value and filter setting) and changing editMode (which "
		 + "the designer implements by clearing the branch being left). Both REQUIRE the new mode-specific value in "
		 + "the same update - defaultValues for add, recordId for edit - because the stored one belongs to the "
		 + "configuration being replaced; a refused update leaves the element untouched. After a completed switch the "
		 + "old branch is CLEARED, which matters beyond tidiness: the runtime applies stored pre-filled values in "
		 + "either mode, so a leftover set would be live configuration nobody asked for. The completion mode is "
		 + "validated against the element's STORED filter here (setElement carries no filter field), so set the "
		 + "conditions with setFilter in the same batch or earlier - and going BACK to onSave needs clearFilter "
		 + "first in the same ordered batch, because a stored filter on an onSave element is refused (it would be "
		 + "kept and never consulted). A performer block REPLACES the whole "
		 + "assignment (type, contact/role and showPage together), so pass every part you want kept; omitting the "
		 + "block leaves the stored assignment - and the stored showPage - alone. A logActivity block writes only "
		 + "the parts you pass, and each interval you do pass writes BOTH its number and its unit - never one "
		 + "alone), "
