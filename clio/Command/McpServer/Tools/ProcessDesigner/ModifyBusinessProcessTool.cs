...
public class ModifyBusinessProcessTool(
	...
	[McpServerTool(Name = ModifyBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("Edit an EXISTING business process on a Creatio environment by applying an ordered JSON array of "
		 ...
		 + "macro family the package does not recognise is not refused by THAT check, but expect an ERROR rather "
		 + "than a warning: the platform's own pre-save validation runs after it and refuses the save with "
		 + "'Process validation failed' unless a converter resolves that macro where you used it — measured over a "
		 + "mapping onto a plain process parameter, an invented family and the real [#ColumnValue…#] and "
		 + "[#SamplingColumnValue…#] were all three refused. Must be "
		 + "one line; "
