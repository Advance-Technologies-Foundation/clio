...
public class ModifyBusinessProcessTool(
	...
	[McpServerTool(Name = ModifyBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("Edit an EXISTING business process on a Creatio environment by applying an ordered JSON array of "
		 ...
		 + "to drop a condition, remove the flow and add a plain one. Refused on a default branch), "
		 ...
		 + "— StartOfMonth, StartOfWeek, StartOfYear, Day, Month, Time, DayOfWeek, DayInRange), unqualified Math "
		 ...
		 + "[#BooleanValue.True#]. From CrtProcessBuilder 1.4.0.1 an expression is VALIDATED before it is stored "
		 ...
		 + "Numeric targets are checked as decimal, so any numeric formula fits any numeric parameter, and a "
		 ...
		 + "actually reads (the route ships from CrtProcessBuilder 1.4.0.1; THIS clio additionally refuses any "
