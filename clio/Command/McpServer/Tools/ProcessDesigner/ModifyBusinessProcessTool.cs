...
public class ModifyBusinessProcessTool(
	...
	[McpServerTool(Name = ModifyBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("Edit an EXISTING business process on a Creatio environment by applying an ordered JSON array of "
		 ...
		 + "{useBackgroundMode?, readData?, changeData?} — changes element-level fields IN PLACE, preserving the "
		 + "element and its flows; only the fields you pass change. useBackgroundMode applies to ANY element kind. "
		 + "readData {source?, mode?:first, columns?, sort?:{column, direction?:asc|desc}} reconfigures a readData "
		 + "element's data configuration: omit source to keep the current source object, omit columns/sort to keep "
		 + "the current selection/order, pass columns:[] to reset to ALL columns. changeData {source?, values?} "
		 + "reconfigures a changeData element: omit source to keep the current target, a supplied values array "
		 + "REPLACES the whole assignment set. For BOTH: retargeting source to a different object clears the "
		 + "dependent config (columns/sort or values) AND the record filter bound to the old entity — re-supply "
		 + "them (and setFilter) in the same batch). "
