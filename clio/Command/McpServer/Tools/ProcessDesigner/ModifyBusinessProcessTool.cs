...
public class ModifyBusinessProcessTool(
	...
	[McpServerTool(Name = ModifyBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("Edit an EXISTING business process on a Creatio environment by applying an ordered JSON array of "
		 ...
		 + "attributed to the one operation that carried it (on CrtProcessBuilder this clio requires 1.4.0.44, the "
		 + "version where the PACKAGE stopped validating formulas a second time itself; below it a refused formula still "
		 ...
		 + "A refusal always names the parametrized element or the parameter, and for the whole 'Formula value "
		 + "error:' family it also carries the character index of the fault and the expression as the "
		 + "platform's own converter left it - NOT as you wrote it: a parameter reference is shown by the "
		 + "parameter NAME, a fractional literal gains an 'm' and a division gains a ((decimal)…) wrapper, so "
		 + "do not conclude the wrong formula was validated. Two classes carry NEITHER an index nor the "
		 + "expression, so do not wait for one: an unresolvable [#…#] parameter reference, which instead names "
		 + "the reference and the remedy, and a newline, which is refused with 'Expression contains invalid "
