...
public class ModifyBusinessProcessTool(
	...
	[McpServerTool(Name = ModifyBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("Edit an EXISTING business process on a Creatio environment by applying an ordered JSON array of "
		 ...
		 + "element IN PLACE: every omitted field keeps its stored value. A supplied defaultValues array REPLACES the "
		 + "whole set, and [] REMOVES every pre-filled value — the one way to empty that block. TWO changes are "
		 + "DESTRUCTIVE and guarded, mirroring the designer: retargeting page and changing editMode. Both REQUIRE the "
		 + "new mode-specific value in the same update (defaultValues for add, recordId for edit); a refused update "
		 + "leaves the element untouched, and a completed switch CLEARS the branch being left. A page change that "
		 + "also changes the OBJECT is refused while object-bound configuration is stored (the completion conditions "
		 + "and the resultsByColumn column), naming the order to use. The completion mode is validated against the "
		 + "element's STORED filter here, since setElement carries no filter field: set the conditions with setFilter "
		 + "in the same batch or earlier, and going BACK to onSave needs clearFilter first in the same ordered batch. "
		 + "A performer block REPLACES the whole assignment (type, contact/role and showPage together), so pass every "
		 + "part you want kept; omitting the block leaves the stored assignment alone. A logActivity block writes "
		 + "only the parts you pass, and each interval you pass writes BOTH its number and its unit. See "
		 + "get-guidance name=process-modeling for what each of these rules is protecting), "
