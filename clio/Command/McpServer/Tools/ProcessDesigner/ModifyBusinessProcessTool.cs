...
public class ModifyBusinessProcessTool(
	...
	[McpServerTool(Name = ModifyBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("Edit an EXISTING business process on a Creatio environment by applying an ordered JSON array of "
		 ...
		 + "failedOperationIndex while one caused by a single operation reports its zero-based index (on CrtProcessBuilder this clio requires 1.6.0.2, the first "
		 ...
		 + "{useBackgroundMode?, readData?, changeData?, email?, approval?, performer?, openEditPage?, preconfiguredPage?} — changes element-level fields IN PLACE, preserving the element and its "
