...
public class ModifyBusinessProcessTool(
	...
	[McpServerTool(Name = ModifyBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("Edit an EXISTING business process on a Creatio environment by applying an ordered JSON array of operations. "
		 ...
		 + "condition written onto one is stored and never evaluated. From CrtProcessBuilder 1.6.2.11 it is "
		 + "ALSO refused when the flow's SOURCE enumerates activity results and nothing is selected yet - the "
		 + "case that used to be accepted, and the one that produced branches nobody could maintain. That "
		 + "refusal names the results the element offers and points at setFlowResults), "
		 + "setFlowResults (from 1.6.2.11; with 'source' and 'target' naming an EXISTING flow, plus a "
		 ...
		 + "MUTUALLY EXCLUSIVE and a write of either clears the other. Refused on a default branch, on a "
		 ...
		 + "the build path with flows[].results. get-guidance name=process-branch-conditions owns the rule "
		 + "and says what to tell the user before building one), "
