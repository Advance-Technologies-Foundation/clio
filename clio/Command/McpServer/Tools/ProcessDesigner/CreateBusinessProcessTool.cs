...
public class CreateBusinessProcessTool(
	...
	[McpServerTool(Name = CreateBusinessProcessToolName, ReadOnly = false, Destructive = false, Idempotent = false,
	 ...
	 Description("Build a business process on a Creatio environment from a declarative JSON descriptor. The "
		 ...
		 + "unless the element already carries one (so {} is how you switch a notification back on using the "
		 + "template it has): the runtime does not check for a template before sending and ignores email errors by "
		 + "default, so a flag with no template yields an element that reports the notification as configured and "
		 + "silently never sends. THREE parameters are DERIVED from 'object' server-side and never accepted as input — the visa "
		 ...
		 + "ConstValue the runtime actually reads (the route needs CrtProcessBuilder 1.4.3.0, the version this clio bundles; THIS clio "
