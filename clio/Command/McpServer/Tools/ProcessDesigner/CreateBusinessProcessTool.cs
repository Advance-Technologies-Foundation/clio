...
public class CreateBusinessProcessTool(
	...
	[McpServerTool(Name = CreateBusinessProcessToolName, ReadOnly = false, Destructive = false, Idempotent = false,
	 ...
	 Description("Build a business process on a Creatio environment from a declarative JSON descriptor. The "
		 ...
		 + "validation failed' and nothing is created. On CrtProcessBuilder this clio requires 1.4.0.58, which is "
		 ...
		 + "with the package's own wording. The floor is 1.4.0.58 rather than .44 for a different KIND of "
		 + "reason: below .58 the package refuses flows[].kind, flows[].condition and the two gateway type "
