...
public class CreateBusinessProcessTool(
	...
	[McpServerTool(Name = CreateBusinessProcessToolName, ReadOnly = false, Destructive = false, Idempotent = false,
	 ...
	 Description("Build a business process on a Creatio environment from a declarative JSON descriptor. The "
		 ...
		 + "condition — a boolean formula, validated by the platform at the pre-save gate. Declare the branch "
		 + "here rather than building the flow plain and setting its condition afterwards: the two-step route "
		 + "saves the process once with a flow that does not yet branch. FLOW ORDER IS BRANCH PRECEDENCE — "
