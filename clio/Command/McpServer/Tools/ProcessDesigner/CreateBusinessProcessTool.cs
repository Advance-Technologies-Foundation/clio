...
public class CreateBusinessProcessTool(
	...
	[McpServerTool(Name = CreateBusinessProcessToolName, ReadOnly = false, Destructive = false, Idempotent = false,
	 ...
	 Description("Build a business process on a Creatio environment from a declarative JSON descriptor. The "
		 ...
		 + "condition — a boolean formula, validated by the platform at the pre-save gate. WHICH conditions can be "
		 + "written HERE is decided by one mechanism: a condition references a parameter by its UId meta-path, never "
		 + "by name, and on this path those UIds DO NOT EXIST YET - the process is being created by this very "
		 + "call and there is no way to pre-declare one. So declare the branch here when the condition needs no "
		 + "parameter UId: a literal, or a [#SysSettings.Code<Type>#] reference. A condition on a PROCESS PARAMETER "
		 + "or an ELEMENT OUTPUT (together 88% of the conditions in the shipped product) cannot be expressed here at "
		 + "all - build that flow as 'default' or plain, then set its condition with modify-business-process (setFlow "
		 + "kind conditional, or setFlowCondition). A name like [#Amount#] is NOT a shortcut: the pre-save gate "
		 + "refuses it with 'Expression expected (at index 0)' and the whole build is aborted. "
		 ...
		 + "validation failed' and nothing is created. On CrtProcessBuilder this clio requires 1.4.0.59, which is "
		 ...
		 + "with the package's own wording. The floor is 1.4.0.59 rather than .44 for a different KIND of "
		 + "reason: below it the package refuses flows[].kind, flows[].condition and the two gateway type "
		 + "tokens outright, so the capability - not merely the wording of a refusal - is what the floor buys. "
