...
public class CreateBusinessProcessTool(
	...
	[McpServerTool(Name = CreateBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("Build a business process on a Creatio environment from a declarative JSON descriptor. "
		 ...
		 + "1.6.2.16: a connector leaving an Approval has no formula field in the designer at all, so a "
		 + "condition set there RUNS and no human can read or edit it - nothing REFUSES that today, so the "
		 + "responsibility is yours. Use all THREE outcomes: a two-way split drops the canceled case. describe "
		 ...
		 + "flows[].condition above; a branch on an activity RESULT is built here too, from 1.6.2.16, with flows[].results.) "
