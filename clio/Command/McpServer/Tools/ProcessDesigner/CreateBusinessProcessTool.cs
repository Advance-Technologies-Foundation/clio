...
public class CreateBusinessProcessTool(
	...
	[McpServerTool(Name = CreateBusinessProcessToolName, ReadOnly = false, Destructive = false, Idempotent = false,
	 ...
	 Description("Build a business process on a Creatio environment from a declarative JSON descriptor. The "
		 ...
		 + "environment offers one. recordType? (a record-type UId) is REQUIRED only when the chosen page serves "
		 + "several types (Activity -> Task/Call/Email register the SAME page once per type): ambiguity is refused "
		 + "listing the types rather than resolved by picking one, because they open different records. "
		 ...
		 + "designer's own initial state. showPage is written as true at create unless you pass false. "
