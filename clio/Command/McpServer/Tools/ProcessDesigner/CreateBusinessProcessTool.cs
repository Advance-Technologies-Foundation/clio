...
public class CreateBusinessProcessTool(
	...
	[McpServerTool(Name = CreateBusinessProcessToolName, ReadOnly = false, Destructive = false, Idempotent = false,
	 ...
	 Description("Build a business process on a Creatio environment from a declarative JSON descriptor. The "
		 ...
		 + "source object/filters/read columns cannot be set yet, tell the user), caption, userTaskName?, "
		 ...
		 + "body? (the HTML custom message, stored verbatim; platform macro image tokens pass through; a "
		 + "server-built body REOPENS in the designer's Content designer as an editable block — verified on a stand — "
		 + "so no extra structural config is needed and the user can keep editing it by hand), "
		 ...
		 + "referenceSchema? (object name, e.g. City — makes it a Lookup), direction (one of In / Out / Variable / "
		 + "Internal — those EXACT tokens; 'Input'/'Output' are rejected), caption, description?, "
		 ...
		 + "(another element's output), processParameter, value (a literal constant, NOT a formula — a [#...#] macro sent as 'value' is stored as a "
		 + "constant and the runtime sends it verbatim, so use 'expression' for a macro), or expression; parameter-to-parameter mappings "
