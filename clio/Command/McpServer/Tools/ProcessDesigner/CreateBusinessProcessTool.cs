...
public class CreateBusinessProcessTool(
	...
	[McpServerTool(Name = CreateBusinessProcessToolName, ReadOnly = false, Destructive = false, Idempotent = false,
	 ...
	 Description("Build a business process on a Creatio environment from a declarative JSON descriptor. The "
		 ...
		 + "reason: below it the package refuses flows[].kind and the two gateway type tokens outright, and "
		 + "below .60 a flows[].condition can only reference a system setting - 88% of real conditions name a "
		 + "parameter, which needs the server-side expansion. The capability, not the wording of a refusal, is "
		 + "what this floor buys. "
