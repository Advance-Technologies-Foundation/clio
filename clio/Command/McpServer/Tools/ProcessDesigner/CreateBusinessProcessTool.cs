...
public class CreateBusinessProcessTool(
	...
	[McpServerTool(Name = CreateBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
	 ...
	 Description("Build a business process on a Creatio environment from a declarative JSON descriptor. "
		 ...
		 + "IS: flows[].kind 'conditional' plus flows[].results with the verdict captions ([\"Positive\"]). Do "
		 + "NOT use a formula, and this is the instruction on this element most likely to be got wrong because "
		 + "it was the advice given until CrtProcessBuilder 1.6.2.11: a connector leaving an Approval has no "
		 + "formula field in the designer at all, so a condition set there RUNS and is unmaintainable. From "
		 + "1.6.2.11 setFlowCondition refuses it and names the results on offer. The outcome set is THREE "
		 + "values — a two-way Approved/Rejected split drops the canceled case. Formula and system-setting "
		 + "value sources are not offered. get-guidance name=process-approval owns this block's full "
		 + "contract, process-branch-conditions the dialect rule), "
		 ...
		 + "reads the flows back after the build and warns when a label you sent did not land), parameters[] ({name, type (a supported scalar or Lookup — other types rejected), "
