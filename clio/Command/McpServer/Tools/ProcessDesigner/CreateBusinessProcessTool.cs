...
public class CreateBusinessProcessTool(
	...
	[McpServerTool(Name = CreateBusinessProcessToolName, ReadOnly = false, Destructive = false, Idempotent = false,
	 ...
	 Description("Build a business process on a Creatio environment from a declarative JSON descriptor. The "
		 ...
		 + "columns?:[<ColumnName>,...] (omit or pass [] to read ALL columns), sort?:{column, direction?:asc|desc}} "
		 + "— configures WHAT the element reads; WHICH records qualify is the element's separate filter block, and "
		 + "the read result feeds later steps via mappings from the element's output parameters), "
		 + "changeData? (changeData elements only: {source:<EntityName> (required), values:[{column, and exactly ONE "
		 + "of value (constant) | processParameter | sourceElement + sourceElementParameter | expression}] (required, "
		 + "one entry per column)} — configures WHAT the element updates; WHICH records is the element's filter block "
		 + "(effectively mandatory — the runtime refuses to update with an empty filter; to update the record read by "
		 + "a preceding readData element, filter on Id with an elementParameter reference to that element's Id "
		 + "output), "
