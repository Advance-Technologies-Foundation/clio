...
public class CreateBusinessProcessTool(
	...
	[McpServerTool(Name = CreateBusinessProcessToolName, ReadOnly = false, Destructive = false, Idempotent = false,
	 ...
	 Description("Build a business process on a Creatio environment from a declarative JSON descriptor. The "
		 ...
		 + "page to a user. It is the DEFAULT choice whenever a user fills in COLUMNS of a record; the other two page "
		 + "elements (Auto-generated page, Pre-configured page) are NOT buildable here, and picking between them is "
		 + "your decision, not a question for the user (asking which OBJECT or COLUMN is meant is fine): "
		 ...
		 + "element names — PLAIN sequence flows only; a conditional branch is not buildable here, build the flow "
		 + "plain and then set its condition with modify-business-process setFlowCondition), parameters[] ({name, type (a supported scalar or Lookup — other types rejected), "
		 ...
		 + "validation failed' and nothing is created. On CrtProcessBuilder this clio requires 1.4.0.44, which is "
