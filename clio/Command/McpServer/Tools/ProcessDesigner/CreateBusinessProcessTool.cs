...
public class CreateBusinessProcessTool(
	...
	[McpServerTool(Name = CreateBusinessProcessToolName, ReadOnly = false, Destructive = false, Idempotent = false,
	 ...
	 Description("Build a business process on a Creatio environment from a declarative JSON descriptor. The "
		 ...
		 + "openEditPage (aliases readData/performTask), caption, userTaskName?, "
		 + "openEditPage? (openEditPage elements only — the \"Open edit page\" element, which shows a record's edit "
		 + "page to a user. It is the DEFAULT choice whenever a user fills in COLUMNS of a record - the other two "
		 + "page elements (Auto-generated page, Pre-configured page) are NOT buildable here, so routing such a "
		 + "request to one of them yields no process at all; and picking between them is your decision, not a "
		 + "question for the user (asking which OBJECT or COLUMN is meant is fine): "
		 + "{page:<PageSchemaName> (required at create, e.g. AccountPageV2 or Accounts_FormPage; "
		 + "pageSchemaUId? is the escape hatch when the name is unknown) — the target OBJECT and, for a typed object, "
		 + "the RECORD TYPE are DERIVED from it, never supplied. Only a page REGISTERED ON A SECTION can be opened: "
		 + "anything else is refused, because the designer resolves the stored page against that same list and would "
		 + "render 'Which page to open?' empty, losing the element's configuration on the next human save — discover "
		 + "valid pages with list-entity-client-schemas for the object and PREFER a kind:freedom entry where the "
		 + "environment offers one. recordType? (a record-type UId) is an optional CHECK, not a selector: the "
		 + "designer offers ONE entry per page, so the record type FOLLOWS the page rather than being chosen - "
		 + "pass it to assert which registration you expect and be refused, naming the registered type, when the "
		 + "environment disagrees. "
		 + "editMode:add|edit (required at create) — 'add' lets the user create a new record and unlocks "
		 + "defaultValues, 'edit' opens an existing one and REQUIRES recordId. The two are mutually exclusive IN "
		 + "STORAGE, not only in the UI, so supplying the other mode's field is refused. defaultValues? (add mode "
		 + "only) pre-fills the new record: [{column, and exactly ONE of value (a constant — TEXT columns only and "
		 + "non-empty; a date/lookup/numeric constant is refused because the runtime reads those typed) | "
		 + "processParameter | sourceElement + sourceElementParameter (an EARLIER element's output) | expression (a "
		 + "raw macro, which is how a LOOKUP value is set: [#Lookup.{objectSchemaUId}.{recordId}#])}] — the same "
		 + "entry shape and the same stored format a Modify data element's values use; an EMPTY array means 'no "
		 + "pre-filled values' - on a modify it CLEARS the stored set, which omitting the field never does. "
		 + "recordId? (edit mode only, "
		 + "required there) says WHICH record to open: exactly one of value (a fixed record Id — wrapped into the "
		 + "lookup macro against the page's own object for you) | processParameter | sourceElement + "
		 + "sourceElementParameter (e.g. a signalStart element's RecordId) | expression. recommendation? is the text "
		 + "shown on the opened page — REQUIRED by the designer, so it defaults to the element caption; the platform "
		 + "stores a SINGLE line, and a line break is REFUSED rather than silently re-encoded (for a value from the "
		 + "process use addMapping against the Recommendation parameter instead, the same policy the email subject "
		 + "follows). hint? is the extra information behind the page's info button. completion?:{mode:onSave|"
		 + "onConditions} — onSave (the default) completes the step as soon as the user saves; onConditions completes "
		 + "it only when the saved record matches the element's separate filter block, and REQUIRES that filter in "
		 + "the same request - a filter whose condition group is EMPTY counts as no conditions and is refused too, "
		 + "because the runtime evaluates an empty group as matching everything. The pairing is enforced BOTH ways: the runtime applies the filter only in onConditions, "
		 + "so a filter without the mode (or the mode without a filter) would store, compile and run green while the "
		 + "condition is silently ignored. performer?:{type:user|manager|role, contact? (user/manager: a contact "
		 + "formula, defaulting to the current user), role? (role: a SysAdminUnit record id OR a role NAME resolved "
		 + "for you - an unknown name is refused), showPage? (whether the page opens by itself)} is 'Who performs "
		 + "the task?' with 'Show page automatically'; omit the block to leave the step unassigned, which is the "
		 + "designer's own initial state and does NOT mean nobody gets the step - the platform runs an unassigned "
		 + "one for the CURRENT USER, which is what the designer card shows for an element storing no performer. showPage follows the performer at create: TRUE for a user performer (or "
		 + "none at all), FALSE for manager/role - the platform opens the page automatically only for the user it "
		 + "is assigned to, and the designer disables that checkbox for the other two. An explicit showPage:true "
		 + "on manager/role is refused rather than stored and ignored. "
		 + "logActivity?:{enabled? (the 'Log activity' checkbox - supplying the block turns it ON unless you "
		 + "pass false), startIn?/duration?/remindIn? (each {value, unit:minutes|hours|days|weeks|months}), "
		 + "showInCalendar?, priority? (an ActivityPriority lookup NAME such as 'Medium', or its record id - an "
		 + "unknown name is refused rather than defaulted)} makes the step create an Activity record. The unit is REQUIRED with a non-zero "
		 + "value and travels WITH it: the platform stores the number and its unit as two separate parameters, "
		 + "so a number alone would be measured in whatever unit the element already had - stored, compiled and "
		 + "run green while meaning something else, which is why it is refused here. Scheduling fields with "
		 + "enabled:false are refused too (no activity is created for them to describe). Enabling it also makes "
		 + "this element's 'Connected to' links effective at run time. "
		 + "resultsByColumn?:{enabled?, column} is 'Create a list of results by column': the step's outcome "
		 + "becomes one result per value of a LOOKUP column of the page's object (a non-lookup column is refused - "
		 + "the runtime builds the list by enumerating the column's referenced object, so another kind would give "
		 + "an empty list), and column is REQUIRED unless you pass enabled:false. TELL THE USER before building "
		 + "one: this tool cannot yet create the CONDITIONAL flows that route those results (gateways and "
		 + "conditional/default flows are not buildable), so the process carries the result list but branches on "
		 + "it only after a human wires the flows in the designer}), "
