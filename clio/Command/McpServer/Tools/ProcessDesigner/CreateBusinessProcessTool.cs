...
public class CreateBusinessProcessTool(
	...
	[McpServerTool(Name = CreateBusinessProcessToolName, ReadOnly = false, Destructive = true, Idempotent = false,
		 OpenWorld = false),
	 Description("BEFORE CALLING with an accessRights block: that block changes who can read, edit or delete LIVE records. Show the user the target object, the element record filter that decides WHICH records are affected, and every grantee with its operations and level - calling out level:delegate as onward re-sharing, level:restrict as the platform Deny level, which is UNVERIFIED: the same value is captioned \"NotSet\" on the record-rights detail and appears in no captured designer specimen, and a restrict entry lands in the add (GRANT) collection, so if the runtime does not treat it as a deny it has ADDED an entry for that grantee instead of blocking one, a remove entry as a revoke, and a supplied add/remove as a REPLACEMENT that drops every entry it does not restate - and get an explicit yes. The element has no output parameters, so nothing at run time will report what it did. "
		 ...
		 + "accessRights? (changeAccessRights elements only — the Change access rights/ChangeAdminRightsUserTask element: grant/revoke record permissions: {object:<EntityName> (required; the object MUST use record permissions — one with AdministratedByRecords off is REFUSED, because the element's runtime silently does NOTHING there and has no output to say so), considerTimeInFilter?, add?/remove? (permission entries, each {operations:[read|edit|delete] (at least one), level?:permit|delegate|restrict (ADD entries only — permit is the default, delegate = the grantee may share the right onward, and restrict writes the platform Deny level and is UNVERIFIED - the same value is captioned \"NotSet\" on the record-rights detail and no captured designer specimen uses it, so do not treat it as a verified substitute for a remove entry; it also lands in the add (GRANT) collection, and was observed to write NO right at all on an object with UseDenyRecordRights OFF - it neither denied nor granted, and nothing reported that; with deny rights ENABLED it is untested. Confirm on a stand before reporting an access block as achieved; a level on a remove entry is refused), grantee:{type:role|employee|selectedEmployees, role? (a role name, record id or [#Lookup…#] macro — checked against the platform role view; an ambiguous name is refused, pass the id), contact? (a contact record id or an echoed [#Lookup…#] macro — both checked to exist — or any other formula stored verbatim, e.g. a process-parameter meta-path; contact NAMES are deliberately not resolved), filter? (selectedEmployees only: a filter block over Contact — 'object' must be Contact or omitted; NOTE the runtime evaluates it with record rights DISABLED)}})} — the legacy allRolesAndUsers grantee is describe-only and refused on write. WHICH records get the permissions is the element's separate record filter, effectively MANDATORY: the runtime silently does nothing on an empty one (target one record by filtering Id against a process parameter or a trigger output). The element has NO output parameters — nothing downstream can branch on whether rights were applied. TWO configurations build green and then silently do NOTHING at run time, and NEITHER is refused: the total ABSENCE of a record filter, and add/remove both empty (a filter that IS present but carries no conditions is a THIRD state, also unrefused, and it acts on EVERY record instead of none) (an accessRights block carrying only 'object' is one). A revoke in either state removes no permission and still reports success, so a successful build is NOT evidence the element will do anything — check the filter and the entries yourself. clio also reads the saved process back and WARNS when the accessRights block did not land at all (a deployed CrtProcessBuilder that predates this element discards it and still answers success); if you see that warning, or one saying the read-back could not be obtained, do not report a grant or revoke as applied), "
