...
public class CreateBusinessProcessTool(
	...
	[McpServerTool(Name = CreateBusinessProcessToolName, ReadOnly = false, Destructive = false, Idempotent = false,
	 ...
	 Description("Build a business process on a Creatio environment from a declarative JSON descriptor. The "
		 ...
		 + "is what the designer itself persists), allowDelegation?:bool, "
		 ...
		 + "flag), notifyAuthor?:{emailTemplate?, recipient?:{exactly ONE of value | processParameter}} (presence "
		 + "switches 'Notify about the approval result' ON), ignoreEmailErrors?:bool (already true by default on the "
		 + "platform)}. THREE parameters are DERIVED from 'object' server-side and never accepted as input — the visa "
		 + "schema, its master column and the section — with the platform's SysApproval fallback when the object has "
		 + "no approval settings. TWO deliberate gaps: the APPROVER (who approves — user/role/employee's manager) is "
		 + "NOT part of this contract and must still be set in the designer, so an element built here is incomplete "
		 + "until it is; and BRANCHING on the outcome is not buildable at all (gateways and conditional flows are "
		 + "not supported yet), so the approved/rejected/canceled result cannot be routed — the outcome is readable "
		 + "as the element's ResultParameter output. Formula and system-setting value sources are not offered), "
		 ...
		 + "ConstValue the runtime actually reads (the route ships from CrtProcessBuilder 1.4.1.0; THIS clio "
