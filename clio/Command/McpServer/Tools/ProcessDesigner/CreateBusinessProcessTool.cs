...
public class CreateBusinessProcessTool(
	...
	[McpServerTool(Name = CreateBusinessProcessToolName, ReadOnly = false, Destructive = false, Idempotent = false,
	 ...
	 Description("Build a business process on a Creatio environment from a declarative JSON descriptor. The "
		 ...
		 + "ResultEntityCollection + the shaped ResultCompositeObjectList, REQUIRES columns, takes numberOfRecords?:<n> "
		 + "for a top-N; count = how many match → ResultCount; aggregation = sum|avg|min|max of a column → REQUIRES "
		 + "aggregation:{function, column}, output picked by the column's type: Integer → ResultIntegerFunction, "
		 + "Float/Money → ResultFloatFunction, Date/Date-time/Time (min/max only) → ResultDateTimeFunction; any other "
		 + "column type is refused because the runtime would write NO result; sort and non-empty columns are refused "
		 + "for count/aggregation — the runtime ignores both there), "
		 ...
		 + "element names), parameters[] ({name, type (a supported scalar, Lookup or Collection — other types rejected), "
		 + "referenceSchema? (object name, e.g. City — makes it a Lookup; refused on a Collection), direction, caption, "
		 + "description?, value? (a literal constant default — not a formula; refused on a Collection)}; or "
		 ...
		 + "ConstValue the runtime actually reads (THIS clio bundles CrtProcessBuilder 1.4.0.42 and "
