...
public sealed class MobilePageConversionGuideTool {
	...
	[Description(
		...
		"plus adaptiveLayout (the responsive layout for each MULTI-column grid container - phone collapses to 1 column and stacks, " +
			...
			"plus normalizations (SILENT property normalization, one section per standard the conversion rules declare - each carries its own note, the elements it normalized with the exact properties written, and anything it had to skip; the set of sections is open, so read them rather than assuming which exist; spacingNormalization is kept as a back-compat alias of the spacing section; semantics in the conversion guide), " +
