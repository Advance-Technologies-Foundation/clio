...
public sealed class MobilePageConversionGuideTool {
	...
	[Description(
		...
		"plus rules-driven positional exclusion (a component an excludedComponents filter bans from a host arrives as an elementMap drop whose reason names the rule, the type, the host and the slot — POSITIONAL, never conversion loss: do NOT re-insert it anywhere and do NOT ask the user about it; the same type outside that position converts normally; which types are banned from which hosts is converter configuration, so read the drop reasons rather than assuming a list; the do/don't rules are in the conversion guide), " +
