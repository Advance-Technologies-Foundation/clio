...
internal static class McpServerInstructions
{
	internal const string Text = """
		...
		1. **Resolve the target platform version.** Call `get-component-info` with `environment-name` set to the environment you will edit, and read `resolvedFrom` on the response:
		   - `resolvedFrom: "environment"` — the platform version is KNOWN (resolved from the environment); the catalog is authoritative, proceed with no confirmation. When no exact per-version catalog is published (e.g. an in-development `10.x`), clio serves the closest `latest` catalog and still reports `environment` — a known version carries no warning.
