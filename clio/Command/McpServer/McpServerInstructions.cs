...
internal static class McpServerInstructions
{
	internal const string Text = """
		...
		2. **Discover the full component set proactively — and do it BEFORE authoring.** Call `get-component-info` with no `component-type` (list mode) to enumerate every component available for that version, then `get-component-info component-type="<type>"` (and `composite="<caption>"` for any matching composite) for the full contract before you write a body. Non-obvious components (e.g. `crt.Gallery`) live in the catalog and must be considered and suggested when relevant — never conclude a capability is missing, or wait for the user to ask you to search, without listing the catalog first. Pass `schema-type: "mobile"` to discover the separate mobile component set. **Reverse-engineering a deployed page's compiled bundle is NOT a substitute for `get-component-info`:** a bundle shows components already expanded as plain `viewConfigDiff` elements, hiding the signals only the catalog carries — `compositeOnly`, `appliesToCustomEntities`, the composite recipe, and the `references.docs[]` authoring guide. Copying a live page's wiring without reading the component's `get-component-info` contract (and its `documentation`) is how an implementation silently lands off-spec.
		3. **Consider composites.** The same list response also returns a `composites` array — pre-built combinations of several components that have NO `componentType` of their own (e.g. "Expanded list", "Attachments", "Next steps", "Approval list", "Communication options"). When the user asks for one of these by name, do NOT hand-assemble it from raw component types: call `get-component-info` with `composite: "<caption>"` to fetch its exact assembly recipe (docs). A component listed with `compositeOnly: true` has no standalone toolbar presence — first look for a composite that assembles it: scan the `composites` array and confirm membership by reading each candidate's recipe (`composite: "<caption>"`). If a composite assembles this component, build it (`composite: "<caption>"`) and follow its recipe. If none assembles it, build the component directly as a fallback — but only when its own applicability allows (check `appliesToCustomEntities` / `entityCouplingNote` on the detail response); never build an entity-coupled component standalone on an entity those fields exclude.
