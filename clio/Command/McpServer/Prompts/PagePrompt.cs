...
public static class PagePrompt {
	...
	public static string GetPage(
		...
		$"""
		 ...
		 Both `{PageSyncTool.ToolName}` and `{PageUpdateTool.ToolName}` run a deterministic AST lint pass on every web page body after the regex validators and before sampling. Error findings block the save (body is not sent to Creatio): `handlers` must be an array literal, `validators` / `converters` must be object literals, custom converter names cannot use the reserved `crt.*` prefix, validator `params: []` is rejected, validator returns must not be literal `true` / `false` / string / empty object (null/undefined are allowed and signal "no error"). Warning findings are reported but do not block: deprecated `request.viewModel` / `request.sender` / `request.$get` / `request.$set` / `request.$context.get`, direct `fetch(...)` inside a converter, `sdk.HandlerChainService.instance.process(...)` in handlers. When a finding fires, fix the body and re-read the matching `page-schema-handlers` / `page-schema-validators` / `page-schema-converters` guidance — never bypass by stripping the offending block.
