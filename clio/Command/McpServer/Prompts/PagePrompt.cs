...
public static class PagePrompt {
	...
	public static string GetPage(
		...
		$"""
		 ...
		 Both `{PageSyncTool.ToolName}` and `{PageUpdateTool.ToolName}` run a deterministic AST lint pass on every web page body after the regex validators and before the body is persisted. The lint pass ships rules that have no regex counterpart — duplicate detection is intentionally NOT shipped (the regex layer catches anti-shapes like array/object mismatch on `handlers`/`validators`/`converters`, empty validator `params`, and the forbidden `request.viewModel` / `request.sender` / `request.$context.get` / `request.$get` / `request.$set` handler-API patterns with its own established wording). Error findings block the save (body is not sent to Creatio): custom converter names cannot use the reserved `crt.*` prefix, validator returns must not be literal `true` / `false` / string / empty object (null/undefined are allowed and signal "no error"), and bodies whose AST nests deeper than the safe traversal cap are rejected as `body-too-deeply-nested`. Warning findings are reported but do not block: direct `fetch(...)` inside a converter, and `request.$context.executeRequest(...)` in handlers (not part of the documented @creatio-devkit/common public API — prefer `sdk.HandlerChainService.instance.process(...)` from `@creatio-devkit/common`). When a finding fires, fix the body and re-read the matching `page-schema-handlers` / `page-schema-validators` / `page-schema-converters` guidance — never bypass by stripping the offending block.
		 ...
		 `resources` on the page write tools set en-US values only. To translate existing page captions into another culture, call `{LocalizePageTool.ToolName}` through `{ClioRunTool.ToolName}` (report-only first to see the missing keys); it never changes en-US or other cultures.
		 Use `{PageUpdateTool.ToolName}` only as a fallback for single-page dry-run or legacy save workflows.
		 EXTERNAL-MODIFICATION CONFLICTS: `{PageGetTool.ToolName}` stores a checksum baseline in `meta.json`; `{PageSyncTool.ToolName}` and `{PageUpdateTool.ToolName}` compare it against the server before saving. A write that fails with `conflict: true` (per-page `conflict`/`conflict-details` in sync-pages) means the page was modified outside this session — do NOT retry with the same body: re-run `{PageGetTool.ToolName}`, re-apply your change on top of the fresh body (preserve the user's external edits — never restore components they deleted), then retry. Set `force: true` ONLY after informing the user about the external changes and receiving their explicit confirmation to overwrite. On `{PageUpdateTool.ToolName}` there is a better answer than `force` for the common false conflict: pass the `editable.checksum` from your own `{PageGetTool.ToolName}` response as the `checksum` argument, and the comparison runs against the body you actually read instead of against a `meta.json` written by a different working directory or superseded by an earlier save. Detection is not weakened — a genuinely stale checksum is still refused. `{PageSyncTool.ToolName}` accepts no `checksum` yet (GH-1320), so on the canonical write path the `meta.json` baseline is still the only one, and a repeat conflict on an edit nothing external touched is the case to bring to the user rather than to `force` past. When `{PageUpdateTool.ToolName}` is called with `target-package-uid` or `target-schema-uid`, the on-disk baseline is not applicable and is not read, but an explicit `checksum` is still compared with the resolved target; a mismatch is a conflict. The response warns that the disk baseline was skipped, and that warning alone is not a reason to pass `force`.
	...
	public static string CreatePage(
		...
		$"""
		 ...
		 To create a DESKTOP (desktop-selector workspace), call `{GuidanceGetTool.ToolName}` with `name` set to `desktop-page` FIRST, then `{PageCreateTool.ToolName}` with `template` `CentralAreaDesktopTemplate`.
