...
public static class PagePrompt {
	...
	public static string GetPage(
		...
		$"""
		 ...
		 If the page-body task adds or edits localizable strings — `$Resources.Strings.*` bindings (including data-bound field captions/labels referencing a vm attribute, e.g. `$Resources.Strings.PDS_UsrStatus`) or `#ResourceString()#` macros, including custom captions, tab titles, button labels, expansion panel titles, or validator error messages — you must call `{GuidanceGetTool.ToolName}` with `name` set to `page-schema-resources` before deciding which resource pattern to use.
