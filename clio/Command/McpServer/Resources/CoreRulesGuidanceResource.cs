...
public sealed class CoreRulesGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
		...
		Text = """
		       ...
		       - Read-only / retry-safe tools (list-pages, list-apps, get-app-info, list-app-sections, get-page, …) are bounded by a response deadline (default 120s, override CLIO_MCP_READ_DEADLINE_SECONDS). If one exceeds it you get a structured result with error-class=creatio-timeout and read-response-timed-out=true: the read did not finish (a busy stand), NOT a failure of your input. It is non-destructive and safe to retry — wait briefly and retry, optionally with a narrower filter / smaller limit. This differs from the write timeout (section-created=in-progress), where you must poll and must NOT retry.
