...
public sealed class PageModificationGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
		...
		Text = """
		       ...
		       | showing a user-facing message/confirmation/info/success/error popup when a button or menu item is clicked (e.g. "just show a short confirmation message", "Approved." / "Saved.") | `page-schema-handlers` | This is a `crt.ShowDialogRequest` dispatched from a handler (message/actions under `dialogConfig.data`), and needs `@creatio-devkit/common` in `SCHEMA_DEPS` + the `sdk` alias in `SCHEMA_ARGS`. NEVER use `alert(...)` / `window.alert(...)` / `confirm(...)` / `prompt(...)` — they are raw browser primitives, not the Freedom UI dialog, and are not acceptable in deployed page-body handlers. |
