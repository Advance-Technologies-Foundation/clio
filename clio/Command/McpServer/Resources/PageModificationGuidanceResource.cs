...
public sealed class PageModificationGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
		...
		Text = """
		       ...
		       | working with a dashboard page (a page inherited from `BaseDashboardTemplate`), or adding/arranging/sizing/styling any analytics widget (metric/indicator or chart) on it | `dashboards` | Mandatory read before composing the body — owns the canonical dashboard layout, widget placement, sizing, and styling. The `dashboards` guide routes to `indicator-widget` / `chart-widget` + `get-component-info` for a widget's runtime contract. |
		       ...
		       | a purely VISUAL styling requirement — set or change a component's color, background, font/typeface, text size, spacing/padding/margin, border, width/height, or alignment (e.g. "make this red", "add padding", "use a bigger font", "change the title font", "center this") | `page-modification` (this guide — see the "CUSTOM CSS IS A LAST RESORT" rule below) | CSS is a LAST RESORT — a `styles` object, a `classes`/CSS class, or a component `extraStyles` hook is custom CSS that can break on a platform upgrade. Native component inputs (via `get-component-info`) must be exhausted first, then (only if none works) a no-native notice, an upgrade-risk warning, and explicit user confirmation are mandatory before it is written. See the "CUSTOM CSS IS A LAST RESORT" rule below. |

		       STOP — CUSTOM CSS IS A LAST RESORT, NOT A DEFAULT. This is a MANDATORY behavioral rule for create-page, update-page, and sync-pages alike. A requirement is about how a component LOOKS whenever it concerns color, background, font/typeface, text size, spacing, padding, margin, border, width/height, or alignment. Writing that look as a custom `styles` object, a `classes`/CSS class, OR a component `extraStyles` hook (e.g. `extraStyles.toggle`, `extraStyles.label`) with raw CSS values (such as `color`, `fill`, or `font-family`) is CUSTOM CSS — ALL THREE carriers count equally, do not treat `extraStyles` as "native" just because it is a component input. The Freedom UI font is Montserrat and colors/typography come from theme tokens, so changing a font-family or hard-coding a color is custom CSS, never a native default.
		       You MUST proceed in this exact order:
		       (1) NATIVE FIRST — first try to satisfy it with the component's NATIVE inputs: call `get-component-info` for the exact `crt.*` type and look for a native property that achieves the look (components commonly expose color / tone / shape / size / spacing inputs, and a component's own contract often steers you to a native input over CSS, e.g. "prefer `toggleType` over styling the toggle via `extraStyles`/CSS"). If a native input achieves it, USE it and do NOT mention CSS.
		       (2) Only when NO native input can achieve the requirement may you fall back to custom CSS (`styles` / `classes` / `extraStyles`). BEFORE writing it into the body you MUST, in this order: (a) tell the user plainly that no native solution exists for this specific requirement; (b) warn that custom CSS may break or conflict during future Creatio platform upgrades (custom styles are NOT covered by the upgrade-compatibility guarantees that native component properties have); (c) ask for explicit user confirmation to proceed with CSS.
		       (3) Do NOT write `styles` / `classes` / `extraStyles` into the body until the user has explicitly confirmed. If the user declines, do NOT apply CSS — offer the closest native alternative, or state plainly that the exact look is not achievable natively.
		       "DON'T ASK" ≠ "DON'T WARN": if the user says "don't ask", "don't warn", or "just apply", that waives the blocking QUESTION but NOT the warning — you MUST still emit at least a one-line upgrade-compatibility warning before applying the CSS, and you must NEVER apply custom CSS silently.
