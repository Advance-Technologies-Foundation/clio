...
public sealed class PageModificationGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
		...
		Text = """
		       ...
		       - `resolvedFrom: "environment"` means clio resolved the platform version from the active environment (the version is KNOWN). Treat the catalog as authoritative for `update-page` and proceed — no confirmation needed. If the exact per-version catalog is not published (e.g. an in-development platform version such as `10.x`), clio serves the closest `latest` catalog and STILL reports `environment`: the version is known, so there is no version warning.
