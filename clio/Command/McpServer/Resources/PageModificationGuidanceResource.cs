...
public sealed class PageModificationGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
		...
		Text = """
		       ...
		       | a purely VISUAL styling requirement — set or change a component's color, background, font, text size, spacing/padding/margin, border, width/height, or alignment (e.g. "make this red", "add padding", "use a bigger font", "center this") | `page-modification` (this guide — see the "CUSTOM CSS IS A LAST RESORT" rule below) | CSS is a LAST RESORT. Native component inputs (discovered via `get-component-info`) must be exhausted first. A custom `styles` object or a `classes`/CSS class risks breaking or conflicting on a platform upgrade, so it requires an explicit upgrade-risk warning AND user confirmation before it is written. Enforcement: update-page / sync-pages HARD-REJECT a body that introduces a `styles` object unless allow-custom-css=true (set only after the user confirms); validate-page surfaces it as a warning; `classes` is not gated. |

		       STOP — CUSTOM CSS IS A LAST RESORT, NOT A DEFAULT. When a requirement is about how a component LOOKS (color, background, font, text size, spacing, padding, margin, border, width/height, alignment), you MUST first try to satisfy it with the component's NATIVE inputs: call `get-component-info` for the exact `crt.*` type and look for a native property that achieves the look (components commonly expose color / tone / shape / size / spacing inputs). If a native input achieves it, USE it and do NOT mention CSS. Only when NO native input can achieve the requirement may you fall back to a custom `styles` object or a `classes`/CSS class — and BEFORE writing it into the body you MUST, in this order: (1) tell the user plainly that no native solution exists for this specific requirement; (2) warn that custom CSS may break or conflict during future Creatio platform upgrades (custom styles are NOT covered by the upgrade-compatibility guarantees that native component properties have); (3) ask for explicit user confirmation to proceed with CSS. Do NOT write `styles` / `classes` into `viewConfigDiff` until the user has explicitly confirmed. If the user declines, do NOT apply CSS — offer the closest native alternative, or state plainly that the exact look is not achievable natively. This rule applies to create-page, update-page, and sync-pages equally. This is enforced, not advisory: update-page and sync-pages HARD-REJECT a body that introduces a 'styles' object until you pass allow-custom-css=true (only after the user confirms). validate-page reports it as a warning. classes are not gated. "DON'T ASK" ≠ "DON'T WARN": if the user says "don't ask", "don't warn", or "just apply", that waives the blocking QUESTION but NOT the warning — you MUST still emit at least a one-line upgrade-compatibility warning before setting allow-custom-css=true, and you must NEVER set allow-custom-css=true silently. Setting that flag is your on-record statement that the user was warned about the upgrade risk.
