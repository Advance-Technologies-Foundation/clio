...
public sealed class PageModificationGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
		...
		Text = """
		       ...
		       | restricting / filtering which records a lookup or ComboBox field offers (e.g. "show only contacts who were created this year", "limit the Assignee field to contacts with a mobile phone", "only accounts that have at least one contact", "show the <Field> only for <records> where …") | `business-rules` | Lookup record-set restriction is an ENTITY business rule (`apply-static-filter`), NOT a page edit. Do NOT add `filterConfig` / `staticFilters` / `dataSourceFilters` to a datasource list attribute for this — call `create-entity-business-rule`. The entity rule applies everywhere the lookup is used and is validated; hand-edited page filters are page-scoped and brittle. This includes restrictions by a computed or relative date/time (e.g. "birthday tomorrow", "created last quarter", "due in the next 7 days", "created at 11:06 AM previous quarter") and conditional/gated restrictions (e.g. "when Call back is enabled, limit Contact to contacts with a mobile phone") — a relative-date/time restriction is still apply-static-filter using a date macro, NOT a handler (do NOT use crt.InitRequest); a gated restriction puts the gate (X = Y) in the rule's condition group with the apply-static-filter action on the target lookup. |
		       ...
		       | business logic, cross-field orchestration, async data loading, side effects | `page-schema-handlers` | Handlers are NOT the same as converters. NOTE: restricting which records a lookup/ComboBox offers — even by a computed or relative date/time (e.g. "birthday tomorrow", "created last quarter") — is NOT handler "business logic"; it is an entity business rule (apply-static-filter). See the lookup-restriction row above. |
		       ...
		       STOP. Adding a related/child list (a "detail"), or making a list show only the records that belong to the current/open record ("filter by page data"), is NOT a single-component insert and NOT an inline `filter` on the list attribute. It is a master-detail composite (`crt.ExpansionPanel` + `crt.DataGrid`) plus a page-scoped `crt.EntityDataSource`, an `isCollection` attribute, and a SEPARATE filter attribute referenced through `modelConfig.filterAttributes`. Read `get-guidance` with name `related-list` before writing the body — an inline `filter` on the collection attribute is silently ignored and the list shows every record.
