...
public sealed class PageModificationGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
		...
		Text = """
		       ...
		       | indicator widget / metric widget / KPI card / aggregate widget creation or editing, especially static widget filters and COUNT/SUM/AVG/MIN/MAX requirements | `indicator-widget` | Indicator widgets use a runtime aggregate-query config shape that differs from a conceptual `from`/`select`/`filters` intent. Read the dedicated translation guide before editing `crt.IndicatorWidget`. |
