...
public sealed class AppModelingGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
			...
			Text = """
			       ...
			       - `create-app` automatically creates: (1) a **package** named after `code`; (2) the **app** record; (3) one default **section** record in `ApplicationSection` that wires the canonical main entity to Creatio's navigation bar — no separate `create-app-section` call is needed or correct for this section; (4) the **canonical main entity** (schema code = `code`) with one default column `UsrName` (MediumText); and (5) **five Freedom UI pages** — `{code}_FormPage`, `{code}_ListPage`, `{code}_Detail`, `{code}_MobileFormPage`, and `{code}_MobileListPage`. The response returns the entity manifest under `entities` and the page manifest under `pages`; the section record itself is not in the response (use `canonical-main-entity-name` to identify the main entity). Read that manifest instead of re-discovering artifacts via additional tool calls.
			       - The auto-generated `{code}_FormPage` starts with `UsrName` pre-inserted in `SideAreaProfileContainer` and Feed + AttachmentList tabs. The auto-generated `{code}_ListPage` starts with `UsrName`, `CreatedOn`, `CreatedBy` as default DataTable columns. Replace or extend these defaults when configuring the section; always call `get-page` to read the current body before writing.
			       - `create-app` already creates the default section for the canonical main entity — no separate `create-app-section` call is needed for it. Use `create-app-section` only when the app needs an additional section backed by a new, separately named entity; Creatio derives the new entity code from the `caption` you provide (e.g. caption `Customer Profile` → entity `UsrCustomerProfile`). For the primary record type, extend the canonical main entity returned by `create-app` — do not create a synonym section for it.
