...
public sealed class AppModelingGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
			...
			Text = """
			       ...
			       - `sync-schemas` retries transient network faults (DNS/reset/timeout/gateway) per operation and, on a mid-batch abort, returns a `resume-plan`. On `success: false` with a `resume-plan`, resubmit ONLY `resume-plan.operations`; do not resend completed operations or hand-build a catch-up batch.
