...
public sealed class AppModelingGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
			...
			Text = """
			       ...
			       - `sync-schemas` retries transient network faults (DNS/reset/timeout/gateway) per operation and, on a mid-batch abort, returns a `resume-plan`. Because the schema ops are convergent, re-submitting the whole batch verbatim is safe; resubmitting only `resume-plan.operations` is the efficient path and is required for `seed-data` (not replay-safe), which the plan converts to a standalone op.
