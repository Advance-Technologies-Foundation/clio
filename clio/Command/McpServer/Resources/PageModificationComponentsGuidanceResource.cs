...
public sealed class PageModificationComponentsGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
		...
		Text = """
		       ...
		       STOP — CUSTOM CSS IS A LAST RESORT. Custom CSS is any of three carriers — a `styles` object, a `classes`/CSS class, or a component `extraStyles` hook (e.g. `extraStyles.toggle`, `extraStyles.label` with `color`/`fill`/`font-family`); `extraStyles` is NOT native just because it is a component input, and the Freedom UI font is Montserrat, so changing a font-family or hard-coding a color is custom CSS. It is NOT covered by the upgrade-compatibility guarantees native inputs have, so it can break or conflict during future Creatio platform upgrades. Order for any visual-styling requirement (color, font/typeface, size, spacing, border, alignment): (1) first try to satisfy it with the component's NATIVE inputs (get-component-info) — if one achieves the look, USE it and do NOT mention CSS; (2) only if none does, tell the user no native solution exists, warn about the upgrade risk, and ask for explicit user confirmation to proceed with CSS before writing any `styles`/`classes`/`extraStyles`; (3) if the user declines, do NOT apply CSS — offer the nearest native alternative. "DON'T ASK" is not "DON'T WARN": a "just apply" instruction waives the question but never the one-line upgrade-risk warning — NEVER apply custom CSS silently.
		       """
	};
