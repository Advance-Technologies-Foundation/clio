...
public sealed class SysSettingsGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
			...
			Text = """
			       ...
			       - `Binary` (a value stored as blob data, e.g. the logo) is WRITE-ONLY through this surface. Set the value with `update-sys-setting` using `value-file-path`: clio reads the file and Base64-encodes it locally, then sends it through the same `PostSysSettingsValues` path as every other type. (`create-sys-setting` has no `value-file-path`; its optional initial `value` accepts inline Base64 only, so for a blob value create the setting first, then upload with `update-sys-setting`.) Do NOT inline the Base64 in `value` for large blobs — a logo is thousands of tokens; pass the file path instead. Uploads honor the environment's file-security policy (`FileSecurityMode` + `FileExtensionsAllowList`/`FileExtensionsDenyList` + `AllowFilesWithUnknownType`): a blocked extension is rejected client-side, and while a policy is active an inline Binary `value` (which has no extension to check) is refused — use `value-file-path`. There is a per-value size cap (default 10 MB) enforced on both file and inline input. Reading a `Binary` value back is not exposed through MCP: `list-sys-settings` DOES list Binary settings (so you can discover codes like `LogoImage`) but shows their value as `<binary>`, and `get-sys-setting` returns an empty value for them, because clio's `SysSettingsValue` model does not map the platform's binary media column. (If you genuinely need the raw bytes, the legacy CLI `clio get-syssetting <code>` returns the Base64 via the cliogate endpoint — but avoid piping a multi-thousand-token blob back through an agent.)
