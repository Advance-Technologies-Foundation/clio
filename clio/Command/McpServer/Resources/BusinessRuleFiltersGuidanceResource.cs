...
public sealed class BusinessRuleFiltersGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
		...
		Text = """
		       ...
		       - "at <HH:mm> [AM/PM]" / "in calendar year <YYYY>" / "every <Nth> of the month" / "each <weekday>" / "in <month>" → a FIXED clock/calendar part: use `datePart` (see leaf fields), NOT a relative macro. "created at 11:06 AM" is `datePart: "HourMinute"` (alias `Time`) with `value: "11:06:00"`. This IS expressible — never report exact time-of-day as unsupported or drop it. For "previous quarter AND at 11:06 AM" emit two leaves under AND: a `valueMacros: "PreviousQuarter"` leaf and a `datePart: "HourMinute"` leaf, both on CreatedOn.
