	...
	internal static readonly TextResourceContents Guide = new() {
		...
		Text = """
		       ...
		       - translating natural-language temporal/age phrases:
		         ...
		         - "birthday today/tomorrow" (day-and-month match ignoring year) → `valueMacros: "DayOfYearTodayPlusDaysOffset"` with `valueMacrosArgument` = day offset (0=today, 1=tomorrow) on the BirthDate column. This is NOT the same as `Today`/`Tomorrow` (which match the full date including year).
		         - "aged between X and Y" / "age = N" / "age < N": Contact has no stored Age column — translate to a BirthDate range. Resolve today's date (call GetCurrentDateTime if unsure), then BirthDate GREATER_OR_EQUAL (today − Y years) AND LESS_OR_EQUAL (today − X years) for "between X and Y"; "age = N" → a one-year BirthDate window; "age < N" → BirthDate GREATER (today − N years). Emit fixed ISO date constants (column arithmetic inside filter values is not supported).
