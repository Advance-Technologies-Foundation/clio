...
public sealed class BusinessRulesGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
		...
		Text = """
		       ...
		       Page condition operand sources (scopeId)
		          - A page-rule condition operand of type AttributeValue carries an optional `scopeId` that selects WHERE its `path` is resolved at runtime. It is the sole platform discriminator; there is no separate operand type per source.
		            - scopeId omitted / empty -> a ROOT page attribute: either a surfaced datasource-bound attribute (from bundle.viewModelConfig.attributes) OR an unbound/technical page-local attribute (an attribute with a declared type / default value and no modelConfig.path — for example a value computed in a handler extension).
		            - scopeId = "PageParameters" -> a PAGE PARAMETER (the page's input parameters, exposed as the PageParameters datasource). Use this to react to values a business process or a parent page pushes into the page on open.
		            - scopeId = a DataSource name from bundle.modelConfig.dataSources (for example "PDS") -> a DataSource FIELD, INCLUDING a column that exists in the entity/DataSource but is NOT surfaced on the page. `path` is the column name within that datasource (a forward path such as "Contact.Account" is allowed).
		          - New operand type `SysSetting` (field `sysSettingName`, the SysSettings code) compares against a system setting's value; its data value type is inherited from the operand it is compared against, like a Const. This satisfies "compare a Page parameter with a system setting". clio cannot enumerate system settings, so `sysSettingName` is not validated at authoring time — a missing or misspelled code resolves to the platform default at runtime with no error, so confirm the setting code exists first.
		          - Type compatibility is enforced across scopes: both operands must resolve to the same data value type, and lookup operands must reference the same schema — a Lookup DataSource field is not comparable to a Text page parameter.
		          - FEATURE GATE: these page condition sources (DataSource field, page parameter, SysSetting operand, unbound page-local attribute) are OFF by default behind the `page-business-rule-condition-sources` feature. While off, page rules keep their prior behaviour and any non-empty scopeId or SysSetting operand is rejected. Enable with `clio experimental --name page-business-rule-condition-sources --enable`.
		          - CAVEAT (runtime vs legacy designer): a rule authored with these sources EXECUTES at runtime, but the legacy 7.x visual condition designer may not yet round-trip a page-parameter / scoped operand for manual editing. Tell the user such a rule is agent-authored and may not be editable in the visual designer until the platform designer adds support. DataSource-field scope is used by shipped Creatio packages and is the safest of the three.
