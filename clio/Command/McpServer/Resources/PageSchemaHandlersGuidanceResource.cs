...
public sealed class PageSchemaHandlersGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
		...
		Text = """
		       ...
		       - ERROR: `request.$get(...)` and `request.$set(...)` — these methods do NOT exist on the handler-chain request object and throw `TypeError: request.$get/$set is not a function` at runtime; flagged as `handler-uses-nonexistent-request-api` and the body is rejected before sampling. Use `await request.$context["<Attr>"]` to read and `await request.$context.set("<Attr>", <value>)` to write.
		       - WARNING (reported alongside the response, write still proceeds): `request.viewModel`, `request.sender`, and `request.$context.get(...)` are flagged with `handler-uses-deprecated-context-api`. They may still resolve at runtime in some flows but are not part of the documented handler API; migrate to the `request.$context["<Attr>"]` / `await request.$context.set(...)` / `request.next?.handle(request)` patterns above.
		       - WARNING: `request.$context.executeRequest(...)` is flagged with `handler-uses-context-execute-request`. Use `await sdk.HandlerChainService.instance.process({ type, $context, scopes })` instead — `executeRequest` is reachable from handler code but is NOT part of the documented `@creatio-devkit/common` public surface, while `HandlerChainService.instance.process(...)` is the canonical SCHEMA_HANDLERS dispatcher per Creatio Academy.
