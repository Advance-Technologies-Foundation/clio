...
public sealed class ThemingGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
		...
		Text = """
		       ...
		       Prerequisites: the `CanCustomizeBranding` license and the `CanManageThemes` system operation. Verify both in one call with `check-theming-access-by-environment` (or `-by-credentials`) — it returns `{ canManageThemes, canCustomizeBranding, hasThemingAccess }`; proceed only when `hasThemingAccess` is true, otherwise tell the user which of the two is missing.
