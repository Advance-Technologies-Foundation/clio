...
public sealed class ThemingGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
		...
		Text = """
		       ...
		       This applies a theme to the profile of the account clio is authenticated as — only that user, not everyone (that is the global default; see "Get / set the default theme"). Because it touches only the caller's own profile it needs no confirmation.
		       - After a successful no-code `create-theme`, apply the new theme to the current user by default: call `set-user-theme` with the theme's `id` (or its caption / css-class-name), then tell the user to refresh the page — the change is visible on the next page load, and no cache flush or session-refresh call is needed. Do NOT set the global `DefaultTheme` to "apply" a theme unless the user asked to change it for everyone.
