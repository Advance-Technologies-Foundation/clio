...
public sealed class ThemingGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
		...
		Text = """
		       ...
		       Once the colours are settled, pass their -500 values to the build step of your flow — see "Building the theme CSS".

		       Building the theme CSS
		       - The `theme.css` is computed by a deterministic OKLCH palette engine over a bundled, version-pinned template — no hand-computed colour math and no external package. It has two entry points: `create-theme` builds it server-side inside the create call (no-code flow), and `build-theme` returns it or writes it as files (workspace flow).
		       - Required either way: a primary colour and a theme name — pass the name as `caption`; `css-class-name` is derived from it (lowercased and hyphenated) when omitted, or pass `css-class-name` explicitly. secondary/accent/success/error and fonts are optional. At least one of `caption` or `css-class-name` is required.
		       - Font weights: both entry points load a standard set of weights for any custom font by default (see the `font-weights` parameter). When a custom Google Font is chosen, confirm the family actually ships those weights — a family that offers only one weight will render the heavier ones as the nearest available fallback.
		       - Fonts come from Google Fonts. First check the family NAME against the contract build-theme enforces: clio trims it and collapses internal whitespace runs, and the normalized name must then start with a letter or digit and contain only letters, digits, spaces and hyphens, at most 100 characters — a name that breaks that FAILS the build with INVALID_FONT_FAMILY, unlike every availability outcome, which only warns. Then check availability yourself before building — fetch `https://fonts.google.com/metadata/fonts/<Family>` (URL-encode spaces), where 200 means published and 404 means not — so the conversation below happens up front and the theme is built once instead of built, warned about, and rebuilt. Family names are case-sensitive with no server-side correction, so on a 404 retry once with the exact published spelling you know ("roboto" is published as "Roboto", "PT sans" as "PT Sans") before concluding anything; some families were renamed ("Muli" is now published as "Mulish").
		       - When a family is not on Google Fonts, do not proceed on your own: say so plainly and let the user settle it — offer `https://fonts.google.com/?query=<family>` so they can find the published spelling or pick an alternative, or get their explicit confirmation that it is a locally installed font. Warn them that a local font shows only on machines where it is already installed — everywhere else the text falls back to a generic face. Once confirmed, build with the family passed as `heading-font` / `body-font` exactly as for a Google font: build-theme runs its own catalogue check, emits no web-font `@import` for an unpublished family (other families keep theirs), and returns a warning. Never hand-author an `@import` yourself.
		       - Everything said here about build-theme's font handling applies to `create-theme` brand mode too: it drives the same engine, so it runs the same catalogue check, suppresses the same imports, and returns the same warnings on its own result. In the no-code flow read those warnings off the create call — there is no separate build step to read them from.
		       - build-theme's own check is the authority: it warns instead of failing. A warning about a family the user already confirmed as local is the expected echo of that confirmation — nothing left to relay. Relay any warning you have not already settled with the user rather than swallowing it — your own check may have used a different spelling or reached the network differently — and once settled, restyle: re-run build-theme (workspace flow) or build again and `update-theme` (no-code flow).
		       - If build-theme warns that it could not verify a family (network failure), the `@import` was kept so a Google font keeps working. Tell the user; if the family is actually a local font, restyle once connectivity is back so the import gets suppressed.
