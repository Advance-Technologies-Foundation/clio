...
public sealed class ThemingGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
		...
		Text = """
		       ...
		       - Fonts come from Google Fonts. Check each requested family yourself before building — fetch `https://fonts.google.com/metadata/fonts/<Family>`, where 200 means published and 404 means not — so the conversation below happens up front and the theme is built once instead of built, warned about, and built again. Family names are case-sensitive, so try the correct spelling before concluding anything: "Roboto" resolves where "roboto" does not.
		       - When a family is not on Google Fonts, treat it as a locally installed font, and do not proceed on your own: ask for explicit confirmation, and tell them plainly that the theme will show that font only on machines where it is already installed — everywhere else the text falls back to a generic face. Once they confirm, build with the family passed as `heading-font` / `body-font` exactly as you would for a Google font AND additionally listed in `local-font-families`: the font parameters are what apply the family, while `local-font-families` only suppresses the web-font request for it. Listing a family in `local-font-families` alone applies nothing — the theme keeps its default font. Never pass `local-font-families` without that confirmation, and never hand-author an `@import` yourself.
		       - build-theme runs the same catalogue check and is the authority: it warns instead of failing when a family is missing or could not be verified. Relay a warning you have not already settled with the user rather than swallowing it — your own check may have used a different spelling or reached the network differently. A family passed to `local-font-families` is never warned about, so a confirmation you already have needs no second round: build once with the flag and there is nothing left to relay.
		       - If build-theme reports that it could not reach Google Fonts to check a family, say so instead of guessing: ask the user whether it is a Google font or a locally installed one, and use `local-font-families` only for the latter.
