...
public sealed class ThemingGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
		...
		Text = """
		       ...
		       6. Preview. Call `operation=preview` to review the base -500 colour of each role together with the system colours — the full palette stops only if the user asks (`fullStops=true`). This is a checkpoint, not an approval gate. Preview readability is for UI colours (non-text); text contrast is finalized during the build.
		       ...
		       2. Create with `create-theme-by-environment`: pass the theme name as `caption` and the CSS as inline `css-content`. `css-class-name` is optional — omit it and clio derives a valid one by slugifying the caption (pass it explicitly only to override); at least one of `caption` or `css-class-name` is required. `id` is optional — omit it to get an auto-generated id back; `package-name` is optional — omit it to use the environment's CurrentPackageId system setting.
		       3. Restyle with `update-theme-by-environment` (by id; a full overwrite of caption + cssClassName + cssContent; the package cannot be changed).
