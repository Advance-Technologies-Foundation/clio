...
public sealed class ExistingAppMaintenanceGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
			...
			Text = """
			       ...
			       - `sync-schemas` retries transient network faults (DNS/reset/timeout/gateway) per operation and returns a `resume-plan` when a batch aborts mid-way. Because the schema ops are convergent, re-submitting the whole batch verbatim is safe; resubmitting only `resume-plan.operations` (failed op + not-run ops, already in re-submittable shape) is the efficient path and is required for `seed-data`, which is NOT replay-safe and which the plan converts to a standalone op.
