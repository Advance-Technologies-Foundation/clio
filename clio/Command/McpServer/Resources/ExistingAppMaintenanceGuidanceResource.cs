...
public sealed class ExistingAppMaintenanceGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
			...
			Text = """
			       ...
			       - When `create-app-section` fails, read `error-class` before deciding what to do next: `transport` means the request never reached Creatio (retry is safe); `creatio-timeout` means Creatio produced no response within the budget and the section may still be created server-side — wait, run `list-app-sections`, and retry only if the section is still absent; `contention` means the insert was aborted without a detailed reason because sections were created in the same app at once — no section was created, so create sections one at a time (clio already serializes and auto-retries once) and, if it recurs, wait a few seconds, run `list-app-sections` to confirm the section is absent, then retry; `server-error` means Creatio rejected the operation with a real reason (fix inputs or server state first). The `section-created` field reports the verified side-effect state (`true`/`false`/`unknown`). Follow the returned `retry-guidance` and never blind-retry the same call.
