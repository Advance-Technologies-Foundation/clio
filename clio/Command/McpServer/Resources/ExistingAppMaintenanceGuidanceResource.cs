...
public sealed class ExistingAppMaintenanceGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
			...
			Text = """
			       ...
			       - `sync-schemas` `create-lookup`, `create-entity`, and `update-entity` are convergent: a schema that already exists in the target package is reconciled (only missing columns are added; columns you did not name stay untouched), never recreated. Read the per-operation `outcome` (`created` | `reconciled` | `already-satisfied` | `collision`): `reconciled` and `already-satisfied` are successes, not failures. `collision` means the op failed with `success: false` + `collision-info` — a same-name schema in a DIFFERENT package (unless it is a `create-entity` `extend-parent: true` replacement, which is `created`), or a same-package schema whose parent/kind is incompatible with the request; resolve the real conflict before retrying.
			       - `sync-schemas` retries transient network faults (DNS/reset/timeout/gateway) per operation and returns a `resume-plan` when a batch aborts mid-way. Because the schema ops are convergent, re-submitting the whole batch verbatim is safe for the SCHEMA operations only (they replay as `already-satisfied`/`reconciled`); when the batch includes `seed-data` or a `resume-plan` was returned, resubmit `resume-plan.operations` (failed op + not-run ops, already in re-submittable shape) instead, because `seed-data` is NOT replay-safe for rows without a `Name` and the plan converts a post-create seed failure to a standalone op.
