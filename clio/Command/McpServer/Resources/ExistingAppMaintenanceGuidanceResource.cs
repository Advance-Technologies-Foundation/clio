...
public sealed class ExistingAppMaintenanceGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
			...
			Text = """
			       ...
			       - When entity-schema-name is omitted, Creatio derives both the section code and the new entity code from the caption (e.g. caption "Customer Profile" → section code and entity code "{prefix}CustomerProfile"). Choose the caption that produces the desired {prefix}<PascalCase> entity name — where {prefix} is the active SchemaNamePrefix; read it via `get-schema-name-prefix` before the first naming decision.
