...
public sealed class AgentExecutionGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
			...
			Text = """
			       ...
			       - When executing a `reuse` decision and the wiring step fails (for example `create-app-section` fails with `Failed to create section ... bound to entity ... is already bound to an existing section`), do not create a substitute entity that duplicates the reused schema's fields. The reused entity already exists and already backs its own section. Report it as available with its capabilities and let the user decide whether to use it as-is, reuse the existing section, or switch to a new entity with separate data storage. Do not blindly retry `create-app-section` with the same `entity-schema-name`.
