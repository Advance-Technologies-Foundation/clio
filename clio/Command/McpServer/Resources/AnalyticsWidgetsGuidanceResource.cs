...
/// guidance and are not duplicated here. Guidance text describes surfaces generically and never names
/// specific shipped dashboards/views.
/// </remarks>
...
public sealed class AnalyticsWidgetsGuidanceResource {
	...
	internal static readonly TextResourceContents PlacementContexts = CreateReference(
		...
		A standalone analytics screen the user opens from the Dashboards section (the user picks a view from a list of available dashboards). This is the surface the dashboards guidance describes in full: metric band on top, chart grid below, sections for multi-topic, plain white throughout.
		...
		A section list page has an analytics view toggled from the list/chart switch in the top-right, plus a Summaries panel. It follows the same skeleton as a dashboard:

		- A top metric band of section-scoped counts/sums (e.g. a record count, an open-item count, a total value, a gap/exception count) — typically 4-up at 3 cols each.
		- A chart grid below: doughnut breakdowns, bar charts, and a wide bar/category chart at the bottom.
		- Plain white cards; metric values default-colored (a warning color is acceptable for gap/overdue counts).

		What differs from a standalone dashboard:

		- The analytics are scoped to the section's records and respect the active folders/filters/tags chosen on the list (e.g. a list filtered to a specific owner). Design metrics/charts to read correctly under filtering.
		...
		Widgets embedded on a record/form page (e.g. a metrics/analytics strip on a record form, or an actions/dashboard-style area).

		- Widgets here are contextual to the open record (this record's related items and metrics).
