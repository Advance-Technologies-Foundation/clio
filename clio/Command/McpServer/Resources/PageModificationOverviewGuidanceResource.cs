...
public sealed class PageModificationOverviewGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
		...
		Text = """
		       ...
		       - Append does NOT support a body in the full `viewModelConfig`/`modelConfig` form (web markers `SCHEMA_VIEW_MODEL_CONFIG`/`SCHEMA_MODEL_CONFIG`, mobile keys `viewModelConfig`/`modelConfig`) instead of the `*Diff` form; use replace mode for such bodies. clio now rejects a full-config INCOMING body up-front (before any server round-trip) with an actionable hint, and still refuses when the CURRENT server body is full-config — in both cases switch to `mode: "replace"`.
		       ...
		       body payload type
		       - `body` is a STRING: the JavaScript schema source text with its `/**MARKER*/` pairs. Pass it as a raw string, NOT a JSON object/dict — a structured object is rejected by the tool contract ("Parameter 'body' must be a string, not dict"). For large bodies use `body-file` (a path) to avoid inline JSON escaping.
		       ...
		       - UPDATING an existing predefined filter: do NOT append a second `["attributes"]` merge — plain-concat has NO dedupe, so it STACKS a duplicate op instead of replacing. Instead `get-page`, edit the existing `Items_PredefinedFilter` op in place, and save with `mode:"replace"` — the sanctioned exception to the "never send the whole body" fast rule (append can't replace an op; cf. static-form bodies in `page-modification-field-contract`).
		       - Do NOT reverse-engineer a `SysFolder` row with a `FilterData` blob and do NOT INSERT it through DataService — `FilterData` is a `System.IO.Stream`, so a DataService InsertQuery fails with `Object reference not set to an instance of an object` or `Attempt to set the value of "System.String" type into the "FilterData" field of the "System.IO.Stream" type`. A `SysFolder` folder-tree entry (left-panel folders) is USER data — a DIFFERENT concept; do not conflate them or hunt through `SysSchema` / `FolderTree` / `BaseFolder`.
