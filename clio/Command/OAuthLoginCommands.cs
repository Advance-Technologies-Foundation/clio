...
/// <summary>Shared options for interactive OAuth commands.</summary>
public abstract class OAuthEnvironmentOptions : EnvironmentNameOptions { }

/// <summary>Options for <c>login</c>.</summary>
[Verb("login", Aliases = ["signin"], HelpText = "Sign in to an SSO-enabled Creatio environment")]
public sealed class LoginOptions : OAuthEnvironmentOptions
{
...
public sealed class LoginCommand : Command<LoginOptions>
{
    ...
    public override int Execute(LoginOptions options) { try { EnvironmentSettings environment = _settings.GetEnvironment(options); if (!options.Force && _store.TryRead(environment, out OAuthTokenSet cached)) { if (cached.ExpiresAt > DateTimeOffset.UtcNow.AddSeconds(60)) { _logger.WriteInfo("A usable OAuth session is already cached."); return 0; } try { _service.ResolveAsync(environment).GetAwaiter().GetResult(); _logger.WriteInfo("A usable OAuth session is already cached."); return 0; } catch (InvalidOperationException) { } } OAuthTokenSet token = _service.LoginAsync(environment, options.NoBrowser, options.Timeout, default).GetAwaiter().GetResult(); _logger.WriteInfo(token.Identity is null ? "Login successful." : $"Signed in as {token.Identity}."); return 0; } catch (Exception e) { _logger.WriteError(e.Message); return 1; } }
}

/// <summary>Options for <c>logout</c>.</summary>
[Verb("logout", Aliases = ["signout"], HelpText = "Revoke the cached SSO session")]
public sealed class LogoutOptions : OAuthEnvironmentOptions { }
...
public sealed class AuthStatusOptions : OAuthEnvironmentOptions { }
...
public sealed class AuthStatusCommand : Command<AuthStatusOptions>
{
    ...
    public override int Execute(AuthStatusOptions options) { try { EnvironmentSettings e = _settings.GetEnvironment(options); if (e.AuthFlow != OAuthFlow.AuthorizationCode) { _logger.WriteInfo("Authentication flow: client-credentials"); return 0; } if (!_store.TryRead(e, out OAuthTokenSet token)) { _logger.WriteError("No cached OAuth session. Run: clio login -e " + options.Environment); return 1; } bool usable = token.ExpiresAt > DateTimeOffset.UtcNow; _logger.WriteInfo($"Authentication flow: authorization-code; cached token: {(usable ? "yes" : "expired")}; access-token expiry: {token.ExpiresAt:O}"); return usable ? 0 : 1; } catch (Exception ex) { _logger.WriteError(ex.Message); return 1; } }
}
