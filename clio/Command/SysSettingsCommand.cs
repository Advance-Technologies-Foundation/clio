...
[Verb("set-syssetting", Aliases = new[] { "syssetting", "sys-setting", "get-syssetting" },
    ...
    : Command<SysSettingsOptions>
{
    ...
    public override int Execute(SysSettingsOptions opts)
    {
        if (opts.IsGet)
        {
            ...
            {
                _logger.WriteError(
                    $"To view SysSetting value by code requires cliogate package version {minClioGateVersion} or higher installed in Creatio.");
