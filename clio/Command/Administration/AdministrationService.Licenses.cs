...
public sealed partial class AdministrationService {
	private static readonly string[] RoleLicenseColumns = ["Id", SysRoleColumn, "SysLicPackage"];
	...
	public JsonElement SetUserLicense(Guid userId, Guid packageId, bool remove) {
		RequireId(packageId);
		JsonElement user = GetUser(userId);
		if (!remove && !user.GetProperty(ActiveColumn).GetBoolean()) { throw new ArgumentException("Activate the user before assigning a license."); }
	...
	public JsonElement GetRoleLicenses(Guid roleId, int offset, int limit) {
		GetRole(roleId);
		return client.Select(SysLicPackageInRoleSchema, RoleLicenseColumns,
	...
	public JsonElement SetRoleLicense(Guid roleId, Guid packageId, bool remove) {
		...
		Dictionary<string, object> filter = new() { [SysRoleColumn] = roleId, ["SysLicPackage"] = packageId };
		JsonElement rows = client.Select(SysLicPackageInRoleSchema, RoleLicenseColumns, filter, limit: 2);
		...
		if (remove && rows.GetArrayLength() == 1) {
			client.DeleteEntity(SysLicPackageInRoleSchema, rows[0].GetProperty("Id").GetGuid());
		} else if (!remove && rows.GetArrayLength() == 0) {
			client.WriteEntity(SysLicPackageInRoleSchema, Guid.NewGuid(), filter, true);
		}
		rows = client.Select(SysLicPackageInRoleSchema, RoleLicenseColumns, filter, limit: 2);
