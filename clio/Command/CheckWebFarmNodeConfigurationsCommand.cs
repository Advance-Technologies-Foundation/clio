    ...
    IDirectoryComparer directoryComparer) : Command<CheckWebFarmNodeConfigurationsOptions>
{
    ...
    public override int Execute(CheckWebFarmNodeConfigurationsOptions options)
    {
        ...
        List<string> paths = options.Paths.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries)
            .Select(p => p.Trim())
            .ToList();
        if (paths.Count < 2)
        {
        ...
        for (int i = 1; i < paths.Count; i++)
        {
            ...
            List<string> differences = _directoryComparer.CompareDirectories(basePath, comparePath);
...
public class DirectoryComparer(IFileSystem fileSystem, ILogger logger) : IDirectoryComparer
{
    ...
    public List<string> CompareDirectories(string path1, string path2)
    {
        ...
        IEnumerable<string> missingDirsInPath2 = dirs1.Except(dirs2).Select(d => $"Folder missing in {path2}: {d}");
        IEnumerable<string> missingDirsInPath1 = dirs2.Except(dirs1).Select(d => $"Folder missing in {path1}: {d}");
        IEnumerable<string> missingFilesInPath2 = files1.Except(files2)
            .Select(f => { return $"File missing in {path2}: {f}"; });
        IEnumerable<string> missingFilesInPath1 = files2.Except(files1)
            .Select(f => { return $"File missing in {path1}: {f}"; });
        List<string> allDifferences = missingDirsInPath2.Concat(missingDirsInPath1)
    ...
    {
        IFileInfo file1Info = _fileSystem.GetFilesInfos(v1);
        IFileInfo file2Info = _fileSystem.GetFilesInfos(v2);
    ...
    private void ProcessPath(string rootPath, ConcurrentBag<string> dirs, ConcurrentBag<string> files)
    {
        ...
        string[] topDirs = _fileSystem.GetDirectories(rootPath, "*", SearchOption.TopDirectoryOnly);
        ...
        string[] topFiles = _fileSystem.GetFiles(rootPath, "*", SearchOption.TopDirectoryOnly);
        ...
        Parallel.ForEach(topDirs, subDir => { ProcessPath(subDir, dirs, files); });
