...
internal class CloneEnvironmentCommand : BaseDataContextCommand<CloneEnvironmentOptions>
{
    ...
    public override int Execute(CloneEnvironmentOptions options)
    {
        ...
        string[] selectedMaintainers = string.IsNullOrWhiteSpace(options.Maintainer) ? null
            : options.Maintainer.Split(',', StringSplitOptions.TrimEntries);
        string[] excludedMaintainers = string.IsNullOrWhiteSpace(options.ExcludeMaintainer) ? null
        ...
        try
        {
            ...
            IEnumerable<CreatioManifestPackage> diffPackages = diffManifest
                                                               .Packages.Where(p =>
                                                                   selectedMaintainers == null ? true
                                                                       : selectedMaintainers.Contains(p.Maintainer))
                                                               .Where(p => excludedMaintainers == null ? true
                                                                   : !excludedMaintainers.Contains(p.Maintainer));
            ...
            PushPkgOptions pushPackageOptions = new()
            {
                Environment = options.Target, Name = commonPackagesZipPath
            };
            pushPackageCommand.Execute(pushPackageOptions);
            PingAppOptions pingCommandOptions = new()
            {
