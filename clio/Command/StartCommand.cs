...
[Verb("start", Aliases = ["start-server", "start-creatio", "sc"], HelpText = "Start local Creatio application")]
	...
	private async Task<int> ExecuteAsync(StartOptions options) {
		...
		try {
			...
			ProgressNotificationValue progressStep1 = new() {
				...
				Total = totalSteps,
				Message = "Obtained environment(s) to start",
			};
			...
			if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
			{
				...
				if (iisSites.Count > 0) {
					int r =  await StartIISSite(envName, iisSites[0]);
					ProgressNotificationValue progressStep2 = new() {
						Progress = ++stepsCompleted,
						Total = totalSteps,
						Message = r == 0 ? "Started IIS site successfully" : "Failed to start IIS site",
					};
			...
			if (options.Terminal) {
				...
				int p =  await PingSite(envName);
				ProgressNotificationValue progressStep2 = new() {
					Progress = ++stepsCompleted,
					Total = totalSteps,
					Message = p == 0 ? "Creatio in Terminal started successfully" : "Failed to start Creatio in Terminal",
				};
				OnStatusChanged(progressStep2);
				return p;
			} else {
				...
				ProgressNotificationValue progressStep2 = new() {
					Progress = ++stepsCompleted,
					Total = totalSteps,
					Message = p == 0 ? "Background service started successfully" : "Failed to start background service",
				};
				OnStatusChanged(progressStep2);
				return p;
			}
