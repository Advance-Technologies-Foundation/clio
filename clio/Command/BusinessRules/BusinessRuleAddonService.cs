...
internal interface IBusinessRuleAddonService {
	...
	/// <summary>
	/// Reads every business rule persisted on the target add-on schema (full hierarchy) and maps
	/// each to the friendly rule contract where possible. Autogenerated apply-filter child rules
	/// are folded into their parent and never returned separately.
	/// </summary>
	/// <param name="request">Identifies the target add-on schema to fetch.</param>
	/// <returns>One read model per parent rule, in persisted order.</returns>
	IReadOnlyList<BusinessRuleReadModel> ReadRules(AddonGetRequestDto request);

	/// <summary>
	/// Replaces existing rules matched by name with freshly converted metadata, preserving stable
	/// identity (rule/case/group/trigger uIds) through <see cref="BusinessRuleIdentityGrafter"/>.
	/// The whole batch persists with a single <c>SaveSchema</c> + cache reset + configuration
	/// rebuild; per-item match failures are isolated and reported.
	/// </summary>
	/// <param name="request">Identifies the target add-on schema to fetch and save once.</param>
	/// <param name="items">Per-rule replacement payloads, matched by rule name.</param>
	/// <returns>Per-item outcomes, one entry per input item, in input order.</returns>
	IReadOnlyList<BusinessRuleBatchItemResult> UpdateRules(
		AddonGetRequestDto request,
		IReadOnlyList<BusinessRuleUpdateItem> items);

	/// <summary>
	/// Deletes rules matched by name, cascading to their autogenerated child rules and caption
	/// resources. Saves once when at least one rule matched; unknown names are reported as
	/// per-item failures without aborting the rest.
	/// </summary>
	/// <param name="request">Identifies the target add-on schema to fetch and save once.</param>
	/// <param name="ruleNames">Internal rule names to delete.</param>
...
/// <summary>
/// One rule replacement inside a batch update.
/// </summary>
/// <param name="Name">The internal rule name used to match the existing rule.</param>
/// <param name="Enabled">Caller intent for the enabled flag; <c>null</c> preserves the existing value.</param>
/// <param name="GeneratedRules">Freshly converted replacement metadata (parent rule first, then
/// any regenerated apply-filter child rules).</param>
...
internal sealed class BusinessRuleAddonService(
	...
	public IReadOnlyList<BusinessRuleReadModel> ReadRules(AddonGetRequestDto request) {
	...
	/// <summary>
	/// Persists the mutated metadata/resources once and stamps every pending item's outcome; a
	/// save failure marks every pending item failed with the same error (single-save atomicity).
	/// Nothing is saved when no item survived matching.
	/// </summary>
	...
	/// <summary>
	/// Rejects caller-supplied rule names that collide with a rule already persisted on the schema
	/// (or repeated inside the batch). Generated names are random and practically collision-free;
	/// this guard exists for explicit names, where a silent duplicate would break the name-based
	...
	private static int FindRuleIndexByName(JsonArray rules, string ruleName) {
		for (int index = 0; index < rules.Count; index++) {
			...
			// Autogenerated apply-filter children are managed through their parent only.
