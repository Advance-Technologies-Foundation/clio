...
/// <summary>
/// One persisted business rule as returned by the read tools. When the friendly contract can
/// represent the rule, <see cref="Rule"/> carries it (including block <c>uId</c>s ready to be
/// passed back to update); otherwise <see cref="Raw"/> carries the untouched add-on metadata so
/// nothing that exists on the schema is hidden from the caller.
/// </summary>
public sealed record BusinessRuleReadModel {

	/// <summary>Gets the internal unique rule name — the match key for update and delete.</summary>
	[JsonPropertyName("name")]
	[Description("Internal unique rule name — the match key for update and delete.")]
	public string Name { get; init; } = string.Empty;

	/// <summary>Gets the rule caption shown to No-code developers.</summary>
	[JsonPropertyName("caption")]
	[JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]
	[Description("Rule caption shown to No-code developers.")]
	public string? Caption { get; init; }

	/// <summary>Gets whether the rule is active.</summary>
	[JsonPropertyName("enabled")]
	[Description("Whether the rule is active.")]
	public bool Enabled { get; init; }

	/// <summary>Gets whether the rule could be mapped to the friendly rule contract.</summary>
	[JsonPropertyName("convertible")]
	[Description("Whether the rule could be mapped to the friendly rule contract. When false, 'raw' carries the persisted metadata instead.")]
	public bool Convertible { get; init; }

	/// <summary>Gets the friendly rule definition when <see cref="Convertible"/> is <c>true</c>.</summary>
	[JsonPropertyName("rule")]
	[JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]
	[Description("Friendly rule definition (create/update contract shape, including block uIds) when convertible.")]
	public BusinessRule? Rule { get; init; }

	/// <summary>Gets the raw persisted add-on metadata when the rule is not convertible.</summary>
	[JsonPropertyName("raw")]
	[JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]
	[Description("Raw persisted add-on metadata when the rule is not convertible.")]
	public JsonNode? Raw { get; init; }

	/// <summary>Gets the reason the rule could not be mapped to the friendly contract.</summary>
	[JsonPropertyName("conversionError")]
	[JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]
	[Description("Reason the rule could not be mapped to the friendly contract; present only when convertible is false.")]
	public string? ConversionError { get; init; }
}

/// <summary>
/// Converts persisted business-rule add-on metadata back into the friendly rule contract — the
/// reverse of <see cref="BusinessRuleMetadataConverter"/>. Autogenerated child rules
/// (<c>parentUId</c> set — the clear/populate helpers of apply-filter) are folded into the parent
/// action's flags and never surfaced as separate rules.
/// </summary>
internal static class BusinessRuleMetadataReader {
	...
	internal static IReadOnlyList<BusinessRuleReadModel> Read(
		JsonArray rules,
		...
		List<BusinessRuleReadModel> models = [];
	...
	private static BusinessRuleReadModel ReadRule(
		...
		try {
			BusinessRule rule = ConvertRule(ruleObject, caption, name, enabled, childRules);
			return new BusinessRuleReadModel {
				Name = name,
				Caption = caption,
				Enabled = enabled,
				Convertible = true,
			...
			// Anything the friendly contract cannot represent (multi-case rules, unknown action or
			// expression shapes, designer-authored constructs) is surfaced raw instead of hidden.
			return new BusinessRuleReadModel {
				Name = name,
				Caption = caption,
				Enabled = enabled,
				Convertible = false,
				Raw = ruleObject.DeepClone(),
	...
	private static BusinessRule ConvertRule(
		...
		if (ruleObject["cases"] is not JsonArray cases || cases.Count != 1 || cases[0] is not JsonObject caseObject) {
			throw new InvalidOperationException("Only single-case business rules are convertible.");
		}
	...
	private static BusinessRuleConditionGroup ConvertConditionGroup(JsonNode? conditionNode) {
		...
		if (string.Equals(typeName, BusinessRuleConditionTypeName, StringComparison.Ordinal)) {
			// A bare single condition (no surrounding group) is representable as a one-condition
			// AND group; update rewrites it as a group, which the platform accepts.
		...
		if (conditionObject["conditions"] is JsonArray conditionNodes) {
			foreach (JsonNode? nested in conditionNodes) {
				...
				if (!string.Equals(nestedTypeName, BusinessRuleConditionTypeName, StringComparison.Ordinal)) {
					// Nested groups are not representable in the friendly single-group contract.
	...
	private static BusinessRuleCondition ConvertCondition(JsonObject conditionObject) {
		// The platform strips zero-valued properties when persisting addon metadata (the same
		// normalization that drops the friendly `type` marker), so an absent comparisonType means
		// the enum default 0 = is-not-filled-in, not an unsupported shape.
	...
	private static BusinessRuleExpression ConvertExpression(JsonObject expressionObject) {
		// The platform omits the friendly `type` marker on read-back for some expression shapes
		// (a saved Const comes back with only typeName/dataValueTypeName/value), so dispatch on
		// the persisted typeName first and use the friendly `type` only as a fallback.
		...
		if (string.Equals(typeName, BusinessRuleFormulaExpressionTypeName, StringComparison.Ordinal)
			...
			// Best-effort: the persisted expression-schema text, not the original caller input.
	...
	private static ApplyFilterBusinessRuleAction ConvertApplyFilterAction(
		...
		// The platform re-serializes addon metadata through its server-side model classes, which do
		// not carry the clearValue/populateValue designer flags — they are dropped on save. The
		// behavior itself lives in the autogenerated child rules, so derive each flag from either
		// the (possibly surviving) flag or the presence of the corresponding child rule.
	...
	/// <summary>
	/// The persisted <c>filterExpression</c> uses the literal string <c>"null"</c> for "no relative
	/// path" (see the converter's <c>BuildFilterLookupExpression</c>); map it back to absent.
	/// </summary>
