...
public static class ApprovalBlockExpectation {
	...
	/// like — the nested approver binds normally and, on a CrtProcessBuilder before 1.6.6.30, the leftover flat
	/// member is discarded unread: no write contract there implements <c>IExtensibleDataObject</c>, and
	/// <c>DataContractJsonSerializer</c> drops an unknown member silently. From 1.6.6.30 the server refuses the
	/// whole request for that member instead, naming it as a read-only describe field (ENG-95244).</para>
	...
	/// marker that fired in ordinary operation - until 1.6.6.30, where such a block no longer saves at all. It still
