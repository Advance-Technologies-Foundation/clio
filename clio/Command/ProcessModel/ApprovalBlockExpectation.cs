...
public static class ApprovalBlockExpectation {
	...
	/// like — the nested approver binds normally and, on a CrtProcessBuilder before 1.6.6.29, the leftover flat
	/// member is discarded unread: no write contract there implements <c>IExtensibleDataObject</c>, and
	/// <c>DataContractJsonSerializer</c> drops an unknown member silently. From 1.6.6.29 the server refuses the
	...
	/// marker that fired in ordinary operation - until 1.6.6.29, where such a block no longer saves at all. It still
