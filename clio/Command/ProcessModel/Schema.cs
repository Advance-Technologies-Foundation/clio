...
public static class DataValueTypeMap{
	...
	public static Guid FromRuntimeValueType(int runtimeDataValueType) => runtimeDataValueType switch {
		...
		37 => Float1DataValueTypeUId,
		38 => Float2DataValueTypeUId,
		39 => Float3DataValueTypeUId,
		40 => Float4DataValueTypeUId,
		41 => Float8DataValueTypeUId,
		42 => PhoneTextDataValueTypeUId,
		44 => WebTextDataValueTypeUId,
		45 => EmailTextDataValueTypeUId,
		46 => CompositeObjectListDataValueTypeUId,
...
public static class ManagerMap{
	...
	public static EventType ResolveDataId(string dataId) {
		...
		return id switch {
			...
			// is already covered by the "usertask"-suffix arm below. "approval" is the same case for the Approval
			// element (ApprovalUserTask): the token does NOT end in "usertask", so without an explicit arm here it
			// falls through to Unknown and validate-process-graph rejects a perfectly valid graph.
			"usertask" or "performtask" or "sendemail" or "approval" => EventType.UserTask,
