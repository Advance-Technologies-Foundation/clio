...
[Verb("call-service", Aliases = new[]
{
    "cs"
}, HelpText = "Call Service Request")]
...
[Verb("dataservice", Aliases = new[]
{
    "ds"
}, HelpText = "DataService Request")]
    ...
    public override int Execute(T options)
    {
        ...
        else
        {
            string requestData = string.IsNullOrWhiteSpace(options.RequestBody)
                ? GetRequestData(options.RequestFileName) : options.RequestBody;
