...
public class Feature : BaseModel
{
    ...
    [DetailProperty("FeatureId")] public virtual List<UserFeatureState> UsersFeatureState { get; set; }
}
...
public class UserFeatureState : BaseModel
{
    ...
    [SchemaProperty("FeatureState")] public int FeatureState { get; set; }
}
...
public class FeatureRepository
{
    ...
    public FeatureRepository(UserConnection userConnection) =>
        Repository = new Repository { UserConnection = UserConnection = userConnection };
    ...
    private Guid GetFeatureByCode(string code)
    {
        ...
        if (entity.FetchFromDB(new Dictionary<string, object> { { "Code", code } }))
        {
    ...
    private Guid ExtractFeatureId(string code)
    {
        ...
        Guid featureId = GetFeatureByCode(code);
        return featureId;
    }

    private UserFeatureState CreateNewFeatureState(Guid userId, Guid featureId, int state)
    {
        UserFeatureState? userFeatureState = Repository.CreateItem<UserFeatureState>();
    ...
    public void SetFeatureState(string code, int state, Guid userId)
    {
        ...
        {
            Feature? feature = Repository.GetItem<Feature>(featureId);
