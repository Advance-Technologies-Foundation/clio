...
public class ATFLogService : BaseService //, System.Web.SessionState.IReadOnlySessionState
{
    ...
    private static readonly Timer _resetTimer = new(_ => Reset(ResetReason.Timer));
    ...
    {
        Guid prevChannelId = AsyncLoggerListener.CurrentChannelId;
        AsyncLoggerListener.StopListening();
        if (prevChannelId != Guid.Empty)
        {
            SendMessage(new LogMessage(
                $"Listening stopped. Caused by reason = {reason}", "Telemetry"), prevChannelId);
        }
    }

    private static void SendMessage(LogMessage message, Guid sysAdminUnitId)
    {
        List<LogMessage> logPortion = new() { { message } };
        TelemetryMessage telemetryMessage = new() { LogPortion = logPortion };
        ...
        SimpleMessage simpleMessage = new() { Id = Guid.NewGuid(), Body = messageBody };
    ...
    {
        CheckOperationRights();
        IEnumerable<string>? rulePatterns = AsyncLoggerListener.GetRulePatterns();
...
{
    [DataMember(Name = "logPortion")] public List<LogMessage> LogPortion { get; set; }

    [DataMember(Name = "cpu")] public int Cpu { get; set; }

    [DataMember(Name = "ramMb")] public int RamMb { get; set; }
}
