...
public sealed class PageValidateToolE2ETests : McpContractFixtureBase {
	...
	[Test]
	[Description("ENG-95429 symptom: a well-formed crt.Button insert into Scaffold/actions passes validation (valid=true) but comes back with the placement warning. This is the body a coding agent actually produced and that persisted successfully while never rendering — the type-placement rule correctly stays silent on it, so the slot advisory is the only thing that can steer the author.")]
	[AllureTag(ToolName)]
	...
	public async Task PageValidateTool_Should_Warn_About_Mobile_Button_In_Scaffold_Actions() {
		...
		// Assert
		response.Valid.Should().BeTrue(
			because: "the placement stays advisory while it is unresolved whether the slot is a guidance defect or a rendering one");
		response.Validation.Should().NotBeNull(
			because: "validation details are always included in the response");
		response.Validation!.Warnings.Should().NotBeNullOrEmpty(
