...
public sealed class MobilePageConversionGuideSandboxE2ETests : McpContractFixtureBase {
	...
	[Test]
	[Description("ENG-94188: converts a real seeded TABBED page through the real clio MCP server and verifies the mandatory tabAreaLayers contract end to end — the guide carries a tabAreaLayers group per converted tab, the two synthesized layers (tab body grid + Area card) sit in the element map right after their tab with no webName, the moved children are reparented onto the Area, and the constraints/nextSteps carry the MANDATORY (never offer to skip) wording.")]
	[AllureTag(ToolName)]
	[AllureName("get-mobile-page-conversion-guide returns the mandatory tabAreaLayers contract for a tabbed page")]
	[AllureDescription("Starts the real clio MCP server, resolves the seeded installed application AutoTestClioMcp, converts its pages until one yields tabAreaLayers (a tabbed record page), and asserts the full serialized contract: group names match synthesized element-map inserts placed at tab+1/tab+2 with no webName, movedChildren point at their own tab's Area, and the MANDATORY constraint/next-step text is present — exercising the bundled WebToMobilePageConversionRules.json tabAreaLayers section and the BuildTabAreaLayers pass through the real MCP surface.")]
	public async Task MobilePageConversionGuideTool_Should_Return_Mandatory_TabAreaLayers_For_Tabbed_Page() {
		...
		if (guide is null) {
			...
			Assert.Ignore(
				...
				+ "exercise the ENG-94188 surface.");
		}

		// Assert — the serialized guide honors the mandatory two-layer contract for EVERY converted tab.
