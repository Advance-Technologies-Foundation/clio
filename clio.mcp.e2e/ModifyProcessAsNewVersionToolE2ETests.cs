...
/// versions of it, gated on a reachable environment carrying CrtProcessBuilder 1.6.6.13 or newer and a
	...
	private static async Task<ArrangeContext> ArrangeAsync(bool requireReachableEnvironment) {
		...
		if (requireReachableEnvironment) {
			if (string.IsNullOrWhiteSpace(environmentName)) {
				Assert.Ignore("Configure McpE2E:Sandbox:EnvironmentName (carrying CrtProcessBuilder 1.6.6.13 or "
