...
internal static class GetProcessSignatureResultParser {
	public static GetProcessSignatureEnvelope Extract(CallToolResult callResult) {
		...
		throw new InvalidOperationException(McpResultDiagnostics.DescribePrefixed("Could not parse get-process-signature MCP result: ", callResult, diagnostics));
	}
	...
	private static bool TryDeserialize(JsonElement element, out GetProcessSignatureEnvelope? envelope, McpParseDiagnostics diagnostics) {
		...
		bool isMeaningfulJsonCandidate = diagnostics.RecordDeserializeAttempt(element, typeof(GetProcessSignatureEnvelope));
		try {
