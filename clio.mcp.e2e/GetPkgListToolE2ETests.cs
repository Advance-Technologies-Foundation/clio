...
/// <summary>
/// End-to-end tests for the get-pkg-list MCP tool.
/// </summary>
[TestFixture]
[AllureNUnit]
[AllureFeature("get-pkg-list")]
[NonParallelizable]
public sealed class GetPkgListToolE2ETests {
	private const string ToolName = GetPkgListTool.GetPkgListToolName;

	[Test]
	[Description("Starts the real clio MCP server, invokes get-pkg-list for the configured sandbox environment, and verifies that a structured non-empty package list is returned.")]
	[AllureTag(ToolName)]
	[AllureName("Get package list returns structured sandbox packages")]
	[AllureDescription("Uses the real clio MCP server to call get-pkg-list against the configured sandbox environment and verifies the returned structured package list contains at least one item with non-empty name, version, and maintainer fields.")]
	public async Task GetPkgList_Should_Return_Structured_Packages() {
		// Arrange
		McpE2ESettings settings = TestConfiguration.Load();
		settings.ClioProcessPath = TestConfiguration.ResolveFreshClioProcessPath();
		if (!settings.AllowDestructiveMcpTests) {
			Assert.Ignore("Set McpE2E:AllowDestructiveMcpTests=true to install cliogate and run get-pkg-list end-to-end tests.");
		}
	...
	[AllureStep("Arrange get-pkg-list MCP session")]
	...
	private static void AssertToolCallSucceeded(GetPkgListActResult actResult) {
		actResult.CallResult.IsError.Should().NotBeTrue(
			because: "get-pkg-list should return a structured MCP payload for a valid sandbox environment");
	}
	...
	private static void AssertStructuredPackagesReturned(GetPkgListActResult actResult) {
		actResult.Packages.Should().NotBeEmpty(
			because: "the sandbox environment should expose at least one package through get-pkg-list");
