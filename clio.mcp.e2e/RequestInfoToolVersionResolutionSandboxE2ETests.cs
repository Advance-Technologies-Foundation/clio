...
public sealed class RequestInfoToolVersionResolutionSandboxE2ETests {
	...
	public async Task RequestInfoTool_Should_ResolveFromEnvironment_WhenScopedToReachableStand() {
		...
		response.ResolvedFrom.Should().BeOneOf(
			...
			because: "ENG-96840: a reachable stand must resolve from the environment tier (the version probe succeeded) — either 'environment' (exact per-version catalog on the CDN) or 'environment-superset' (version known from the stand, latest served as the closest catalog). The pre-fix disposal race degraded resolution to 'latest-fallback' with reason 'probe-error' (source != Environment); the offline NoEnvironment lane can only assert the degraded tier");
