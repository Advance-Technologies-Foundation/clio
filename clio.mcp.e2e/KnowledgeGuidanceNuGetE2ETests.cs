...
public sealed class KnowledgeGuidanceNuGetE2ETests : McpContractFixtureBase {
	...
	public async Task PersistentCache_ShouldHotReloadInSameMcpProcess_AfterCliUpdate() {
		...
		// A gated topic's URI and an identifier nobody publishes must be answered the same way; the read is
		// the only surface where a client could otherwise probe for hidden content.
		string gatedReadError = await CaptureReadError(context, _fixture.GatedGuideUri);
		string unknownUri =
			$"{KnowledgeResolver.NamespacedUriPrefix}{SyntheticKnowledgeNuGetFixture.LibraryId}/synthetic-never-published";
		string unknownReadError = await CaptureReadError(context, unknownUri);
		...
		discoveredResources.Should().NotContain(resource => resource.Uri == _fixture.GatedGuideUri,
			because: "a topic whose required feature is disabled must stay out of the advertised listing");
		gatedReadError.Should().Contain(KnowledgeGuidanceNotFoundException.ErrorCode,
			because: "reading a gated topic must be refused as an unknown resource rather than surfacing the "
				+ "generic internal error a plain server-side exception collapses into");
		gatedReadError.Replace(_fixture.GatedGuideUri, unknownUri, StringComparison.Ordinal).Should()
			.Be(unknownReadError,
				because: "beyond the identifier it echoes, a gated read and an unpublished read must be "
					+ "word-for-word identical, or the error text becomes an existence oracle for hidden topics");
	...
	// result, so the message is the only thing an assertion can compare.
	private static async Task<string> CaptureReadError(ArrangeContext context, string uri) {
		try {
			await context.Session.ReadResourceAsync(uri, context.CancellationTokenSource.Token);
		} catch (Exception exception) {
