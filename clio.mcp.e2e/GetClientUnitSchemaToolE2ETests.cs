...
public sealed class GetClientUnitSchemaToolE2ETests : McpContractFixtureBase {
	...
	[AllureTag(ToolName)]
	[AllureName("get-client-unit-schema rejects an out-of-bounds output-file")]
	[AllureDescription("Invokes get-client-unit-schema through the real MCP server with an output-file that traverses out of the OS temp directory and verifies the command fails before writing, so an agent-supplied path cannot overwrite an arbitrary file.")]
	public async Task GetSchema_Should_Reject_OutputFile_Outside_AllowedZones() {
		// Arrange
		McpE2ESettings settings = TestConfiguration.Load();
		settings.ClioProcessPath = TestConfiguration.ResolveFreshClioProcessPath();
		await using ArrangeContext arrangeContext = await ArrangeAsync(settings, TimeSpan.FromMinutes(3));
		string escapingPath = Path.Combine(
			Path.GetTempPath(), "..", "..", "..", "clio-e2e-escape", $"body-{Guid.NewGuid():N}.js");
		...
		CallToolResult callResult = await arrangeContext.Session.CallToolAsync(
			...
			arrangeContext.CancellationTokenSource.Token);
		GetClientUnitSchemaResponse response =
			EntitySchemaStructuredResultParser.Extract<GetClientUnitSchemaResponse>(callResult);
