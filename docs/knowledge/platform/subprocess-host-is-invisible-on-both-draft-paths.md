...
a candidate, whether it is saved or new.
...
which registers the item nowhere (`SchemaManager.cs:4193-4202`). While the edit runs, `FindItemByName`
and `FindItemByUId` both miss the host. The version clone's `ParentSchemaUId` already names the family
root before the edit pipeline runs.
...
calls itself once it is activated. A unit test that registers the host and then runs Create mode
arranges a state production never has, and stays green through both.
