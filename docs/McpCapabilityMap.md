...
- `modify-business-process-as-new-version` (`ReadOnly=false`, `Destructive=false`, `Idempotent=false`, `OpenWorld=false`, **environment-sensitive**) — applies the SAME operations array as `modify-business-process` to a CLONE of the source (by `process-name` or `process-uid`) and saves that clone as a new version of it. What the product's designer calls “Save new version” (Ctrl+Alt+N), against `modify-business-process`'s “Save current version”. `Destructive=false` is literal: the source version is never opened, saved or activated, and what the environment EXECUTES is unchanged when the call returns. Optional `package-name` chooses the target package — omitted, the version goes to the SOURCE's package, always: there is no design-package fallback, so a source in a package that refuses edits is refused rather than redirected. A version does NOT inherit the root's package, and cross-package families are normal. An omitted or empty `operations` array is legal and takes a plain snapshot of the source, which is how an agent takes a restore point before editing in place. Reports the created version's schema UId, the name the PLATFORM composed (root + package + number — not predictable or choosable), the number the platform allocated, `isActiveVersion:false`, the FLAT family root UId (a version of a version still points at the root) and the applied-operation count. The number and the flag are reported only when the environment reported them: an omitted number is ABSENT rather than 0 (0 means family ROOT here), and an omitted flag yields a neutral 'the environment did not report which version is actual' rather than the claim that the source still runs. Atomic: any failed operation aborts the call, and a failure before the save leaves nothing behind. Activation is a separate, explicit gesture (`set-active-business-process-version`) — the product itself asks before making a new version actual, so an agent must not activate on its own initiative. A version can never be DELETED: the platform has no such operation, so every version created is permanent. Requires CrtProcessBuilder 1.6.6.14 or newer — the operation first ships in 1.6.1.0, the floor moved to 1.6.2.1 with ENG-95986 and to 1.6.6.14 with ENG-99856, both because the operations vocabulary grew a member an older server silently discards while answering success — the Send email template members (`email.messageSource`/`template`/`templateEntity`) and then `subProcess.multiInstanceOptions` — and this route runs no read-back to notice; an older package is refused up front by the package gate rather than answering the route with a 404.
  - **Strict keys (ENG-95244), all three write tools above.** Before anything is sent, clio checks every key of the `create-business-process` descriptor and of each `operations[]` item against `clio/Command/ProcessModel/Schemas/process-builder-write-keys.schema.json` — a JSON Schema generated from the bundled CrtProcessBuilder's `[DataContract]` sources and pinned to them by `ProcessBuilderWriteKeySchemaTests`. An unknown or wrongly-cased key (which the server would drop in silence while reporting success) is REFUSED, naming its JSON path and the nearest valid key; on an environment whose CrtProcessBuilder is NEWER than the bundle it is a warning and the call proceeds. Case-sensitive, keys only (values are the server's to refuse).
