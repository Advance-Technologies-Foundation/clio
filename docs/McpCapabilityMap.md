...
- `get-fsm-mode`
- `set-fsm-mode`
- `compile-creatio` (full, one `package-name`, or one `process-name`: the package a business process is in, compiled by CrtProcessBuilder 1.6.6.32+ - the only non-full route that picks up a script task or process methods saved by create/modify-business-process on Creatio 10.x)
