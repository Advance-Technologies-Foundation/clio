...
- `process-add-element` (`ReadOnly=false`, `Destructive=false`, `Idempotent=false`, `OpenWorld=false`, **environment-sensitive**) — validates a planned `Start → Read data → End` graph, then drives the live Process Designer over CDP to append + configure a Read data element and SAVE, returning `{success, code, uId, caption}`. Args: `environment-name`, `element-type` (`read-data`), `read-object` (e.g. Contact), optional `process-id` / `process-caption` / `headed`. Creates a new process when `process-id` is omitted (non-destructive; supplying `process-id` modifies an existing process). Aborts before opening a browser on any validator error; never reports a false-positive save (requires the "Successfully saved" signal and no `.djs-validate-outline`). Requires a forms-auth environment + a local Chromium.
