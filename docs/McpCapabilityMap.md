...
- `install-dashboards-migrator` (`ReadOnly=false`, `Destructive=true`, `Idempotent=true`, `OpenWorld=false`, **environment-sensitive**) — installs (or updates) the bundled `CrtDashboardsMigratorApp` package (the "Dashboards migrator" app, which converts Classic UI 7.x dashboards to Freedom UI) into a registered environment (the package requires Creatio 8.3.1 or later; not checked up front). Not part of the process-designer family — listed here because it shares `install-process-builder`'s whole shape: bundled source-only archive, target-side configuration build and restart, the ungated `Ping` outcome check (liveness, not identity), the two backwards-move refusals with a CLI-only override, and the same in-progress note past the response deadline. No gated tool sends a caller here; the need comes from the user's task, and the follow-up is the migration the user runs from System Designer.
