...
`CrtProcessBuilder` (ProcessDesignService) package on the target environment — **with three exceptions,
all listed below**: `get-process-signature`, which reads the built-in DataService and needs neither;
`run-process`, which calls the built-in ProcessEngineService and needs neither; and
`install-process-builder`, which is neither gated nor a consumer of the package, because it is the
...
- `run-process` (`ReadOnly=false`, `Destructive=true`, `Idempotent=false`, `OpenWorld=false`, **environment-sensitive**) — LAUNCHES a business process at runtime through the built-in `ServiceModel/ProcessEngineService.svc/RunProcess`. Neither gated nor package-gated (one of the three exceptions named in the section note above). MCP-only, no CLI verb, like the rest of this family. Distinct from a *run-process button* on a page (`crt.RunBusinessProcessRequest`), which is authored with `update-page`.
  Identify the process by `process-name`, which must be the schema CODE: a display caption is refused, naming the code it resolved to. Captions are not unique, and unlike the read-only `get-process-signature` this tool STARTS a process, so resolving by an ambiguous key could launch the wrong one. Supply `parameters` keyed by parameter CODE, optionally `result-parameters` (codes to read back) and `timeout` (seconds, unbounded when omitted).
  **The outcome is `status`; `error` is the failure signal** (non-null means the call did not succeed). There is deliberately no `success` boolean: it was a pure function of `status` that carried no information of its own while reading `true` for the two no-verdict statuses, and the platform's own flag cannot be trusted either (`docs/knowledge/platform/runprocess-success-flag-is-not-the-run-verdict.md`). `status` is either the platform's process status lowercased — `completed` (the enum name is `Done`) / `error` / `cancelled` / `cancelling` / `running` / `inactive`, plus `unknown-status-<n>` carrying the raw code for a status this clio does not know — or one of three states the platform's scale cannot express:
  - `not-started` — the platform declined to start the process and nothing ran. The common case is a process whose only start events are automatic (a signal, a timer): it has no manual entry point at all, so no `run-process` call can ever start it.
  - `queued-background` — the schema starts in background mode, so the platform queued it fire-and-forget and returned NO process id, status or result. For such a process the launch IS the whole outcome. Supplying `result-parameters` is what forces the same process to run synchronously and produce a verdict, because a requested result parameter bypasses the background branch.
  - `still-running` — the run outlived the MCP response deadline and clio answered first. There is genuinely no handle to report: the process id exists only in the RunProcess response, and the run's `SysProcessLog` row is buffered and written when the run ends. In both no-verdict cases do NOT re-run to find out — a second launch duplicates the work.
  `status` is **absent** when the call was rejected before launch; read `error` in that case. A `running` verdict means the process suspended on something external (a user task, a timer, a signal) and its `processId` is real — it is also the PRIMARY KEY of the run's `SysProcessLog` row, so poll it with `odata-read` on `SysProcessLog` filtered by `Id` when you need to await completion. The tool itself never polls and never retries — a second launch can duplicate the work.
  Validation runs BEFORE any server call and lists the codes that are accepted: an unknown parameter code (including a case-only mismatch — the platform compares names with `Ordinal`), an `Output` parameter in `parameters`, an `Input` parameter in `result-parameters`, and a non-Guid value for a lookup parameter are each rejected up front. A `String` parameter is passed through VERBATIM and never re-encoded — a serialized ESQ filter double-encoded on the way in produces an EMPTY selection that the platform reports as success, not an error.
