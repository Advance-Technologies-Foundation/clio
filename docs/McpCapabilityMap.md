...
- `run-process` (`ReadOnly=false`, `Destructive=true`, `Idempotent=false`, `OpenWorld=false`, **environment-sensitive**) — LAUNCHES a business process at runtime through the built-in `ServiceModel/ProcessEngineService.svc/RunProcess`. Neither gated nor package-gated (one of the three exceptions named in the section note above). MCP-only, no CLI verb, like the rest of this family. Distinct from a *run-process button* on a page (`crt.RunBusinessProcessRequest`), which is authored with `update-page`.
  Identify the process by `process-name` (schema code OR caption; the resolved code comes back as `resolvedProcessCode`), supply `parameters` keyed by parameter CODE, optionally `result-parameters` (codes to read back) and `timeout` (seconds, unbounded when omitted).
  **The outcome is `mode`, not `success`.** `success` is false for a rejected call, a refused launch and a failed run alike, so it cannot tell them apart (`docs/knowledge/platform/runprocess-success-flag-is-not-the-run-verdict.md` explains why the platform's own flag cannot be trusted either). `mode` is either the platform's status scale lowercased — `completed` (the enum name is `Done`) / `error` / `cancelled` / `cancelling` / `running` / `inactive`, the same scale as `SysProcessStatus.Value`, plus `unknown-status-<n>` for a code this clio does not know — or one of three non-status outcomes:
