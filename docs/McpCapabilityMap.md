...
- `run-process` (`ReadOnly=false`, `Destructive=true`, `Idempotent=false`, `OpenWorld=false`, **environment-sensitive**) — LAUNCHES a business process through the built-in `ServiceModel/ProcessEngineService.svc/RunProcess`. Ungated, MCP-only. Not the *run-process button* (`crt.RunBusinessProcessRequest`), which `update-page` authors.
