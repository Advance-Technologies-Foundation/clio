...
- `run-process` (`ReadOnly=false`, `Destructive=true`, `Idempotent=false`, `OpenWorld=false`, **environment-sensitive**) — LAUNCHES a business process at runtime through the built-in `ServiceModel/ProcessEngineService.svc/RunProcess`. NOT feature-gated and NOT package-gated, for the same reason as `get-process-signature`: the endpoint ships with every Creatio and never touches ProcessDesignService, and gating it would make the capability unavailable exactly where it would have worked. MCP-only, no CLI verb (the ENG-90883 decision for this family).
  ...
  **The verdict is `mode`, never `success`.** `success` only reports that clio executed the call; the platform itself answers `success=true` for a FAILED run whenever its own `Feature-SetErrorInfoIfProcessHasFailedExecution` flag is off, so `mode` is the single field carrying the outcome. It is one of the platform's own statuses — `completed` / `error` / `cancelled` / `cancelling` / `running` / `inactive`, the same scale as `SysProcessStatus.Value` — or one of three non-status outcomes:
  ...
  A `running` verdict means the process suspended on something external (a user task, a timer, a signal) and its `processId` is real — it is also the PRIMARY KEY of the run's `SysProcessLog` row, so poll it with `odata-read` on `SysProcessLog` filtered by `Id` when you need to await completion. The tool itself never polls and never retries: a retry can duplicate work, because idempotency is a property of the specific process and not of the transport.
