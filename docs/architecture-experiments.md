...
## Contributor architecture revision — 22 September 2026

This is a dated continuation of the historical conclusion above. Kirill subsequently authorized contributor review, the generation-queue experiments and publication of the revised [contributor architecture](clio-10-contributor-review.md). That document is the current presentation of the design; this ledger retains the evidence and decision provenance. The original round's closure does not describe the later authorizations.

### Evidence added after the original conclusion

- The runtime/settings joint fixture and its later corrections are linked in the contributor document's appendix, including [13/13 peer platform results](https://github.com/Advance-Technologies-Foundation/clio/discussions/1643#discussioncomment-18545362) and the [corrected reference flow at 4e7f622b7d8a](https://github.com/Advance-Technologies-Foundation/clio/blob/4e7f622b7d8a/experiments/JointIntegration/reference-flow.md). These results are distinct from the queue fixture below.
- Kirill's queue-and-drain idea was published at [18552589](https://github.com/Advance-Technologies-Foundation/clio/discussions/1643#discussioncomment-18552589), prototype **0ab87e21c92bc00ee43ff41ee6b55a76cfd172a1**. Ten Windows cases and a failing unsafe-fallback control establish bounded fixture behavior, with independent peer reports.
- The [two-target extension](https://github.com/Advance-Technologies-Foundation/clio/discussions/1643#discussioncomment-18553068), **1d61e0c08f98eb8f657a7f666ddf150c8eec0b54**, recorded 13/13 Windows cases, three failed mutations and restored Q2/Q11 passes. [Raw evidence](https://github.com/Advance-Technologies-Foundation/clio/tree/1d61e0c08f98eb8f657a7f666ddf150c8eec0b54/experiments/GenerationQueue/evidence/20260922T123800858Z) exercises overlapping target lifetimes, shared drain including cleanup, safe deferral and distinct outcomes after worker loss. All 22 owned sessions exited. These are synthetic targets, not real Creatio or MCP integration.
- [Claude review rev_0a1eef5d066c4f66](https://github.com/Advance-Technologies-Foundation/clio/blob/c0757a2b1ac3274e3a87c0bb7fe5570b0d669fb7/experiments/GenerationQueue/CLAUDE-REVIEW.md) found no blocking issue in that slice. It was read-only, not execution. The source-manifest/commit mapping, 800 ms timing window and preserved-success/ExecutorLost wording qualifications remain explicit.
- [Alex 18553093](https://github.com/Advance-Technologies-Foundation/clio/discussions/1643#discussioncomment-18553093) reports Windows reproduction including all three controls, withdraws the per-target-supervisor objection and raises environment-package compatibility. [Vova 18553156](https://github.com/Advance-Technologies-Foundation/clio/discussions/1643#discussioncomment-18553156) reports macOS arm64 13/13 and both new controls; he distinguishes incompatible targets from busy work and highlights shared supervisor loss. These are attributed peer reports, not new Codex runs.

### Design direction incorporated at Kirill's instruction

Kirill asked to update and publish the architecture after discussing these boundaries:

1. Optional supervised hosting uses one executing backend shared by the session's participating environments. The supervisor owns admission, selected identities, queue, dispatch and records as Core hosting responsibilities. Independent target operations may overlap. Keeping several installed runtime versions does not require simultaneous backend generations.
2. If a candidate needs an environment package or another requirement that is unavailable, keep the current runtime/configuration and service open. Compatibility refusal precedes cutoff and identifies the blocker; it is distinct from a timed wait for busy work. Target/capability scope and the actual compatibility check remain implementation work and are not proved by the fixture.
3. The supervised queue and records are intentionally in memory. Supervisor failure requires restart/reconnect and affects its entire session. An old missing record means the outcome cannot be established, not that nothing executed. Read-only retry and reconciliation/safe repetition for side effects remain caller concerns; there is no automatic replay or durable-acceptance promise.
4. Supervisor scope is enforced through design and code review. Feature policy stays in the runtime; ordinary configuration handles tunable values. Pre-start requirements must be available without starting the candidate. A general policy-description framework is not required. Occasional supervisor code updates require restart/reconnect.

These are the design choices submitted for renewed validation, not retrospective peer consensus or completed production mechanisms. Settings migration, trusted acquisition, real integration and migration planning retain their stated limits. The architecture still centers on reusable layers, partner workflows, one selected context per operation and structured results. No new implementation, test run, merge or release accompanies this documentation revision.

The graph checkpoint before these decisions is **207**, index **906f201e-286f-594b-8674-bd597604c57a** in `clio/discussions/1643`. The next decision/publication records link the revised document and the review invitation, preserving the earlier objections and corrections.
