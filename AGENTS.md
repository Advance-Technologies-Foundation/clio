...
For manual testing of committed business-process functionality, use the `bp-test-cases` and `bp-test-run` skills. The run is a debugging session for three components — `CrtProcessBuilder`, clio itself, and the clio knowledge library — and the prompt is the instrument that exercises them, not the subject.
...
- `bp-test-run` has two modes. `--mode agent` executes the prompt on a stand with locally built clio, local guidance library, and locally built package, then inspects only the read-back description of what was created plus the executor transcript. `--mode browser` runs afterwards against what that left on the stand: opens the processes in the designer, executes them, and verifies the runtime result in the browser.
- An `agent` run establishes the stored level only and must say so in its verdict; it must not clean up the processes it created, because they are the input to the `browser` run.
...
- Every finding is attributed to one owner: `CrtProcessBuilder` (fix in `cli-process-builder`), clio (fix here), or the knowledge library (fix in `clio-knowledge`). Efficiency findings belong to the last two, never to the executor.
- Revising the prompt is a repair path for a case the prompt itself invalidated. Never revise a prompt to make a product defect go away.
