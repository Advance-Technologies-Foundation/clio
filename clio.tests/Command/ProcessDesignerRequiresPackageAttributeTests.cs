...
namespace Clio.Tests
{
    ...
    public class ProcessDesignerRequiresPackageAttributeTests
    {
        ...
        [Test]
        [Description("The two versioning commands declare 1.4.14.0 for a STRICTER reason than its siblings: the ModifyProcessAsNewVersion OPERATION does not exist before that archive at all. Create/Modify name a version because an older server MISHANDLES a newer input form; these name a version because an older server has no such route, and answers a 404 the caller would read as a transport fault rather than 'your package is behind'. The bundled-archive guard asserts the shipped archive satisfies the literal, so the floor can never demand a version clio does not carry - which is why these floors land with their tools and the archive ships first.")]
        public void VersioningOptionsType_ShouldDeclareTheVersionTheOperationFirstShippedIn(Type optionsType)
        {
            ...
            requirement!.Version.Should().Be("1.4.14.0",
                because: "both versioning operations first ship in the 1.4.14.0 archive. Presence-only would let "
