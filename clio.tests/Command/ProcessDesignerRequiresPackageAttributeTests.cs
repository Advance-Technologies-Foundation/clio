...
namespace Clio.Tests
{
    ...
    // convergence rule (TestCategory=Unit&Module=Common) has to reach them.
    ...
    public class ProcessDesignerRequiresPackageAttributeTests
    {
            ...
            Type optionsType, string expectedVersion)
        {
            ...
            requirement!.Version.Should().Be(expectedVersion,
                because: "Each floor states what its OWN command advertises, never what clio happens to bundle - which is why the two have DIVERGED before and may again, even though they agree today at 1.6.6.14. They first parted at 1.6.0.0/1.6.0.1: both were raised to 1.6.0.0 for the Approval reasons below - the released 1.5.0.0 minor outranks every 1.4.x the approval behaviour was numbered in, so a 1.4.x floor was satisfied by a server that drops the whole block - and Modify then sat one patch ABOVE, at 1.6.0.1, for a promise only it makes: a page change reconciles a Pre-configured page element's data sources - declare them, undeclared ones removed and reported, removal refused while something still maps from one. Below that archive the same call is accepted with dataSources omitted, the previous page's DataSource_* parameter rides onto a page that does not declare it, and the instance never leaves Running while describe still answers inSync:true (ENG-95461, reproduced on a stand). 1.6.0.1 and not 1.6.0.0 because 1.6.0.0 was stamped before that fix merged. Create was NOT raised with it: it cannot change an element's page, so nothing it advertises depended on the newer archive. They converged again at 1.6.2.1 (the Send email template members) and moved together to 1.6.6.14 (subProcess.multiInstanceOptions), because those two vocabularies are advertised by BOTH commands. What both numbers still carry: TWO reasons stand behind this floor. ENG-96325's lookup-constant contract shipped in the 1.4.0.40 archive - a mappings[] value on a Lookup target may carry an already-composed macro that an older server rejects as 'not a bare Guid' - and setFlowCondition is an operation an older server does not carry AT ALL — its dispatch "
        ...
        [TestCase(typeof(ModifyProcessAsNewVersionOptions), "1.6.6.30")]
        [TestCase(typeof(SetActiveProcessVersionOptions), "1.6.1.0")]
        [Test]
        [Description("The two versioning commands declare a version for a STRICTER reason than their siblings: the versioning OPERATIONS do not exist before the 1.6.1.0 archive at all. Create/Modify name a version because an older server MISHANDLES a newer input form; these name a version because an older server has no such route, and answers a 404 the caller would read as a transport fault rather than 'your package is behind'. modify-process-as-new-version then moved to 1.6.2.1 with ENG-95986 and to 1.6.6.14 with ENG-99856, both for the same reason: it shares the operations vocabulary, which now advertises the Send email template members and `subProcess.multiInstanceOptions`, it runs no read-back, and convergence already refuses everything below the shipped archive on it - so the raise adds no lockout and keeps the route fail-closed in convergence's degraded modes. set-active-business-process-version carries no operations and stays at 1.6.1.0. Modify-as-new-version moved with the vocabulary to 1.6.6.30 (ENG-92711), for a scriptTask block riding a setElement, which an older server drops while answering success. The bundled-archive guard asserts the shipped archive satisfies every literal, so a floor can never demand a version clio does not carry.")]
            ...
            string expectedVersion)
        {
            ...
            requirement!.Version.Should().Be(expectedVersion,
                because: "the versioning operations first ship in the 1.6.1.0 archive, and the one that carries an "
                    + "operations array followed the operations vocabulary up twice - to 1.6.2.1 with the Send email "
                    + "template members (ENG-95986) and to 1.6.6.14 with subProcess.multiInstanceOptions "
                    + "(ENG-99856). Presence-only "
