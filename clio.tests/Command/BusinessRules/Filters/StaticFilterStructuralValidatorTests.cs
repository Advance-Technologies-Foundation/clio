...
public sealed class StaticFilterStructuralValidatorTests {
	...
	public void Validate_Should_Throw_FilterRequired_When_Filter_Is_Null() {
		// Act
		System.Action act = () => StaticFilterStructuralValidator.Validate(null!);
	...
	public void Validate_Should_Throw_LogicalOperationUnknown_For_Unknown_Token() {
		// Arrange
		StaticFilterGroup group = new("XOR", [], []);

		// Act
		System.Action act = () => StaticFilterStructuralValidator.Validate(group);
	...
	public void Validate_Should_Throw_PathUnknown_When_Leaf_ColumnPath_Is_Empty() {
		// Arrange
		StaticFilterGroup group = new("AND",
			[new StaticFilterLeaf(string.Empty, "EQUAL", JsonDocument.Parse("\"x\"").RootElement)],
			[]);
	...
	public void Validate_Should_Throw_ComparisonUnknown_For_Unknown_Token() {
		// Arrange
		StaticFilterGroup group = new("AND",
			[new StaticFilterLeaf("Name", "MATCHES", JsonDocument.Parse("\"x\"").RootElement)],
			[]);
	...
	public void Validate_Should_Throw_ValueRequired_For_Binary_Without_Value() {
		// Arrange
		StaticFilterGroup group = new("AND",
			[new StaticFilterLeaf("Name", "EQUAL", null)],
			[]);
	...
	public void Validate_Should_Throw_BackwardReference_When_Path_Missing() {
		...
		StaticFilterGroup group = new("AND",
			[],
			[new StaticFilterBackwardReference(string.Empty, nested)]);
	...
	public void Validate_Should_Pass_Happy_Path() {
		...
		StaticFilterGroup group = new("AND",
			...
			new List<StaticFilterBackwardReference>());
