...
public sealed class EsqFilterConverterClientTests {

	private const string Envelope =
		"{\"items\":{\"abc\":{\"filterType\":4}},\"rootSchemaName\":\"City\",\"filterType\":6}";

	private IApplicationClient _applicationClient = null!;
	private EsqFilterConverterClient _client = null!;

	[SetUp]
	public void SetUp() {
		_applicationClient = Substitute.For<IApplicationClient>();
		_client = new EsqFilterConverterClient(_applicationClient);
	}

	[Test]
	[Category("Unit")]
	[Description("Posts the static filter wrapped under filterRequests[0] with the supplied rootSchemaName.")]
	public void ConvertToEsqFilter_Should_Post_Wrapped_Request() {
		...
		StaticFilterGroup filter = new(
			"AND",
			[new StaticFilterLeaf("Country", "EQUAL", JsonDocument.Parse("\"a-guid\"").RootElement)],
			[]);
	...
	[Test]
	[Category("Unit")]
	[Description("Maps backward-reference filters to columnPath + subFilters per the server-side ESQ converter contract.")]
	public void ConvertToEsqFilter_Should_Map_Backward_Reference_Filter() {
		...
		StaticFilterGroup nested = new(
			"AND",
			[new StaticFilterLeaf("Name", "EQUAL", JsonDocument.Parse("\"x\"").RootElement)],
			[]);
	...
	private static StaticFilterGroup BuildEmptyGroup() =>
		new("AND", new List<StaticFilterLeaf>(), new List<StaticFilterBackwardReference>());
