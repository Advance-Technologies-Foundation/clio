...
public sealed class ODataUpdateToolTests {
	...
	public void Update_Should_Read_Data_From_Rows_File() {
		...
		// Act
		ODataWriteResponse response = new ODataUpdateTool(resolver, new ODataFileContract(fileSystem, new MockConfinedFileAccess(fileSystem))).Update(new ODataUpdateArgs {
	...
	public void Update_Should_Reject_An_Unbound_File_Source_Alias_Before_Any_Patch() {
		...
		// Act
		ODataWriteResponse response = new ODataUpdateTool(resolver, new ODataFileContract(fileSystem, new MockConfinedFileAccess(fileSystem))).Update(args);
	...
	public void Update_Should_Patch_Addressed_Key_With_Body() {
		...
		ODataUpdateTool tool = new(resolver, new ODataFileContract(Substitute.For<System.IO.Abstractions.IFileSystem>(), new MockConfinedFileAccess(Substitute.For<System.IO.Abstractions.IFileSystem>())));
