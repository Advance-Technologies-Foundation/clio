...
/// <summary>
/// Story 8 (ENG-93347) passthrough behavior of the <c>delete-app-section</c> MCP tool: header-only
/// calls execute against the header tenant resolved by <see cref="IToolCommandResolver"/> across the
/// WHOLE nested call graph — the tool drives a REAL <see cref="ApplicationSectionDeleteService"/> so
/// the single nested dependency (today <c>ApplicationSectionDeleteCommand.cs:76</c>,
...
public sealed class ApplicationSectionDeleteToolPassthroughTests {
	...
	private ApplicationSectionDeleteTool _tool;
	...
	public void SetUp() {
		...
		// The tool drives the REAL section-delete service so the nested FindApplicationId call is
		// exercised for real instead of being hidden behind a service substitute.
		IApplicationSectionDeleteService sectionDeleteService = new ApplicationSectionDeleteService(
			_settingsRepository,
			_applicationClientFactory,
			_serviceUrlBuilder,
			_applicationInfoService,
			Substitute.For<ILogger>());
		_tool = new ApplicationSectionDeleteTool(Substitute.For<ILogger>(), _commandResolver, sectionDeleteService);
	}
	...
	private static ApplicationSectionDeleteArgs CreateArgs(string? environmentName) =>
		new(
	...
	/// <summary>
	/// Stubs the full happy-path collaborator chain (application lookup, section select/readback,
	/// delete-query persist) so <see cref="ApplicationSectionDeleteService.DeleteSection(EnvironmentSettings, ApplicationSectionDeleteRequest)"/>
	/// completes end-to-end against <paramref name="settings"/>.
	/// </summary>
	private void ConfigureSuccessfulDelete(EnvironmentSettings settings, InstalledAppSummary applicationSummary) {
	...
	private static string BuildMockResponse(string requestBody) {
		...
		bool isDeleteQuery = requestBody.Contains(
			"\"__type\":\"Terrasoft.Nui.ServiceModel.DataContract.DeleteQuery\"", StringComparison.Ordinal);
	...
	[Test]
	[Category("Unit")]
	[Description("AC-01/AC-02: a header-only passthrough call (no environment-name) deletes the section against the header tenant resolved by IToolCommandResolver — the settings-based service overload runs end-to-end and the legacy 'Environment name is required' failure never surfaces.")]
	public async Task ApplicationSectionDelete_ShouldExecuteAgainstHeaderTenant_WhenHeaderOnly() {
		// Arrange
		...
		ConfigureSuccessfulDelete(headerTenantSettings, applicationSummary);

		// Act
		ApplicationSectionDeleteContextResponse result = await _tool.ApplicationSectionDelete(
	...
	public async Task ApplicationSectionDelete_ShouldResolveApplicationIdAgainstHeaderTenant_WhenHeaderOnly() {
		// Arrange
		EnvironmentSettings headerTenantSettings = CreateTenantSettings();
		ConfigureHeaderTenant(headerTenantSettings);
		InstalledAppSummary applicationSummary = CreateApplicationSummary();
		...
		// Assert
		result.Success.Should().BeTrue(
			because: "the nested application lookup must not derail a header-only delete call");
	...
	public async Task ApplicationSectionDelete_ShouldRejectCall_WhenHeaderAndEnvironmentNameBothPresent() {
		// Arrange
		_commandResolver.Resolve<EnvironmentSettings>(
	...
	[Test]
	[Category("Unit")]
	[Description("AC-05: on stdio / registered-environment transports an explicit environment-name resolves through the unchanged registry branch and the whole delete-section flow — including the nested FindApplicationId call — executes against those settings exactly as before.")]
	public async Task ApplicationSectionDelete_ShouldBehaveUnchanged_WhenRegisteredEnvironmentStdio() {
		// Arrange
		...
		InstalledAppSummary applicationSummary = CreateApplicationSummary();
		ConfigureSuccessfulDelete(registeredSettings, applicationSummary);
	...
	public async Task ApplicationSectionDelete_ShouldReturnResolverRequirednessError_WhenNoEnvironmentNameAndNoPassthrough() {
		// Arrange
		_commandResolver.Resolve<EnvironmentSettings>(
				Arg.Is<EnvironmentOptions>(options => string.IsNullOrWhiteSpace(options.Environment)))
	...
	[Test]
	[Category("Unit")]
	[Description("AC-ERR(b): a valid passthrough header whose target delete operation fails yields the typed error envelope with redacted text — no accessToken/login/password material leaks.")]
	public async Task ApplicationSectionDelete_ShouldReturnRedactedErrorEnvelope_WhenHeaderTenantOperationFails() {
		// Arrange
		EnvironmentSettings headerTenantSettings = CreateTenantSettings();
		ConfigureHeaderTenant(headerTenantSettings);
	...
	[Test]
	[Category("Unit")]
	[Description("AC-02 (schema relax, FR-05a): environment-name on ApplicationSectionDeleteArgs carries no [Required] attribute, so a header-only call reaches the tool method instead of being rejected at MCP binding.")]
	public void ApplicationSectionDeleteArgs_ShouldBeSchemaOptional_WhenEnvironmentNamePropertyIsInspected() {
		// Arrange
		PropertyInfo environmentNameProperty = typeof(ApplicationSectionDeleteArgs)
			.GetProperty(nameof(ApplicationSectionDeleteArgs.EnvironmentName))!;
		...
		hasRequiredAttribute.Should().BeFalse(
			because: "FR-05a makes environment-name schema-optional on delete-app-section so a header-only passthrough call is not rejected before the tool runs");
	}
	...
	public async Task ApplicationSectionDelete_ShouldDeriveTenantLockKeyFromCallOptions_WhenExecuted() {
		// Arrange
		EnvironmentSettings registeredSettings = new() { Uri = "https://registered.example.com" };
		...
		// Act
		await _tool.ApplicationSectionDelete(CreateArgs("sandbox"), server: null);
