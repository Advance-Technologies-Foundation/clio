...
public sealed class InstallGateToolTests {
	...
	public void InstallGate_Should_Resolve_Command_For_Environment_And_Return_Exit_Code() {
		...
		try {
			// Act
			CommandExecutionResult result = tool.InstallGate(new InstallGateRunArgs("sandbox"));
	...
	[Ignore("ENG-90312 Phase 2: install-gate folded into clio-run; the per-method [McpServerTool] safety flags/description now live on clio-run itself, so the legacy attribute reflection no longer applies.")]
