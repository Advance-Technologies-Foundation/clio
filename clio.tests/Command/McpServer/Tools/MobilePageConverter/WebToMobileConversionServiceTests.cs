...
public sealed class WebToMobileConversionServiceTests {
	...
	[Test]
	[Description("ENG-93152 REGRESSION BASELINE (pre-headerActionsContainer): today, a page-specific survivor of " +
		"the web MainHeader — hoisted to the tree root by template-component pruning — is caught by the " +
		"CardContentWrapper:top positional rule ONLY because it happens to land as an array sibling of " +
		"CardContentWrapper, landing in MainContainer at index 0 purely by tree-order accident (no explicit " +
		"header rule exists yet). This test locks in that CURRENT behavior as a signal: once WalkElements gains " +
		"the deterministic headerActionsContainer bucket, this exact placement must change (superseded by a " +
		"synthesized header-actions container), and this test must be updated/replaced in the same change — not " +
		"left passing or failing unexamined.")]
	public void Analyze_ElementMap_HeaderOriginSurvivor_AccidentallyCaughtByPositionalTop_PreHeaderActionsContainerBaseline() {
		PageBundleInfo bundle = Bundle("""
			[
			  { "name": "MainHeader", "type": "crt.FlexContainer", "items": [
			      { "name": "TitleContainer", "type": "crt.FlexContainer", "items": [] },
			...
			""");
		var mobileTypes = new HashSet<string>(StringComparer.OrdinalIgnoreCase) {
			"crt.GridContainer", "crt.FlexContainer", "crt.Button"
		};
		var map = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase) {
			["CardContentWrapper"] = "GeneralTabContainer", ["Tabs"] = "Tabs"
		};
		var placements = new List<WebToMobileAnalysisService.PositionalPlacement> { new("CardContentWrapper", "Tabs") };
		var mobileParents = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase) { ["Tabs"] = "MainContainer" };
		IReadOnlySet<string> templateNames = Names("MainHeader", "TitleContainer", "ActionButtonsContainer");
		...
		// MainHeader chrome (and its own nested chrome, ActionButtonsContainer) is pruned; CreateOrderButton,
		// its only non-chrome descendant, is hoisted all the way to the tree root — landing as a direct array
		// sibling of CardContentWrapper.
		guide.SourceStructure.Should().NotContain(s => s.Name == "MainHeader" || s.Name == "ActionButtonsContainer");
		ElementMapEntry button = Element(guide, "CreateOrderButton");
		button.Operation.Should().Be("insert");
		button.ParentName.Should().Be("MainContainer",
			because: "it is now a sibling of CardContentWrapper and is caught by the CardContentWrapper:top rule purely by array position");
		button.Index.Should().Be(0,
			because: "it is the only sibling ABOVE CardContentWrapper — an accident of tree order, not a deliberate header-actions rule");
	}

	[Test]
	[Description("ENG-93152: with headerActionsContainer declared on the template rule, a page-specific MainHeader " +
		"survivor is routed into ONE synthesized crt.FlexContainer at bucket 0 of the mobile Tabs' own parent — " +
		"deterministic classification by tree origin, replacing the CardContentWrapper:top positional-rule " +
		"accident locked in by the pre-change baseline test above.")]
	public void Analyze_ElementMap_HeaderActionsContainer_RoutesSurvivorIntoSynthesizedBucket0() {
		PageBundleInfo bundle = Bundle("""
			[
			  { "name": "MainHeader", "type": "crt.FlexContainer", "items": [
			      { "name": "TitleContainer", "type": "crt.FlexContainer", "items": [] },
		...
		var templateRule = new TemplateMappingRule {
			Web = "PageWithTabsFreedomTemplate", Mobile = "MobilePageWithTabsFreedomTemplate",
			HeaderActionsContainer = new HeaderActionsContainerRule {
				Web = "MainHeader",
				Container = new SynthesizedContainerRule {
					NamePrefix = "HeaderActionsContainer_",
					Values = JsonSerializer.Deserialize<Dictionary<string, JsonElement>>(
						"""{ "type": "crt.FlexContainer", "direction": "row" }""")
				}
			}
		};
		...
		MobilePageConversionGuide guide = WebToMobileAnalysisService.Analyze(
			bundle, mobileTypes, new HashSet<string>(StringComparer.OrdinalIgnoreCase),
			webByType: new Dictionary<string, ComponentRegistryEntry>(StringComparer.OrdinalIgnoreCase),
			mobileByType: null, GridRule, templateRule: templateRule,
			sourcePage: "Leads_FormPage", sourceTemplate: "PageWithTabsFreedomTemplate",
			suggestedTarget: "UsrLeads_MobileFormPage", containerNameMap: map,
			positionalPlacements: placements, mobileContainerParents: mobileParents,
			templateComponentNames: templateNames);
		...
		ElementMapEntry button = Element(guide, "CreateOrderButton");
		button.Operation.Should().Be("insert");
		button.ParentName.Should().NotBe("MainContainer",
			because: "it must go into the synthesized header-actions container, not directly into MainContainer");
		button.Reason.Should().Contain("MainHeader-origin");

		guide.HeaderActionsContainer.Should().NotBeNull();
		guide.HeaderActionsContainer.AnchorContainer.Should().Be("MainHeader");
		guide.HeaderActionsContainer.ParentName.Should().Be("MainContainer");
		guide.HeaderActionsContainer.MovedChildren.Should().Contain("CreateOrderButton");
		button.ParentName.Should().Be(guide.HeaderActionsContainer.ContainerName);

		ElementMapEntry synthesized = guide.ElementMap.Single(
			e => e.WebName is null && e.MobileName == guide.HeaderActionsContainer.ContainerName);
		synthesized.Operation.Should().Be("insert");
		synthesized.MobileType.Should().Be("crt.FlexContainer");
		synthesized.ParentName.Should().Be("MainContainer");
		synthesized.Index.Should().Be(0);

		guide.Constraints.Should().Contain(c => c.Contains("headerActionsContainer is MANDATORY"),
			because: "the model must never treat this relocation as optional");
		guide.NextSteps.Should().Contain(s => s.Contains("guide.headerActionsContainer"),
			because: "the build flow must point at the synthesized container the same way tabAreaLayers does");
	}

	private static TemplateMappingRule HeaderActionsTemplateRule() => new() {
		Web = "PageWithTabsFreedomTemplate", Mobile = "MobilePageWithTabsFreedomTemplate",
		HeaderActionsContainer = new HeaderActionsContainerRule {
			Web = "MainHeader",
			Container = new SynthesizedContainerRule {
	...
	[Test]
	[Description("ENG-93152: multiple header-origin survivors (two page-added buttons under MainHeader) collapse into ONE synthesized wrapper — no duplicate containers, both children moved in.")]
	public void Analyze_ElementMap_HeaderActionsContainer_MultipleSurvivors_CollapseIntoOneWrapper() {
		PageBundleInfo bundle = Bundle("""
			[
			  { "name": "MainHeader", "type": "crt.FlexContainer", "items": [
			      { "name": "ActionButtonsContainer", "type": "crt.FlexContainer", "items": [
			...
			""");
		var mobileTypes = new HashSet<string>(StringComparer.OrdinalIgnoreCase) { "crt.GridContainer", "crt.FlexContainer", "crt.Button" };
		var map = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase) {
			["CardContentWrapper"] = "GeneralTabContainer", ["Tabs"] = "Tabs"
		};
		var placements = new List<WebToMobileAnalysisService.PositionalPlacement> { new("CardContentWrapper", "Tabs") };
		var mobileParents = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase) { ["Tabs"] = "MainContainer" };
		IReadOnlySet<string> templateNames = Names("MainHeader", "ActionButtonsContainer");

		MobilePageConversionGuide guide = WebToMobileAnalysisService.Analyze(
			bundle, mobileTypes, new HashSet<string>(StringComparer.OrdinalIgnoreCase),
			webByType: new Dictionary<string, ComponentRegistryEntry>(StringComparer.OrdinalIgnoreCase),
			mobileByType: null, GridRule, templateRule: HeaderActionsTemplateRule(),
			sourcePage: "Leads_FormPage", sourceTemplate: "PageWithTabsFreedomTemplate",
			suggestedTarget: "UsrLeads_MobileFormPage", containerNameMap: map,
			positionalPlacements: placements, mobileContainerParents: mobileParents,
			templateComponentNames: templateNames);
		...
		guide.HeaderActionsContainer.Should().NotBeNull();
		guide.HeaderActionsContainer.MovedChildren.Should().BeEquivalentTo(["CreateOrderButton", "PrintButton"]);
		guide.ElementMap.Count(e => e.WebName is null && e.MobileType == "crt.FlexContainer" && e.ParentName == "MainContainer")
			.Should().Be(1, because: "both survivors share ONE synthesized wrapper, never one each");
		Element(guide, "CreateOrderButton").ParentName.Should().Be(guide.HeaderActionsContainer.ContainerName);
		Element(guide, "PrintButton").ParentName.Should().Be(guide.HeaderActionsContainer.ContainerName);
	}

	[Test]
	[Description("ENG-93152: a header-origin survivor together with generic CardContentWrapper:top content — header-actions is bucket 0, the generic :top sibling shifts to bucket 1 (index 1), no gaps.")]
	public void Analyze_ElementMap_HeaderActionsContainer_TogetherWithGenericTop_OrdersHeaderFirst() {
		PageBundleInfo bundle = Bundle("""
			[
			  { "name": "MainHeader", "type": "crt.FlexContainer", "items": [
			      { "name": "ActionButtonsContainer", "type": "crt.FlexContainer", "items": [
			          { "name": "CreateOrderButton", "type": "crt.Button" } ] } ] },
			  { "name": "ProgressBarContainer", "type": "crt.FlexContainer", "items": [] },
			  { "name": "CardContentWrapper", "type": "crt.GridContainer", "items": [
			      { "name": "Tabs", "type": "crt.TabPanel", "items": [] } ] }
			]
			""");
		...
		ElementMapEntry synthesized = guide.ElementMap.Single(e => e.WebName is null && e.MobileType == "crt.FlexContainer" && e.ParentName == "MainContainer");
		synthesized.Index.Should().Be(0, because: "the header-actions container is always bucket 0");
		ElementMapEntry progress = Element(guide, "ProgressBarContainer");
		progress.ParentName.Should().Be("MainContainer");
		progress.Index.Should().Be(1, because: "the generic CardContentWrapper:top sibling shifts to bucket 1, right after the header-actions container");
	}

	[Test]
	[Description("ENG-93152: headerActionsContainer declared on the template but the page has NO MainHeader survivor — behavior is unchanged (no regression): the generic CardContentWrapper:top sibling still lands at index 0, and no synthesized container is reported.")]
	public void Analyze_ElementMap_HeaderActionsContainer_DeclaredButNoSurvivor_NoRegression() {
		PageBundleInfo bundle = Bundle("""
			[
			  { "name": "ProgressBarContainer", "type": "crt.FlexContainer", "items": [] },
			  { "name": "CardContentWrapper", "type": "crt.GridContainer", "items": [
			...
			""");
		var mobileTypes = new HashSet<string>(StringComparer.OrdinalIgnoreCase) { "crt.GridContainer", "crt.FlexContainer" };
		var map = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase) {
			["CardContentWrapper"] = "GeneralTabContainer", ["Tabs"] = "Tabs"
		};
		var placements = new List<WebToMobileAnalysisService.PositionalPlacement> { new("CardContentWrapper", "Tabs") };
		...
		guide.HeaderActionsContainer.Should().BeNull();
		guide.ElementMap.Should().NotContain(e => e.WebName == null && e.MobileType == "crt.FlexContainer" && e.ParentName == "MainContainer");
		Element(guide, "ProgressBarContainer").Index.Should().Be(0,
			because: "with no header-origin survivor, the generic :top sibling keeps its plain bucket-0 slot — unchanged");
	}

	[Test]
	[Description("ENG-93152 edge case: the only header-origin candidate is itself unconvertible (unsupported leaf type) and drops — no synthesized container is created, no orphan index is left behind in MainContainer.")]
	public void Analyze_ElementMap_HeaderActionsContainer_OnlyCandidateDrops_NoContainerNoOrphanIndex() {
		PageBundleInfo bundle = Bundle("""
			[
			  { "name": "MainHeader", "type": "crt.FlexContainer", "items": [
			      { "name": "ActionButtonsContainer", "type": "crt.FlexContainer", "items": [
			          { "name": "UnsupportedWidget", "type": "crt.NotOnMobile" } ] } ] },
			  { "name": "CardContentWrapper", "type": "crt.GridContainer", "items": [
			      { "name": "Tabs", "type": "crt.TabPanel", "items": [] } ] }
			]
			""");
		...
		Element(guide, "UnsupportedWidget").Operation.Should().Be("drop");
		guide.HeaderActionsContainer.Should().BeNull();
		guide.ElementMap.Should().NotContain(e => e.WebName == null && e.MobileType == "crt.FlexContainer" && e.ParentName == "MainContainer");
		guide.ElementMap.Where(e => e.Operation == "insert" && e.ParentName == "MainContainer" && e.Index is not null)
			.Should().BeEmpty(because: "with no header-actions container and no generic :top content, MainContainer gets no indexed inserts at all");
	}

	[Test]
	[Description("ENG-93152: MainHeader is present but is PURE chrome (title/back-button only, no page-specific content) — pruned as usual by template-component pruning, and no header-actions container is synthesized because CollectHeaderOriginNames found nothing to relocate.")]
	public void Analyze_ElementMap_HeaderActionsContainer_MainHeaderPureChrome_NoContainerSynthesized() {
		PageBundleInfo bundle = Bundle("""
			[
			  { "name": "MainHeader", "type": "crt.FlexContainer", "items": [
			      { "name": "TitleContainer", "type": "crt.FlexContainer", "items": [
		...
		var placements = new List<WebToMobileAnalysisService.PositionalPlacement> { new("CardContentWrapper", "Tabs") };
		var mobileParents = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase) { ["Tabs"] = "MainContainer" };
		IReadOnlySet<string> templateNames = Names("MainHeader", "TitleContainer", "PageTitle", "BackButton");
		...
		guide.SourceStructure.Should().NotContain(s => s.Name == "MainHeader" || s.Name == "PageTitle" || s.Name == "BackButton",
			because: "pure template chrome keeps being dropped by PruneTemplateComponents, unaffected by headerActionsContainer");
		guide.HeaderActionsContainer.Should().BeNull();
		guide.Constraints.Should().NotContain(c => c.Contains("headerActionsContainer is MANDATORY"));
	}
	...
	// ── ENG-93152: header-actions origin tracking (CollectHeaderOriginNames, standalone from BuildElementMap) ──

	[Test]
	[Description("A page-specific, non-chrome leaf nested several baseline layers deep under the declared header container is collected by name; the chrome layers themselves (and the container's own name) are not.")]
	public void CollectHeaderOriginNames_ChromeLayersSkipped_PageSpecificLeafCollected() {
		JArray tree = JArray.Parse("""
			[ { "name": "MainHeader", "items": [
			      { "name": "TitleContainer", "items": [] },
			      { "name": "ActionButtonsContainer", "items": [
			          { "name": "CreateOrderButton" } ] } ] },
			  { "name": "CardContentWrapper", "items": [] } ]
			""");
		var containerNameMap = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
		IReadOnlySet<string> baseline = Names("MainHeader", "TitleContainer", "ActionButtonsContainer");

		HashSet<string> origins = WebToMobileAnalysisService.CollectHeaderOriginNames(
			tree, "MainHeader", containerNameMap, componentMap: null, baseline: baseline);

		origins.Should().BeEquivalentTo("CreateOrderButton");
	}

	[Test]
	[Description("A survivor that is itself a container: only its own name is collected, never its descendants — they stay nested under it after the hoist and get their normal parent when the walk later recurses into it.")]
	public void CollectHeaderOriginNames_ContainerSurvivor_OnlyItsOwnNameCollected_NotDescendants() {
		JArray tree = JArray.Parse("""
			[ { "name": "MainHeader", "items": [
			      { "name": "CustomButtonGroup", "items": [
			          { "name": "Button_a" }, { "name": "Button_b" } ] } ] } ]
			""");
		IReadOnlySet<string> baseline = Names("MainHeader");

		HashSet<string> origins = WebToMobileAnalysisService.CollectHeaderOriginNames(
			tree, "MainHeader", new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase), componentMap: null, baseline: baseline);

		origins.Should().BeEquivalentTo("CustomButtonGroup");
	}

	[Test]
	[Description("A mapped twin (container or component) nested inside the header is carved out: neither the twin nor its descendants are collected — it has its own conversion rule and is out of scope for this bucket.")]
	public void CollectHeaderOriginNames_MappedTwin_CarvedOut_NeitherTwinNorDescendantsCollected() {
		JArray tree = JArray.Parse("""
			[ { "name": "MainHeader", "items": [
			      { "name": "CardToggleTabPanel", "items": [
			          { "name": "NestedTab" } ] },
			      { "name": "CreateOrderButton" } ] } ]
			""");
	...
	[Test]
	[Description("An anonymous wrapper between the header and a page-specific leaf is transparent — the leaf underneath is still collected by name, since it (not the nameless wrapper) is what the walk can actually reroute.")]
	public void CollectHeaderOriginNames_AnonymousWrapper_IsTransparent() {
		JArray tree = JArray.Parse("""
			[ { "name": "MainHeader", "items": [
			      { "items": [ { "name": "CreateOrderButton" } ] } ] } ]
			""");
		IReadOnlySet<string> baseline = Names("MainHeader");
	...
	[Test]
	[Description("No baseline available (template probe unavailable): returns empty rather than misreading every MainHeader child as a survivor — without a baseline there is nothing to distinguish chrome from page content, and PruneTemplateComponents itself would not run either.")]
	public void CollectHeaderOriginNames_NoBaseline_ReturnsEmpty() {
		JArray tree = JArray.Parse("""
			[ { "name": "MainHeader", "items": [ { "name": "TitleContainer", "items": [] } ] } ]
			""");

		HashSet<string> origins = WebToMobileAnalysisService.CollectHeaderOriginNames(
			tree, "MainHeader", new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase), componentMap: null, baseline: null);

		origins.Should().BeEmpty();
	}

	[Test]
	[Description("The declared header container is absent from the tree entirely: returns empty rather than throwing.")]
	public void CollectHeaderOriginNames_ContainerAbsentFromTree_ReturnsEmpty() {
		JArray tree = JArray.Parse("""[ { "name": "CardContentWrapper", "items": [] } ]""");
		IReadOnlySet<string> baseline = Names("MainHeader");
