...
public sealed class CaptionCultureArgMappingToolTests {
	...
	public async Task ApplicationSectionCreate_ShouldMapCaptionCulture_WhenCaptionCultureProvided() {
		...
		EnvironmentSettings resolvedSettings = new() { Uri = "https://dev.example.com" };
		IToolCommandResolver commandResolver = Substitute.For<IToolCommandResolver>();
		...
		service.CreateSection(resolvedSettings, Arg.Do<ApplicationSectionCreateRequest>(request => capturedRequest = request), Arg.Any<int?>(), Arg.Any<int?>(), Arg.Any<bool>());
