...
/// <summary>
/// Story 4 (ENG-93347) passthrough behavior of the <c>get-app-info</c> MCP tool: header-only calls
...
public sealed class ApplicationGetInfoToolPassthroughTests {
	...
	private ApplicationGetInfoTool _tool;

	[SetUp]
	public void SetUp() {
		_applicationInfoService = Substitute.For<IApplicationInfoService>();
		_commandResolver = Substitute.For<IToolCommandResolver>();
		_tool = new ApplicationGetInfoTool(Substitute.For<ILogger>(), _commandResolver, _applicationInfoService);
	}

	private static ApplicationInfoResult CreateInfoResult(string applicationCode) =>
		new(
			"pkg-uid",
			"UsrTenantApp",
			[],
	...
	public void ApplicationGetInfo_ShouldExecuteAgainstHeaderTenant_WhenHeaderOnly() {
		// Arrange
		...
		_applicationInfoService.GetApplicationInfo(headerTenantSettings, null, "TENANTAPP")
			.Returns(CreateInfoResult("TENANTAPP"));

		// Act
		ApplicationContextResponse result = _tool.ApplicationGetInfo(new ApplicationGetInfoArgs(
		...
		result.ApplicationCode.Should().Be("TENANTAPP",
			because: "the tool must return the application info the settings-based service overload loaded from the header tenant");
		_applicationInfoService.Received(1).GetApplicationInfo(headerTenantSettings, null, "TENANTAPP");
	...
	public void ApplicationGetInfo_ShouldRejectCall_WhenHeaderAndEnvironmentNameBothPresent() {
		// Arrange
		_commandResolver.Resolve<EnvironmentSettings>(
		...
		// Act
		ApplicationContextResponse result = _tool.ApplicationGetInfo(new ApplicationGetInfoArgs(
			EnvironmentName: "other-registered-env",
			Id: null,
			Code: "TENANTAPP"));
	...
	public void ApplicationGetInfo_ShouldBehaveUnchanged_WhenRegisteredEnvironmentStdio() {
		// Arrange
		...
		_applicationInfoService.GetApplicationInfo(registeredSettings, "app-id", null)
			.Returns(CreateInfoResult("ALPHA"));
		...
		result.ApplicationCode.Should().Be("ALPHA",
			because: "the tool must return the application info loaded from the registered environment's settings");
	...
	public void ApplicationGetInfo_ShouldReturnResolverRequirednessError_WhenNoEnvironmentNameAndNoPassthrough() {
		// Arrange
		_commandResolver.Resolve<EnvironmentSettings>(
				Arg.Is<EnvironmentOptions>(options => string.IsNullOrWhiteSpace(options.Environment)))
	...
	public void ApplicationGetInfo_ShouldReturnRedactedErrorEnvelope_WhenHeaderTenantOperationFails() {
		// Arrange
		EnvironmentSettings headerTenantSettings = new() { Uri = "https://tenant.example.com" };
	...
	[Test]
	[Category("Unit")]
	[Description("AC-02 (schema relax, FR-05a): environment-name on ApplicationGetInfoArgs carries no [Required] attribute, so a header-only call reaches the tool method instead of being rejected at MCP binding.")]
	public void ApplicationGetInfoArgs_ShouldBeSchemaOptional_WhenEnvironmentNamePropertyIsInspected() {
		// Arrange
		PropertyInfo environmentNameProperty = typeof(ApplicationGetInfoArgs)
			.GetProperty(nameof(ApplicationGetInfoArgs.EnvironmentName))!;
		...
		hasRequiredAttribute.Should().BeFalse(
			because: "FR-05a makes environment-name schema-optional on get-app-info so a header-only passthrough call is not rejected before the tool runs");
	}
	...
	public void ApplicationGetInfo_ShouldDeriveTenantLockKeyFromCallOptions_WhenExecuted() {
		// Arrange
		EnvironmentSettings registeredSettings = new() { Uri = "https://registered.example.com" };
		_commandResolver.Resolve<EnvironmentSettings>(Arg.Any<EnvironmentOptions>())
			.Returns(registeredSettings);
		...
		// Act
		_tool.ApplicationGetInfo(new ApplicationGetInfoArgs(
			EnvironmentName: "sandbox",
			Id: "app-id",
			Code: null));
