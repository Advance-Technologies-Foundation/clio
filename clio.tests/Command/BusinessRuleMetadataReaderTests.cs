...
public sealed class BusinessRuleMetadataReaderTests {
	...
	public void Read_Should_RoundTrip_Converter_Metadata_With_Block_UIds() {
		...
		// Act
		IReadOnlyList<BusinessRuleReadModel> models = BusinessRuleMetadataReader.Read(rules, []);

		// Assert
		models.Should().ContainSingle(because: "one persisted parent rule yields one read model");
		BusinessRuleReadModel model = models[0];
		model.Convertible.Should().BeTrue(because: "converter-generated metadata is always representable by the friendly contract");
		model.Name.Should().Be(metadata.Name, because: "the internal rule name is the update/delete match key and must round-trip");
		model.Caption.Should().Be("Round trip", because: "the persisted caption round-trips onto the read model");
		model.Enabled.Should().BeTrue(because: "the rule was persisted enabled");
		BusinessRule rule = model.Rule!;
		rule.Name.Should().Be(metadata.Name, because: "the friendly rule carries the match key too");
	...
	public void Read_Should_Skip_Child_Rules_And_Fold_ApplyFilter_Flags_Into_Parent_Action() {
		...
		models.Should().ContainSingle(
			because: "autogenerated child rules with parentUId must never surface as separate read models");
		BusinessRuleReadModel model = models[0];
		model.Convertible.Should().BeTrue(because: "converter-generated apply-filter parents are representable");
		ApplyFilterBusinessRuleAction action = model.Rule!.Actions.Single()
	...
	public void Read_Should_Map_SetFilter_Action_To_ApplyStaticFilter_With_Esq_Envelope() {
		...
		// Assert
		models.Should().ContainSingle(because: "the single persisted rule yields one read model");
		models[0].Convertible.Should().BeTrue(because: "persisted set-filter rules are representable by the friendly contract");
		ApplyStaticFilterBusinessRuleAction action = models[0].Rule!.Actions.Single()
	...
	[Test]
	[Category("Unit")]
	[Description("Falls back to a non-convertible raw model when the action typeName is unknown, still populating name, caption, and the persisted enabled=false flag.")]
	public void Read_Should_Return_Raw_Model_When_Action_TypeName_Is_Unknown() {
		// Arrange
		JsonArray rules = ParseRules($$"""
			[
			  {
		...
		// Assert
		models.Should().ContainSingle(because: "non-convertible rules are still surfaced instead of hidden");
		BusinessRuleReadModel model = models[0];
		model.Convertible.Should().BeFalse(because: "an unknown action typeName cannot be represented by the friendly contract");
		model.Rule.Should().BeNull(because: "non-convertible models carry no friendly rule");
		model.Raw.Should().NotBeNull(because: "non-convertible models must carry the untouched persisted metadata");
		model.Raw!["uId"]!.GetValue<string>().Should().Be("bbbbbbbb-0000-0000-0000-000000000001",
			because: "the raw payload is the persisted rule object itself");
		model.Name.Should().Be("BusinessRule_odd", because: "the match key is populated even for non-convertible rules");
		model.Caption.Should().Be("Designer authored", because: "the caption is populated even for non-convertible rules");
		model.Enabled.Should().BeFalse(because: "the persisted enabled=false flag is honored on the read model");
	}

	[Test]
	[Category("Unit")]
	[Description("Falls back to a non-convertible raw model for multi-case rules, which the single-case friendly contract cannot represent.")]
	public void Read_Should_Return_Raw_Model_When_Rule_Has_Multiple_Cases() {
		// Arrange
		JsonArray rules = ParseRules($$"""
			[
			  {
		...
		// Assert
		models.Should().ContainSingle(because: "multi-case rules are surfaced raw instead of being hidden");
		models[0].Convertible.Should().BeFalse(because: "the friendly contract only represents single-case rules");
		models[0].Raw.Should().NotBeNull(because: "the persisted multi-case metadata must be returned raw");
		models[0].Name.Should().Be("BusinessRule_multi", because: "the match key is populated even for non-convertible rules");
	}

	[Test]
	[Category("Unit")]
	[Description("Falls back to a non-convertible raw model when the condition group nests another group, which the single-group friendly contract cannot represent.")]
	public void Read_Should_Return_Raw_Model_When_Condition_Group_Contains_Nested_Group() {
		// Arrange
		JsonArray rules = ParseRules($$"""
			[
			  {
		...
		// Assert
		models.Should().ContainSingle(because: "nested-group rules are surfaced raw instead of being hidden");
		models[0].Convertible.Should().BeFalse(because: "the friendly contract only supports a single flat condition group");
		models[0].Raw.Should().NotBeNull(because: "the persisted nested-group metadata must be returned raw");
	}
	...
	public void Read_Should_Resolve_Caption_From_Resources_And_Default_Enabled_When_Missing() {
		...
		// Act
		IReadOnlyList<BusinessRuleReadModel> models = BusinessRuleMetadataReader.Read(rules, resources);
		...
		models[0].Convertible.Should().BeTrue(
			because: "a case with no condition maps to an empty AND group and no actions is a valid friendly rule");
		models[0].Rule!.Condition.LogicalOperation.Should().Be("AND",
			because: "a missing case condition maps to an empty AND group");
		models[0].Rule!.Condition.Conditions.Should().BeEmpty(
	...
	public void Read_Should_Wrap_Bare_Condition_In_And_Group_And_Split_Csv_Items() {
		...
		models.Should().ContainSingle(because: "the single persisted rule yields one read model");
		BusinessRuleReadModel model = models[0];
		model.Convertible.Should().BeTrue(because: "a bare single condition is representable as a one-condition AND group");
		model.Enabled.Should().BeFalse(because: "the persisted enabled=false flag must be honored");
		model.Rule!.Enabled.Should().BeFalse(because: "the friendly rule carries the persisted enabled flag");
		model.Rule!.Condition.LogicalOperation.Should().Be("AND",
			because: "a bare condition is wrapped into an AND group, which the platform accepts on update");
		model.Rule!.Condition.Conditions.Should().ContainSingle(
			because: "the bare condition becomes the single entry of the wrapping group");
		model.Rule!.Condition.Conditions[0].UId.Should().Be("ffffffff-0000-0000-0000-000000000003",
			because: "the wrapped condition keeps its persisted block uId");
		MakeEditableBusinessRuleAction action = model.Rule!.Actions.Single()
	...
	public void Read_Should_Map_Formula_SetValue_Source_To_Formula_Expression_With_Schema_Text() {
		...
		// Assert
		models.Should().ContainSingle(because: "the single persisted rule yields one read model");
		models[0].Convertible.Should().BeTrue(because: "formula set-value sources are representable by the friendly contract");
		SetValuesBusinessRuleAction action = models[0].Rule!.Actions.Single()
			.Should().BeOfType<SetValuesBusinessRuleAction>(
				because: "the persisted set-values action type maps back to the friendly action").Subject;
	...
	public void Read_Should_Convert_Const_Expression_When_Server_Omits_Type_Marker() {
		...
		// Assert
		BusinessRuleReadModel model = models.Should().ContainSingle(
				because: "the server-normalized rule is a single parent rule")
			.Which;
		model.Convertible.Should().BeTrue(
			because: "a Const value expression without the friendly 'type' marker must still convert via its persisted typeName");
		BusinessRuleExpression rightExpression = model.Rule!.Condition.Conditions[0].RightExpression!;
	...
	public void Read_Should_Map_Absent_ComparisonType_To_IsNotFilledIn() {
		...
		BusinessRuleReadModel model = models.Should().ContainSingle(
			...
			.Which;
		model.Convertible.Should().BeTrue(
			because: "a persisted is-not-filled-in condition stripped of its zero comparisonType must still convert");
		model.Rule!.Condition.Conditions[0].ComparisonType.Should().Be("is-not-filled-in",
	...
	public void Read_Should_Derive_ApplyFilter_Flags_From_Child_Rules_When_Server_Drops_Flags() {
		...
		BusinessRuleReadModel model = models.Should().ContainSingle(
			...
			.Which;
		model.Convertible.Should().BeTrue(
			because: "the persisted apply-filter parent is representable in the friendly contract");
		ApplyFilterBusinessRuleAction action = model.Rule!.Actions.OfType<ApplyFilterBusinessRuleAction>().Should()
	...
	[Test]
	[Category("Unit")]
	[Description("Populates conversionError alongside raw metadata when a rule cannot be mapped to the friendly contract, so callers can see why without diffing raw metadata.")]
	public void Read_Should_Populate_ConversionError_When_Rule_Is_Not_Convertible() {
		// Arrange
		JsonArray rules = ParseRules("""
			[{
				"typeName": "Terrasoft.Core.BusinessRules.BusinessRule",
		...
		// Assert
		BusinessRuleReadModel model = models.Should().ContainSingle(
				because: "the non-convertible rule must still be surfaced")
			.Which;
		model.Convertible.Should().BeFalse(
			because: "the ValidateElement action type is not representable in the friendly contract");
		model.Raw.Should().NotBeNull(
			because: "non-convertible rules carry their raw persisted metadata");
		model.ConversionError.Should().Contain("BusinessRuleActionValidateElement",
			because: "the conversion error should name the unsupported construct so callers can act on it");
	}
