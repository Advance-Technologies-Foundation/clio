...
public sealed class DbHubConnectionSourceFactoryTests : BaseClioModuleTests {
	...
	[Test]
	[Description("Detects PostgreSQL from a generic db connection string containing PostgreSQL-specific fields.")]
	public void Create_ShouldReadPostgresSource_WhenGenericDbKeyUsesPostgresShape() {
		// Arrange
		WriteConfig("db", "Server=pg.local;Port=5544;Database=creatio;User ID=app;Password=secret");

		// Act
		DbHubSourceDiscoveryResult result = _sut.Create("Generic Postgres", new EnvironmentSettings {
			EnvironmentPath = _directory
		});

		// Assert
		result.Success.Should().BeTrue(because: "the connection-string shape identifies PostgreSQL independently of its XML key");
		result.Source.Type.Should().Be("postgres", because: "PostgreSQL-specific fields must select the Npgsql parser");
	}
	...
	[Test]
	[Description("Supports SQL authentication connection strings generated by clio without explicit TLS options.")]
	public void Create_ShouldReadSqlServerSource_WhenGeneratedTlsOptionsAreOmitted() {
		// Arrange
		WriteConfig("db", "Data Source=sql.local;Initial Catalog=creatio;User ID=sa;Password=secret");

		// Act
		DbHubSourceDiscoveryResult result = _sut.Create("Generated SQL", new EnvironmentSettings {
			EnvironmentPath = _directory
		});

		// Assert
		result.Success.Should().BeTrue(because: "clio-generated SQL authentication settings are eligible for dbHub synchronization");
		result.Source.SslMode.Should().Be("require", because: "omitted TLS options must not downgrade database traffic to plaintext");
	}
