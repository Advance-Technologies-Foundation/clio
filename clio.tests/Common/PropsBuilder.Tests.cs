...
public class PropsBuilder_Tests
{
    ...
    public void Test1()
    {
        ...
        _fileSystem.GetFiles(Arg.Is(ExpectedPath("net472")),
            Arg.Is("*.dll"),
            Arg.Is(SearchOption.TopDirectoryOnly)).Returns(files);
        _fileSystem.GetFiles(Arg.Is(ExpectedPath("netstandard")),
            Arg.Is("*.dll"),
            Arg.Is(SearchOption.TopDirectoryOnly)).Returns(files);
        ...
        //Assert
        _fileSystem.Received(1).GetFiles(Arg.Is(ExpectedPath("net472")),
            Arg.Is("*.dll"),
            Arg.Is(SearchOption.TopDirectoryOnly));

        _fileSystem.Received(1).GetFiles(Arg.Is(ExpectedPath("netstandard")),
            Arg.Is("*.dll"),
            Arg.Is(SearchOption.TopDirectoryOnly));
