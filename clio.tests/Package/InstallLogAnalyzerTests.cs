...
public sealed class InstallLogAnalyzerTests {
	...
	[Test]
	[Description("GH-1299: a completed run that skipped a locally modified schema AND failed to compile is not downgraded, so a broken configuration never exits zero.")]
	public void ShouldTreatAsSuccess_ShouldReturnFalse_WhenTheRunAlsoCarriesACompilerError() {
		// Arrange - all four original terms hold; only the compiler diagnostic is added.
		string log = LocallyModifiedLog
			+ "2026-09-05 21:18:24,665 Errors and (or) warnings occurred while compiling configuration dll\r\n"
			+ "2026-09-05 21:18:24,665 Autogenerated\\Src\\UsrI1299BadSvc.UsrI1299B.cs(1,79) error CS1519:"
			+ " Invalid token '!=' in a member declaration\r\n";

		// Act
		bool treatAsSuccess = InstallLogAnalyzer.ShouldTreatAsSuccess(
			CreateResponse("Exception", "Packages installation failed"), log, false);

		// Assert
		treatAsSuccess.Should().BeFalse(
			because: "push-pkg installs with ContinueIfError=true by default, so one archive can carry a skipped schema and a package that fails to compile and still reach the completion marker - measured on Creatio 10.1.725, where clio reported that run as a success while the configuration no longer compiled (GH-1299)");
	}

	[Test]
	[Description("GH-1299: unrelated error text and compiler warnings in the shared installation log do not block the downgrade, because a healthy run carries both.")]
	public void ShouldTreatAsSuccess_ShouldReturnTrue_WhenTheLogCarriesUnrelatedErrorTextAndWarningsOnly() {
		// Arrange - the exact noise a successful run produced on Creatio 10.1.725.
		string log = LocallyModifiedLog
			+ "2026-09-05 21:16:16,882 Error while saving the metadata of application AutoTestUC from packages"
			+ " 'AutoTestUC' into DB\r\n"
			+ "Terrasoft.Common.ValidateException: Package with UId: 19aca643-9554-136f-aeca-8860ae2ac575 is not"
			+ " found in system\r\n"
			+ "2026-09-05 21:16:55,145 Errors and (or) warnings occurred while compiling configuration dll\r\n"
			+ "2026-09-05 21:16:55,145 ChannelPerformanceStatusSchema.CrtBaseAICommunication.cs(455,23) warning"
		...
		// Assert
		treatAsSuccess.Should().BeTrue(
			because: "the installation log is shared across the packages of a run and a healthy install routinely carries foreign error text plus warning-only compiler output - refusing on any error text would refuse the classification almost always, so only a CS diagnostic of severity error counts (GH-1299)");
	}

	[Test]
	[Description("GH-1299: a run refused because it failed to compile names the compiler diagnostic instead of echoing the platform's generic message.")]
	public void DescribeFailure_ShouldNameTheCompilerDiagnostic_WhenTheGenericFailureLogCarriesACompilerError() {
		// Arrange
		string log = LocallyModifiedLog
			+ "2026-09-05 22:00:52,665 Autogenerated\\Src\\UsrI1299BadSvc.UsrI1299B.cs(1,79) error CS1519:"
			+ " Invalid token '!=' in a member declaration\r\n";

		// Act
		string reason = InstallLogAnalyzer.DescribeFailure(
			CreateResponse("Exception", "Packages installation failed"), log);

		// Assert
		reason.Should().Contain("error CS1519",
			because: "the compiler line names the schema, the position and the diagnostic, which is what the operator has to act on");
		reason.Should().NotContain("Packages installation failed",
			because: "the platform answers a failed compilation with the same generic message it answers a skipped schema with, so echoing it tells the operator nothing (measured on Creatio 10.1.725)");
	}

	[Test]
	[Description("A response that names its own reason keeps that reason, even when the shared log also carries a compiler error from another package.")]
	public void DescribeFailure_ShouldKeepTheReportedReason_WhenTheResponseNamesASpecificFailure() {
		// Arrange
		string log = "SomeOther.cs(1,1) error CS1519: Invalid token\r\n";

		// Act
		string reason = InstallLogAnalyzer.DescribeFailure(
			CreateResponse("Exception", "Cannot compile configuration"), log);

		// Assert
		reason.Should().Be("Exception: Cannot compile configuration",
			because: "a specific server reason is more precise than a line scraped out of an environment-wide log, so it must not be overridden");
	}

	[Test]
	[Description("A compiler diagnostic of severity error is recognized, while warnings and unrelated error prose are not.")]
	public void HasCompilationFailure_ShouldOnlyMatchCompilerErrors_WhenLogAlsoCarriesWarningsAndErrorProse() {
		// Act
		bool fromError = InstallLogAnalyzer.HasCompilationFailure(
			"UsrI1299BadSvc.UsrI1299B.cs(1,79) error CS1519: Invalid token '!=' in a member declaration");
		bool fromWarning = InstallLogAnalyzer.HasCompilationFailure(
			"ChannelPerformanceStatusSchema.cs(455,23) warning CS0114: hides inherited member.");
		bool fromProse = InstallLogAnalyzer.HasCompilationFailure(
			"Errors and (or) warnings occurred while compiling configuration dll\r\n"
			+ "Error while saving the metadata of application AutoTestUC into DB");
		bool fromBlank = InstallLogAnalyzer.HasCompilationFailure("   ");

		// Assert
		fromError.Should().BeTrue(because: "a CS diagnostic of severity error is emitted only for a build that failed");
		fromWarning.Should().BeFalse(because: "a healthy configuration build routinely emits CS warnings");
		fromProse.Should().BeFalse(
			because: "the platform prints the \"Errors and (or) warnings\" line for warning-only builds and logs unrelated saving errors on healthy runs");
		fromBlank.Should().BeFalse(because: "a blank log carries no evidence and must not throw");
	}
