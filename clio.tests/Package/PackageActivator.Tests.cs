    ...
    public void Activate_ActivatesPackageByName()
    {
        ...
        PackageActivationResponse activationResponse = new()
        {
            ...
            [
                ...
                {
                    PackageName = packageName, Success = true
                },

                new PackageActivationResultDto
                {
                    PackageName = packageName1, Success = true, Message = "SomeError"
                },

                new PackageActivationResultDto
                {
                    PackageName = packageName2, Success = false
                }
            ]
        };
        ...
        packageActivationResults.First(result =>
                                    result.PackageName == packageName && result.Success &&
                                    string.IsNullOrEmpty(result.Message)).Should()
                                .NotBeNull();

        packageActivationResults.First(result =>
                                    result.PackageName == packageName1 && result.Success &&
                                    !string.IsNullOrEmpty(result.Message)).Should()
                                .NotBeNull();
        packageActivationResults.First(result =>
