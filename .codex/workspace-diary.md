...
## 2026-03-26 09:12 – Publish 8.0.2.41 release notes
Context: User asked to wait for PR `#485` workflows, cut the next release, and ensure the GitHub release had a proper description instead of the autogenerated placeholder.
Decision: Verified the release body on GitHub, replaced the placeholder text with curated notes summarizing the schema/MCP changes shipped in `8.0.2.41`, and linked the merged PR.
Discovery: The generated release for tag `8.0.2.41` was published with the default body `Automated release 8.0.2.41`, so release notes need a manual follow-up step even when the tag and release are created correctly.
Files: clio/clio.csproj, .codex/workspace-diary.md
Impact: Future release work can reuse the reminder that the GitHub release body should be checked and curated after publishing, rather than assuming the autogenerated text is sufficient.
...
## 2026-03-26 22:39 – Document external-AI view of MCP capabilities
Context: User asked for a detailed document describing how a third-party AI sees the current clio MCP server in terms of capabilities.
Decision: Audited the registered MCP surface from source and added a dedicated capability-map document that explains discovery counts, execution model, domain groupings, prompts/resources, and the main integration rough edges for external AI clients.
Discovery: The current source-registered surface exposes 60 tools, 50 prompts, and 3 resources; page/schema/workspace flows are the strongest AI-facing areas, while runtime lifecycle tools still show legacy inconsistencies such as mixed naming, mixed argument shapes, and missing explicit safety metadata.
Files: docs/McpCapabilityMap.md, clio/BindingsModule.cs, clio/Command/McpServer/Tools, clio/Command/McpServer/Prompts, clio/Command/McpServer/Resources, .codex/workspace-diary.md
Impact: Future onboarding of external MCP clients can start from a source-backed capability map instead of reverse-engineering the surface from scattered tools, prompts, and help resources.

## 2026-03-26 22:55 – Move app-modeling guidance into MCP-owned resources
Context: User asked which orchestrator instructions should move down to the clio MCP layer and suggested starting from the MCP capability-map document.
Decision: Added a new MCP resource `docs://mcp/guides/app-modeling`, updated application/entity prompts to reference it and to spell out canonical-entity, lookup-display, and default-semantics rules, added unit coverage for the new guidance surface, and updated the capability map with an instruction-ownership section.
Discovery: The guidance best suited for MCP ownership is the stable contract around stdio transport, kebab-case args, DB-first schema semantics, batch-first page/schema workflows, application-create canonical-entity behavior, BaseLookup display rules, and explicit default semantics; repository-specific plan/gate/output rules should stay outside clio.
Files: clio/Command/McpServer/Resources/AppModelingGuidanceResource.cs, clio/Command/McpServer/Prompts/ApplicationPrompt.cs, clio/Command/McpServer/Prompts/EntitySchemaPrompt.cs, clio.tests/Command/McpServer/McpGuidanceResourceTests.cs, clio.tests/Command/McpServer/ApplicationToolTests.cs, clio.tests/Command/McpServer/EntitySchemaToolTests.cs, docs/McpCapabilityMap.md, .codex/workspace-diary.md
Impact: Future MCP clients can discover core app-modeling guardrails from the server itself instead of depending on duplicated consumer-repository instructions.
## 2026-03-26 23:20 – Enforce modeling guardrails in MCP tool responses and lookup creation
Context: User asked to continue from the MCP guidance refactor and implement the server-side contract changes rather than limiting the work to prompts/resources.
Decision: Added shared modeling guardrails for canonical main entity resolution and BaseLookup inherited-column validation, surfaced `canonical-main-entity-name` in application context responses, and extended unit plus MCP E2E coverage for `application-create`, `create-lookup`, and `schema-sync`.
Discovery: The inherited-column guardrail can be verified without a reachable environment because validation runs before environment resolution; local `clio.mcp.e2e` execution is still blocked here because the project targets `net10.0` while the installed SDK is `8.0.124`.
Files: clio/Command/McpServer/Tools/ApplicationToolResponses.cs, clio/Command/McpServer/Tools/ApplicationToolSupport.cs, clio/Command/McpServer/Tools/EntitySchemaTool.cs, clio/Command/McpServer/Tools/SchemaSyncTool.cs, clio/Command/McpServer/Tools/ModelingGuardrails.cs, clio.tests/Command/McpServer/ApplicationToolTests.cs, clio.tests/Command/McpServer/CreateEntitySchemaToolTests.cs, clio.tests/Command/McpServer/SchemaSyncToolTests.cs, clio.mcp.e2e/ApplicationToolE2ETests.cs, clio.mcp.e2e/Support/Results/ApplicationEnvelope.cs, clio.mcp.e2e/EntitySchemaToolE2ETests.cs, clio.mcp.e2e/SchemaSyncToolE2ETests.cs, .codex/workspace-diary.md
Impact: MCP clients now receive explicit canonical main entity metadata from application context tools, and both direct lookup creation and batched schema-sync flows fail fast when callers try to redefine inherited BaseLookup fields.

## 2026-03-27 00:41 – Review clio and ADAC contract alignment
Context: User asked to review whether the current `ENG-87492-Alfa-version-03-26` branch stays consistent between `clio` and `ai-driven-app-creation` (ADAC).
Decision: Compared the branch diffs in both repositories, verified the changed unit/doc tests locally, and reviewed the ADAC wrapper rules against the new `clio` MCP contract and guardrails.
Discovery: `clio` correctly enforces inherited `BaseLookup` column rejection and exposes optional `canonical-main-entity-name`, but ADAC still silently strips `Name` in lookup sync planning instead of surfacing the new server-side validation and still hard-requires `package-name` for `page-list` even though the current `clio` tool marks all page-list args optional; local `clio.mcp.e2e` verification remains blocked on this machine by the missing .NET 10 SDK.
Files: .codex/workspace-diary.md, /Users/a.kravchuk/Projects/ai-driven-app-creation/scripts/mcp_schema_sync.py, /Users/a.kravchuk/Projects/ai-driven-app-creation/tests/test_mcp_schema_sync.py, /Users/a.kravchuk/Projects/ai-driven-app-creation/scripts/mcp_client.py, /Users/a.kravchuk/Projects/ai-driven-app-creation/tests/test_mcp_client.py, clio/Command/McpServer/Tools/EntitySchemaTool.cs, clio/Command/McpServer/Tools/SchemaSyncTool.cs, clio/Command/McpServer/Tools/PageListTool.cs
Impact: Future ADAC alignment work should update wrapper validation and lookup planning semantics before relying on the branch as a fully synchronized MCP contract pair.

## 2026-03-27 01:17 – Verify ADAC contract fixes against current clio branch
Context: User asked to apply the required consistency fixes after the cross-repo review of `clio` and ADAC.
Decision: Kept `clio` code unchanged, confirmed the ADAC worktree now matches the current `clio` MCP contract for optional `page-list` arguments and fail-fast lookup guardrails, and re-ran the targeted Python and .NET test slices that cover those contracts.
