...
## 2026-06-08 00:00 – Routing fix: relative-date/time & gated lookup restrictions → apply-static-filter
Context: For prompts whose lookup restriction uses a computed/relative date-time ("birthdays tomorrow", "created previous quarter + 11:06"), the model misrouted to a page handler (crt.InitRequest) instead of create-entity-business-rule + apply-static-filter. Root cause: every routing example in the MCP surface was attributive (Type=Customer, Age=30, mobile phone); no relative-date/time example, and the adjacent "business logic / side effects → handlers" line acted as a false attractor. Filter CONSTRUCTION was already correct (business-rule-filters guide) — only ROUTING text was the gap. Code/builder/tests untouched.
Decision: One generalized lookup-restriction routing insertion (covers the whole relative-date/time + gated class) added in 4 places, plus a handler-row carve-out so the handler trigger explicitly excludes lookup record-set restriction.
Files:
 - clio/Command/McpServer/Tools/PageGetTool.cs (GetPage [Description]: added lookup-restriction relative-date/gated clause + handler-row carve-out)
 - clio/Command/McpServer/Tools/PageUpdateTool.cs (UpdatePage [Description]: appended relative-date/gated sentence after the existing apply-static-filter routing; handler-row carve-out)
 - clio/Command/McpServer/Resources/BusinessRulesGuidanceResource.cs (new sub-bullet under the apply-static-filter decision row)
 - clio/Command/McpServer/Resources/PageModificationGuidanceResource.cs (extended lookup-restriction gate-row "Why" cell + handler-row NOTE carve-out)
 - clio.tests/Command/McpServer/McpGuidanceResourceTests.cs (+2 assertions on BusinessRules guide), PageToolsTests.cs (+2 assertions each on PageGet/PageUpdate descriptions)
Discovery: tool [Description] strings are concatenated regular C# strings (escape internal quotes as \"; file mixes — and literal — , both compile); the two resources are C# 11 raw string literals indented \t\t+7 spaces (rows/top bullets) / \t\t+9 spaces (sub-bullets), using literal — (U+2014) and → (U+2192). Inside the markdown-ish tables in PageModificationGuidanceResource, never use a literal | in inserted cell text. Existing guidance/description unit tests are all Should().Contain (additive-safe); no verbatim full-text equality asserted anywhere, so insertions don't break them. clio.tests targets net10.0 only.
Build/verify: net10.0 build clean; net8.0 COMPILED clean (obj DLL built) — only bin copy was blocked by 3 live .NET Host MCP servers (clio→net8.0) holding the old DLL; restart the plugin to pick up the new text. McpServer module unit tests: 1006/1006 pass. MCP reviewed: tool descriptions + both guidance resources updated and kept in sync (per MCP maintenance policy); unit coverage added; no behavior/arg/flag/destructive change, so no e2e contract change (existing e2e advertises business-rules URI but does not assert body text). Live stand verification of prompts #4/#11/#1 pending MCP restart.
Impact: Closes the whole relative-date/time + gated lookup-restriction misroute class with one generalized formulation; future prompts of this shape (next 7 days, last quarter, time-of-day) are covered without per-example additions.

## 2026-06-08 02:00 – Feature: datePart (time-of-day / fixed calendar part) in apply-static-filter friendly contract
Context: Follow-up from evaluating a Sonnet-4.6 run on "contacts created at 11:06 AM and previous quarter". Routing was correct (apply-static-filter, no handler detour — the earlier ENG-88588 guidance fix working), but the agent DROPPED the 11:06 AM constraint and declared time-of-day "unsupported / needs C# override". Verified that was TRUE for the friendly contract: StaticFilterDeserializer leaf read only columnPath/comparisonType/value/valueMacros/valueMacrosArgument; LocalEsqFilterBuilder emitted only IsNull/Macros/Lookup-IN/scalar — EsqFunctionType.DatePart=3 + EsqDataValueType.Time=9 existed as enums but were never emitted. So a guide-only fix would have been actively misleading. User chose "implement the feature".
Decision: Add a `datePart` leaf field to the friendly apply-static-filter contract that emits a DatePart function (functionType 3) leftExpression extracting a calendar/clock part, compared to a constant. datePartType map taken from the already-authoritative ESQ guides (EsqGuidanceResource / EsqFiltersGuidanceResource): 1 Day, 2 Week, 3 Month, 4 Year, 5 Weekday, 6 Hour, 7 HourMinute. Integer parts (Day/Week/Month/Year/Weekday/Hour) → Integer(4) right param; HourMinute (alias "Time") → Time(9) right param. The HourMinute emission mirrors the user's verified-working platform sample (leaf dataValueType=column code, isAggregative=false, trimDateTimeParameterToDate=true); integer parts mirror the documented year-2021 sample (no leaf dataValueType, no trim).
Files (code): clio/Command/BusinessRules/Filters/Esq/EsqFilterEnums.cs (new EsqDatePartType enum), DatePartCatalog.cs (new; name→type+valueKind), StaticFilterDtos.cs (StaticFilterLeaf.DatePart), StaticFilterDeserializer.cs (read datePart), StaticFilterStructuralValidator.cs (ValidateDatePart: mutually-exclusive-with-macros, value required, equality/relational only, known name, Integer-vs-Time value kind), Schema/SchemaAwareFilterValidator.cs (ValidateDatePartColumn: Date/DateTime/Time only; skips column-typed scalar check for datePart leaves), Esq/EsqEnvelopeDtos.cs (EsqCompareFilterDto.LeftExpression → object for polymorphism; new EsqDatePartFunctionExpressionDto), Esq/LocalEsqFilterBuilder.cs (BuildDatePartCompareFilter, branch before macros).
Files (MCP surface): Resources/BusinessRuleFiltersGuidanceResource.cs (datePart leaf bullet + temporal-translation line: exact time-of-day IS expressible, do not drop), Tools/ToolContractGetTool.cs (apply-static-filter-group Context now documents datePart with 11:06 + year-2021 examples). Tool-contract examples-array entry skipped (tab-matching friction); Context covers it. MCP reviewed: contract + guidance aligned; create-entity-business-rule tool consumes the same pipeline so no tool-code change needed.
Files (tests): clio.tests/Command/BusinessRules/LocalEsqFilterBuilderTests.cs (+8: HourMinute emit, Year integer emit, PreviousQuarter+HourMinute combined under AND, schema-reject datePart on Text, structural-reject unknown part / HourMinute-with-number / Year-with-string / datePart+macros).
Discovery: friendly contract leaf is the ONLY thing the model authors — adding builder support is mandatory for the capability; guide text alone is inert because the deserializer silently ignores unknown leaf fields. System.Text.Json serializes a property declared `object` by runtime type (already relied on by RightExpression), so widening LeftExpression to object is safe and needs no converter. datePartType is owned by devkit ɵDatePartType; values cross-checked against the in-repo ESQ guides rather than guessed.
Build/verify: net10.0 + net8.0 build clean (net8.0 required stopping the live MCP servers again — restart the clio plugin). Targeted tests Module=Command|McpServer: 2350 passed / 11 skipped / 0 failed. CAVEAT (flagged for live gate): the HourMinute Time-parameter wire shape is emitted as the builder's existing Time form ({dataValueType:9, value:"HH:mm:ss"}) plus trim=true to mirror the user's working sample; the Freedom UI Designer emits a richer dateValue+escaped-value Time parameter — final confirmation that the platform accepts the builder's form for a HourMinute DatePart comparison needs one live create-entity-business-rule on the stand (prompt #2). Integer parts (year/month/day/hour) match the already-documented ESQ pattern and are lower-risk.
Impact: closes "created at 11:06 AM" and the whole fixed-clock/calendar-part class (calendar year, each Nth, each weekday, specific month/hour) in the no-code apply-static-filter surface; removes the false "time-of-day unsupported" dead-end the model hit.
