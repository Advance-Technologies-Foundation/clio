...
## 2026-05-26 00:00 – Resolve PR 624 merge conflicts with origin/master
Context: A merge of origin/master into the PR #624 worktree failed across MCP routing files after master added mobile-page guidance, OData read exposure, and MCP deserialization handling.
Decision: Kept the PR #624 clio-run architecture and renamed run-args types, then selectively merged master-only additions: business-rule guidance text, OData read discovery, and MCP serializer/filter wiring.
