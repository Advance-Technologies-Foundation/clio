...
## 2026-04-22 16:59 – Move canonical binding guidance into clio and slim ADAC to consumer policy
Context: Implemented the binding refactor plan so `clio` owns executable binding guidance, assets, and contract surfaces while `ai-driven-app-creation` stops acting like the source of truth for generic binding behavior.
Decision: Added `docs://mcp/guides/data-bindings`, extended `get-tool-contract` to advertise local binding tools, created the installable `clio-data-bindings` skill/assets, and rewrote ADAC binding docs as policy-only while deleting repo-local stable-ID and generic template copies.
Discovery: `clio.mcp.e2e` prefers `C:\ADAC\clio\clio\bin\Debug\net10.0\clio.exe` when it exists, so a stale long-lived `clio.exe mcp` process can hide newly added MCP resources until the Debug binary is rebuilt.
Files: C:\ADAC\clio\clio\Command\McpServer\Resources\DataBindingsGuidanceResource.cs, C:\ADAC\clio\clio\Command\McpServer\Tools\ToolContractGetTool.cs, C:\ADAC\clio\.agents\skills\clio-data-bindings\SKILL.md, C:\ADAC\ai-driven-app-creation\skills\data-bindings-creation\SKILL.md, C:\ADAC\ai-driven-app-creation\context\data-bindings-reference.md, C:\ADAC\clio\.codex\workspace-diary.md
Impact: Binding workflow ownership is now centralized in `clio`, ADAC keeps only consumer-specific reporting obligations, and future MCP regressions are less likely to come from duplicated binding docs or assets.

## 2026-04-22 18:29 – Install Account binding on ts1_15070173_0422_check via local artifact flow
Context: Needed to create a data binding for `Account` with the row name `11111111111111` on `http://ts1-infr-web02:88/sae_m_seeenu_15070173_0422` without an existing custom package on the stand.
Decision: Created a temporary workspace and package `PkgAccBind0422`, pushed it to `ts1_15070173_0422_check`, enabled hotfix, generated a local `Account` binding with only `Id` and `Name`, and pushed the workspace again so Creatio installed the data.
Discovery: `create-data-binding-db` fails for `Account` because the runtime schema includes unsupported `dataValueType` `16`, but local `create-data-binding` succeeds when `values` limits the descriptor to the explicitly provided columns plus the primary key.
Files: C:\ADAC\tmp\account-binding-15070173-0422\ws-account-binding\packages\PkgAccBind0422\Data\Account\descriptor.json, C:\ADAC\tmp\account-binding-15070173-0422\ws-account-binding\packages\PkgAccBind0422\Data\Account\data.json, C:\ADAC\clio\.codex\workspace-diary.md
Impact: Future binding requests for complex system objects can fall back to the local artifact workflow when DB-first schema generation rejects unsupported runtime column types.

## 2026-04-22 18:35 – Add Account binding row to package Custom on ts1_15070173_0422_check
Context: Needed to add an `Account` data binding row with `Name = 2222` specifically into package `Custom` on `http://ts1-infr-web02:88/sae_m_seeenu_15070173_0422`.
Decision: Created a temporary empty workspace, pulled package `Custom` into it with `add-package Custom -e ts1_15070173_0422_check`, generated a local `Account` binding constrained to `Id` and `Name`, and pushed the workspace back to the same environment.
Discovery: `add-package <Package> -e <env>` can materialize package `Custom` into a temp workspace even when commands that depend on `cliogate` are unavailable; the subsequent package install log explicitly confirms both `Schema data "Account" saved in package "Custom"` and `Data "Account" from package "Custom" installed`.
Files: C:\ADAC\tmp\custom-account-binding-15070173-0422\ws-custom-account-binding\packages\Custom\descriptor.json, C:\ADAC\tmp\custom-account-binding-15070173-0422\ws-custom-account-binding\packages\Custom\Data\Account\descriptor.json, C:\ADAC\tmp\custom-account-binding-15070173-0422\ws-custom-account-binding\packages\Custom\Data\Account\data.json, C:\ADAC\clio\.codex\workspace-diary.md
Impact: Future requests to place bindings into an existing remote package can reuse the targeted temp-workspace flow instead of creating a throwaway package on the stand.

## 2026-04-22 20:10 – Project DB-first binding metadata to referenced columns and align MCP guidance
Context: `create-data-binding-db` and the DB-first row-maintenance commands were still rebuilding SaveSchema metadata from the full runtime schema, so `Account`-like schemas failed on unrelated unsupported runtime column types and the MCP guidance surface still implied install-log-only verification.
Decision: Changed `DataBindingDbService` to validate only requested binding columns, project SaveSchema metadata to `Id` plus columns referenced by bound/requested rows, preserve that projection across create/upsert/remove flows, and update MCP prompts/contracts/guidance/help/skill docs to describe read-back verification and the subset-column rule. Added unit coverage for unused-vs-requested unsupported runtime columns plus DB-first upsert/remove projection, and added an E2E regression for the `Account` path.
Discovery: The MCP E2E suite reads the `Debug` `clio.exe` when it exists, so stale `clio.exe mcp` processes can make resource/contract tests report old guide text until those processes are stopped and the Debug binary is rebuilt.
Files: C:\ADAC\clio\clio\Command\DataBindingDbCommand.cs, C:\ADAC\clio\clio.tests\Command\DataBindingDbCommandTests.cs, C:\ADAC\clio\clio.mcp.e2e\DataBindingDbToolE2ETests.cs, C:\ADAC\clio\clio\Command\McpServer\Prompts\DataBindingDbPrompt.cs, C:\ADAC\clio\clio\Command\McpServer\Resources\DataBindingsGuidanceResource.cs, C:\ADAC\clio\clio\Command\McpServer\Tools\ToolContractGetTool.cs, C:\ADAC\clio\clio.tests\Command\McpServer\DataBindingDbToolTests.cs, C:\ADAC\clio\clio.tests\Command\McpServer\McpGuidanceResourceTests.cs, C:\ADAC\clio\clio.tests\Command\McpServer\ToolContractGetToolTests.cs, C:\ADAC\clio\clio.mcp.e2e\McpGuidanceResourceE2ETests.cs, C:\ADAC\clio\clio.mcp.e2e\ToolContractGetToolE2ETests.cs, C:\ADAC\clio\clio\help\en\create-data-binding-db.txt, C:\ADAC\clio\clio\help\en\upsert-data-binding-row-db.txt, C:\ADAC\clio\clio\help\en\remove-data-binding-row-db.txt, C:\ADAC\clio\clio\docs\commands\create-data-binding-db.md, C:\ADAC\clio\clio\docs\commands\upsert-data-binding-row-db.md, C:\ADAC\clio\clio\docs\commands\remove-data-binding-row-db.md, C:\ADAC\clio\.github\skills\clio\SKILL.md, C:\ADAC\clio\.github\skills\clio\references\commands-reference.md, C:\ADAC\clio\.codex\workspace-diary.md
Impact: DB-first bindings no longer fall back to local artifacts just because the runtime schema contains unrelated unsupported columns, and callers now get consistent contract/guidance text that pushes them toward remote read-back instead of trusting install logs.

## 2026-04-23 00:00 – Re-review binding split and plan base-vs-section refactor
Context: Reviewed the pending `clio` binding refactor after feedback that the new base binding skill should stay generic and should not absorb section-registration guidance as part of the default mechanism.
Decision: Keep `data-bindings` as the base mechanism only, then extract section-registration behavior, stable IDs, invariants, and examples into a dedicated specialized surface instead of teaching them through the base skill and base guidance resource.
Discovery: The current implementation and the driving refactor docs both encode section registration into the base layer: `.agents/skills/clio-data-bindings`, `docs://mcp/guides/data-bindings`, prompt/tool-contract routing text, and `refactoring/02` through `refactoring/05`. Also, `spec/data-binding/data-binding-template-catalog.md` still documents only `SysSettings` even though runtime/tooling now treat `SysModule` as a built-in offline template.
Files: C:\ADAC\clio\.agents\skills\clio-data-bindings\SKILL.md, C:\ADAC\clio\.agents\skills\clio-data-bindings\references\workflow.md, C:\ADAC\clio\.agents\skills\clio-data-bindings\references\section-registration-invariants.md, C:\ADAC\clio\clio\Command\McpServer\Resources\DataBindingsGuidanceResource.cs, C:\ADAC\clio\clio\Command\McpServer\Prompts\DataBindingPrompt.cs, C:\ADAC\clio\clio\Command\McpServer\Prompts\DataBindingDbPrompt.cs, C:\ADAC\clio\clio\Command\McpServer\Tools\ToolContractGetTool.cs, C:\ADAC\clio\.github\skills\clio\SKILL.md, C:\ADAC\clio\.github\skills\clio\references\commands-reference.md, C:\ADAC\refactoring\02-clio-binding-guidance-resource-spec.md, C:\ADAC\refactoring\03-clio-data-bindings-skill-spec.md, C:\ADAC\refactoring\04-clio-binding-assets-migration-spec.md, C:\ADAC\refactoring\05-clio-prompt-and-contract-alignment-spec.md, C:\ADAC\clio\spec\data-binding\data-binding-template-catalog.md, C:\ADAC\clio\.codex\workspace-diary.md
Impact: Future implementation can start from the corrected ownership split and avoid reintroducing section-specific rules into the generic binding mechanism.

## 2026-04-23 01:15 – Split base binding guidance from section-registration specialization and align tests
Context: Implemented the planned base-vs-section refactor so the generic `data-bindings` layer stops teaching `SysModule` / `SysModuleEntity` invariants directly while still remaining the entrypoint for workflow selection.
Decision: Slimmed `.agents/skills/clio-data-bindings` down to generic workflow guidance, added `.agents/skills/clio-section-registration` for section-specific invariants/assets/examples, added `docs://mcp/guides/section-registration`, updated `GuidanceCatalog`, `AppModelingGuidanceResource`, `ExistingAppMaintenanceGuidanceResource`, prompts, tool-contract wording, help/docs, and `.github` skill routing to point at the correct base or specialization surface. Also updated `spec/data-binding/data-binding-template-catalog.md` to document `SysModule` as a built-in template, rewrote `refactoring/00` through `refactoring/05` to encode the new split, and refreshed unit/E2E guidance tests for both guides.
Discovery: The guidance and tool tests were the fastest way to lock the architecture boundary: `data-bindings` now asserts only generic flow rules plus a redirect to `section-registration`, while the new section guide owns `CardSchemaUId`, `SectionSchemaUId`, stable-ID asset sourcing, and repository example pointers. Targeted tests passed after the split without further runtime fixes.
Files: C:\ADAC\clio\.agents\skills\clio-data-bindings\SKILL.md, C:\ADAC\clio\.agents\skills\clio-data-bindings\references\workflow.md, C:\ADAC\clio\.agents\skills\clio-section-registration\SKILL.md, C:\ADAC\clio\.agents\skills\clio-section-registration\references\section-registration-invariants.md, C:\ADAC\clio\.agents\skills\clio-section-registration\assets\bindings-lookup.json, C:\ADAC\clio\clio\Command\McpServer\Resources\DataBindingsGuidanceResource.cs, C:\ADAC\clio\clio\Command\McpServer\Resources\SectionRegistrationGuidanceResource.cs, C:\ADAC\clio\clio\Command\McpServer\Resources\GuidanceCatalog.cs, C:\ADAC\clio\clio\Command\McpServer\Resources\AppModelingGuidanceResource.cs, C:\ADAC\clio\clio\Command\McpServer\Resources\ExistingAppMaintenanceGuidanceResource.cs, C:\ADAC\clio\clio\Command\McpServer\Prompts\DataBindingPrompt.cs, C:\ADAC\clio\clio\Command\McpServer\Prompts\DataBindingDbPrompt.cs, C:\ADAC\clio\clio\Command\McpServer\Tools\GuidanceGetTool.cs, C:\ADAC\clio\clio\Command\McpServer\Tools\ToolContractGetTool.cs, C:\ADAC\clio\clio\docs\commands\mcp-server.md, C:\ADAC\clio\clio\help\en\mcp-server.txt, C:\ADAC\clio\.github\skills\clio\SKILL.md, C:\ADAC\clio\.github\skills\clio\references\commands-reference.md, C:\ADAC\clio\spec\data-binding\data-binding-template-catalog.md, C:\ADAC\clio\clio.tests\Command\McpServer\GuidanceGetToolTests.cs, C:\ADAC\clio\clio.tests\Command\McpServer\McpGuidanceResourceTests.cs, C:\ADAC\clio\clio.tests\Command\McpServer\ToolContractGetToolTests.cs, C:\ADAC\clio\clio.mcp.e2e\GuidanceGetToolE2ETests.cs, C:\ADAC\clio\clio.mcp.e2e\McpGuidanceResourceE2ETests.cs, C:\ADAC\refactoring\00-binding-refactor-index.md, C:\ADAC\refactoring\01-binding-ownership-matrix.md, C:\ADAC\refactoring\02-clio-binding-guidance-resource-spec.md, C:\ADAC\refactoring\03-clio-data-bindings-skill-spec.md, C:\ADAC\refactoring\04-clio-binding-assets-migration-spec.md, C:\ADAC\refactoring\05-clio-prompt-and-contract-alignment-spec.md, C:\ADAC\clio\.codex\workspace-diary.md
Impact: `clio` now exposes a clean generic binding mechanism with an explicit section-registration specialization layered on top, and the supporting docs/tests/specs all reinforce the same architecture instead of pulling the codebase back toward the mixed model.

## 2026-04-23 01:28 – Remove section-specific routing from the base binding skill text
Context: Follow-up review found that the base `clio-data-bindings` skill still named concrete section-registration functionality inside its own text, which violated the requirement that the base skill stay generic.
Decision: Removed the direct `section-registration` / `SysModule` / `SysModuleEntity` routing line from `.agents/skills/clio-data-bindings/references/workflow.md` and rewrote `.agents/skills/clio-data-bindings/SKILL.md` so it refers only to separate specialized skills in generic terms.
Discovery: The only remaining specialization leakage inside the base bundle was textual guidance, not assets or examples; a targeted grep over `.agents/skills/clio-data-bindings` confirmed that no section-specific identifiers remain after the edit.
Files: C:\ADAC\clio\.agents\skills\clio-data-bindings\SKILL.md, C:\ADAC\clio\.agents\skills\clio-data-bindings\references\workflow.md, C:\ADAC\clio\.codex\workspace-diary.md
Impact: The base skill now reads as a pure mechanism layer and no longer hard-links itself to a concrete specialization scenario.

## 2026-04-23 02:10 – Remove section-registration specialization from clio binding surfaces entirely
Context: Follow-up direction changed the target split again: `clio` should no longer expose any dedicated `section-registration` guidance/skill/assets at all, and the repository should keep only the generic data-binding mechanism.
Decision: Removed the `section-registration` MCP guide and skill bundle, deleted the `SysModule` built-in binding template and section-specific sample package data, stripped `SysModule` / palette normalization logic from `DataBindingCommand`, rewrote prompts/contracts/help/docs around generic bindings only, and rebased binding tests onto generic runtime/local schemas such as `UsrLookupBinding` and `UsrImageBinding`.
Discovery: After removing the specialization and then cleaning up the leftover negative assertions, `section-registration` no longer appears anywhere in the repository. Remaining `SysModule` references belong to unrelated application-section create/delete behavior rather than the binding mechanism. A targeted unit rerun passed 39/39 after correcting the runtime image mock to use data value type `13`, the targeted MCP E2E rerun passed 24/24, and the final post-cleanup guidance subsets passed 6/6 unit plus 4/4 E2E.
Files: C:\ADAC\clio\clio\Command\DataBindingCommand.cs, C:\ADAC\clio\clio\Command\McpServer\Resources\DataBindingsGuidanceResource.cs, C:\ADAC\clio\clio\Command\McpServer\Resources\AppModelingGuidanceResource.cs, C:\ADAC\clio\clio\Command\McpServer\Resources\ExistingAppMaintenanceGuidanceResource.cs, C:\ADAC\clio\clio\Command\McpServer\Resources\GuidanceCatalog.cs, C:\ADAC\clio\clio\Command\McpServer\Tools\GuidanceGetTool.cs, C:\ADAC\clio\clio\Command\McpServer\Tools\DataBindingTool.cs, C:\ADAC\clio\clio\Command\McpServer\Prompts\DataBindingPrompt.cs, C:\ADAC\clio\clio\Command\McpServer\Tools\ToolContractGetTool.cs, C:\ADAC\clio\clio\docs\commands\mcp-server.md, C:\ADAC\clio\clio\docs\commands\create-data-binding.md, C:\ADAC\clio\clio\docs\commands\add-data-binding-row.md, C:\ADAC\clio\clio\help\en\mcp-server.txt, C:\ADAC\clio\clio\help\en\create-data-binding.txt, C:\ADAC\clio\clio\help\en\add-data-binding-row.txt, C:\ADAC\clio\.github\skills\clio\SKILL.md, C:\ADAC\clio\.github\skills\clio\references\commands-reference.md, C:\ADAC\clio\spec\data-binding\data-binding-template-catalog.md, C:\ADAC\clio\clio.tests\Command\DataBindingCommandTests.cs, C:\ADAC\clio\clio.tests\Command\McpServer\DataBindingToolTests.cs, C:\ADAC\clio\clio.tests\Command\McpServer\GuidanceGetToolTests.cs, C:\ADAC\clio\clio.tests\Command\McpServer\McpGuidanceResourceTests.cs, C:\ADAC\clio\clio.tests\Command\McpServer\ToolContractGetToolTests.cs, C:\ADAC\clio\clio.mcp.e2e\DataBindingToolE2ETests.cs, C:\ADAC\clio\clio.mcp.e2e\GuidanceGetToolE2ETests.cs, C:\ADAC\clio\clio.mcp.e2e\McpGuidanceResourceE2ETests.cs, C:\ADAC\clio\clio.mcp.e2e\ToolContractGetToolE2ETests.cs, C:\ADAC\clio\.codex\workspace-diary.md
Impact: The repository now keeps only the generic data-binding mechanism; section registration is no longer modeled as a first-class binding specialization inside `clio`, and the contract/doc/test surfaces all reinforce that narrower scope.
