...
## 2026-06-10 16:00 – find-app fast app discovery + unified actionable env-not-found (ENG-91275)
Context: ENG-91275 (sub-task of ENG-90506) — app-discovery phase took ~18 min. No semantic `find-app` (agent did N+1 list-apps + per-app list-app-sections), and the env-not-found error was the opaque "...not found. Check your clio configuration." Approach: code-delivery + short ADR (spec/find-app/find-app-adr.md), mirroring find-entity-schema.
Decision: New `FindAppCommand : Command<FindAppOptions>` (CLI verb `find-app`) + `FindAppTool : BaseTool<FindAppOptions>` (MCP, env-aware via ToolCommandResolver → so its env-not-found is the good message by construction). FindApplications issues exactly 2 SelectQuery calls — ALL SysInstalledApp rows + ALL ApplicationSection rows (no ApplicationId filter) — joins in memory by ApplicationId, filters by --search-pattern (substring over app name/code/description AND section caption/code) and/or exact --code; empty pattern → all apps with sections. Reused existing public record ApplicationSectionRecord for the section query DTO. New shared Clio.Common.EnvironmentNotFoundError.Build(name, IEnumerable<string>?|ISettingsRepository?) → "...not found." + available envs + copy-pasteable `clio reg-web-app <name> -u <url> -l <login> -p <password>`. Routed ToolCommandResolver + all 7 application-family services/commands (List/Info/SectionGetList/SectionCreate/SectionUpdate/SectionDelete/CreateService) through it — kills the message-level split-brain.
Discovery: list-apps/get-app-info/list-app-sections do NOT use ToolCommandResolver — they use startup-injected services with their own generic message (the divergence the ticket flagged). Process-level split-brain (two clio contexts / settings files) is environment/bootstrap and belongs to ENG-91276 — out of scope; documented in ADR. IApplicationListService/IApplicationInfoService are auto-registered by BindingsModule.RegisterAssemblyInterfaceTypes (no explicit line); BaseTool-derived tools (like FindEntitySchemaTool) are NOT explicitly registered — WithToolsFromAssembly handles DI, so only `AddTransient<FindAppCommand>()` was needed. Two queries distinguished in tests by request-body Contains("SysInstalledApp"|"ApplicationSection"). FsmModeStatusService keeps the old message (unrelated domain, left intentionally). Rebase onto origin/master hit a PRE-EXISTING committed conflict marker in this diary (feature/ENG-88588-st-filter-guidance vs master) — resolved by keeping all entries chronologically and stripping the stray markers. Same 3 pre-existing macOS path failures (CreateUiProject_Should_Switch_Working_Directory, ListCreatioBuilds_*) — full unit suite 3493 pass / 3 fail, all pre-existing.
Files: NEW clio/Command/FindAppCommand.cs, clio/Command/McpServer/Tools/FindAppTool.cs, clio/Common/EnvironmentNotFoundError.cs, clio/help/en/find-app.txt, clio/docs/commands/find-app.md, spec/find-app/find-app-adr.md, clio.tests/Command/FindAppCommandTests.cs, clio.tests/Command/McpServer/FindAppToolTests.cs, clio.tests/Common/EnvironmentNotFoundErrorTests.cs, clio.mcp.e2e/{FindAppToolE2ETests.cs,Support/Results/FindAppEnvelope.cs}. EDITED clio/{BindingsModule.cs,Program.cs,Commands.md,Wiki/WikiAnchors.txt}, docs/McpCapabilityMap.md, clio/Command/McpServer/Tools/ToolCommandResolver.cs, clio/Command/McpServer/Resources/{ExistingAppMaintenanceGuidanceResource,AppModelingGuidanceResource}.cs, clio/Command/McpServer/Prompts/ApplicationPrompt.cs, clio/Command/Application{ListService,InfoService,SectionGetListCommand,SectionCreateCommand,SectionUpdateCommand,SectionDeleteCommand,CreateService}.cs.
Impact: One find-app call maps an imprecise app name → real code (matching section captions) and returns sections inline — removes the N+1 discovery loop; env-not-found is now self-healing across both clio MCP env paths. Validated: dotnet build clio (0 err/0 warn); dotnet test --filter "Category=Unit" → 3493 pass / 3 pre-existing fail; 18 new tests pass; clio.mcp.e2e compiles (E2E needs live Creatio, not run locally).
