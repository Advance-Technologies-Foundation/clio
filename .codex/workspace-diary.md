...
## 2026-06-08 — System variables in business-rule conditions (clio create-entity/page-business-rule)
Context: Add the ability to use Creatio system variables (CurrentDate/CurrentTime/CurrentDateTime/CurrentUser/CurrentUserContact/CurrentUserAccount/CurrentUserRoles) as the RIGHT-hand expression of a business-rule condition, for both entity and page rules, with type+reference-schema validation. clio-only feature; creatio-ui/core were reference for the platform contract.
Decision: New right-expression kind `SysValue`. Platform wire shape is `BusinessRuleSysValueExpression` (typeName Terrasoft.Core.BusinessRules.Models.Expressions.BusinessRuleSysValueExpression, type "SysValue", payload `sysValueName`), confirmed in core SystemValue.cs/SystemValueManager.cs and creatio-ui business-rule-7x-converter.service.ts (VALUE_TYPE_7X.SYSVALUE). The SysValue expression INHERITS the left attribute's dataValueTypeName + referenceSchemaName on the wire; `sysValueName` selects the runtime value. Single source of truth: `BusinessRuleConstants.SupportedSystemVariables` (sysValueName -> (DataValueType, ReferenceSchemaName?)). Validation in BusinessRuleValidator.ValidateSysValueRightExpression: name must be in catalog, catalog DataValueType must equal left type, and for lookup variables left must be a Lookup whose ReferenceSchemaName equals the variable's schema (rejects e.g. CurrentUserAccount vs a Contact lookup, CurrentTime vs Date). Conditions only (not set-values). Both rule types covered by the shared BusinessRuleMetadataConverter/BusinessRuleValidator.
Discovery: (1) The MCP condition contract reuses the friendly BusinessRuleConditionGroup -> BusinessRuleExpression directly, so adding `SysValueName` to BusinessRuleExpression made `sysValueName` round-trip with ZERO MCP contract-type changes. (2) Page descriptors carry ReferenceSchemaName (PageBusinessRuleAttributeProvider clones the entity descriptor via `descriptor with { Path = attributeName }`), so reference-schema validation works for page rules even though page OUTPUT omits referenceSchemaName on attribute expressions. (3) A SysValue right side adds NO change trigger (EnumerateTriggerNames already guards on AttributeValue-only). (4) BusinessRuleLookupReferenceValidator only inspects Const right expressions, so SysValue is safely skipped (no record-existence probe). (5) create-entity/page-business-rule are MCP-ONLY commands (no [Verb], no help/en or docs/commands or Commands.md entries) — doc surface is spec/business-rules/*.md + MCP guidance/tool-contract. (6) EntityBusinessRuleService.Create fetches package+attributes from the server BEFORE local validation, so an unreachable/broken Creatio 500s before validation can reject a bad payload.
Files: clio/Command/BusinessRules/{BusinessRuleModels.cs (SysValueName + descriptions), BusinessRuleConstants.cs (BusinessRuleSysValueExpressionTypeName, SysValueExpressionType, SupportedSystemVariables catalog + SystemVariableDescriptor record + SupportedSystemVariablesDescription), BusinessRuleMetadataDtos.cs (SysValueName on expression DTO, reordered JsonPropertyOrder), BusinessRuleMetadataConverter.cs (SysValue branch in BuildRightExpression), BusinessRuleValidator.cs (ValidateSysValueRightExpression)}, clio/Command/McpServer/Tools/ToolContractGetTool.cs (sys-value validator + SysValueBusinessRuleExample helper + entity/page examples + Rule field descriptions), clio/Command/McpServer/Resources/BusinessRulesGuidanceResource.cs (System variables section), spec/adr/adr-business-rule-condition-system-variables.md (new), spec/business-rules/{create-entity,create-page}-business-rule-spec.md. Tests: clio.tests/Command/BusinessRuleMetadataConverterTests.cs (+2 entity/page SysValue), BusinessRuleValidatorTests.cs (+7 accept/reject incl. updated unsupported-type message), McpServer/BusinessRuleToolTests.cs (+1 SysValue deserialize), clio.mcp.e2e/{EntityBusinessRuleToolE2ETests.cs (+binding +destructive readback), PageBusinessRuleToolE2ETests.cs (+binding), Support/Creatio/BusinessRuleAddonReadback.cs (AssertEntityRuleSysValueConditionExistsAsync + AssertSysValueConditionMetadata)}.
Impact: Targeted suite green — `dotnet test --filter "Category=Unit&(Module=Command|Module=McpServer)"` = 2351 passed / 0 failed / 11 skipped. clio.mcp.e2e builds clean; 2 new SysValue binding e2e tests pass against the REAL clio mcp-server (entity + page). Live create against env `dev` could NOT be completed: dev's Creatio returns (500) Internal Server Error for ALL business-rule creates (verified the pre-existing constant-only baseline ALSO 500s), and the 500 occurs at package/attribute resolution BEFORE local validation — an environment defect, not a code defect. The configured e2e sandbox `d2` is unregistered. Validated: dotnet test --filter "Category=Unit&(Module=Command|Module=McpServer)".

## 2026-06-08 (cont.) — Fix business-rule e2e readback: match by rule name + tolerate omitted trigger type
Context: Live verification of the SysValue feature against a real Creatio (env `livecheck` = http://ts1-core-dev24:88/sae_m_seeenu_15512002_0630) showed create succeeds (exit 0) but BusinessRuleAddonReadback found rule==null. Root cause was NOT the feature: the readback matched rules by `caption`, but clio persists the caption as a localizable addon RESOURCE keyed `{ruleUId}.Caption` (BusinessRuleAddonService.UpsertCaptionResource, mirroring Creatio frontend) — the rule object in metaData.rules[] has NO `caption` field (keys = typeName,uId,cases,triggers,name). Second issue: AddonSchemaDesignerService.GetSchema OMITS the trigger `type` when it equals the default ChangeAttributeValue (0), so `ReadInt(trigger["type"]) == 0` failed (key absent).
Decision: (1) Match rules by the generated `name` (createdRules[0].Name, returned by the create as "Rule name: BusinessRule_xxxx") instead of caption. Added McpCommandExecutionParser.ExtractBusinessRuleName(execution). Refactored all BusinessRuleAddonReadback public asserts (AssertEntityRuleExistsAsync, AssertPageRuleExistsAsync, AssertEntityApplyFilterRuleFamilyExistsAsync, AssertEntityRuleSysValueConditionExistsAsync) + private matchers (AssertRuleMetadata, AssertSysValueConditionMetadata, AssertApplyFilterRuleFamilyMetadata) from expectedCaption -> expectedRuleName; apply-filter child rules now matched by their autogenerated `name` (`Autogenerated_{parentUId}_ClearValue/_PopulateValue`) instead of caption. Callers extract the name from the create execution envelope. (2) Added ReadTriggerType(JsonObject) that reads a missing `type` as 0; used in AssertTriggerMetadata + AssertApplyFilterParentTriggers.
Discovery: Verified on livecheck via direct call-service GetSchema that 3 persisted SysValue rules round-trip exactly: leftExpression.path=Owner, rightExpression {typeName=...BusinessRuleSysValueExpression, type=SysValue, sysValueName=CurrentUserContact, dataValueTypeName=Lookup, referenceSchemaName=Contact}. The platform's GetSchema normalization (drop caption to resources, omit default-0 ints) is the same class of behavior behind both bugs. clio call-service inline JSON over the Bash tool can mangle quotes/colon -> use PowerShell with a $body variable.
Files: clio.mcp.e2e/Support/Results/McpCommandExecutionParser.cs (ExtractBusinessRuleName), clio.mcp.e2e/Support/Creatio/BusinessRuleAddonReadback.cs (name matching + ReadTriggerType), clio.mcp.e2e/EntityBusinessRuleToolE2ETests.cs + PageBusinessRuleToolE2ETests.cs (pass extracted rule name to readback).
Impact: Against live env `livecheck`, BusinessRuleCreate_Should_Create_Contact_Rule_In_Creatio AND BusinessRuleCreate_Should_Create_SysValue_Condition_Rule_In_Creatio now PASS (create + readback). The page test (Contact_Page) and apply-filter test fail on livecheck only due to missing sandbox DATA (Contact_FormPage page schema / City-Country lookup columns on Contact) — environmental preconditions, not readback/code defects. Unit suite unaffected (e2e-only changes). The readback fix is a general robustness improvement that was always latent (e2e not in CI).

## 2026-06-08 (cont. 2) — Fix page schema name + apply-filter e2e arg shape + wrong comparison expectations
Context: User review of the business-rule e2e tests on live env `livecheck`.
Changes: (1) PageBusinessRuleToolE2ETests: page schema name corrected Contact_FormPage -> Contacts_FormPage (replace_all incl. descriptions/messages). (2) EntityBusinessRuleToolE2ETests apply-filter destructive test was sending top-level camelCase keys (environmentName/packageName/entitySchemaName) WITHOUT the ["args"] wrapper -> MCP binding error (callResult.IsError=true, fast 12s fail). Wrapped in ["args"] with kebab-case keys like every other destructive test. (3) BusinessRuleAddonReadback apply-filter child comparisonType expectations were WRONG: clear child expected 2 but clio emits ComparisonNotEqual=3; populate child expected 9 but clio emits ComparisonIsFilledIn=1. Verified ground truth via live call-service GetSchema (Autogenerated_*_ClearValue comparisonType=3 left=Country right=City.Country; _PopulateValue comparisonType=1 left=City). Fixed to 3 and 1.
Discovery: Contact entity DOES expose Owner(->Contact), Account(->Account), City(->City), Country(->Country), Region, Name, etc. (get-entity-schema-properties); City.Country(->Country) exists, so the apply-filter City->City.Country vs source Country chain is valid. The two apply-filter comparison constants in the readback were authored wrong and never caught because e2e isn't in CI and the test previously failed earlier (caption match) before reaching them. Page create for Contacts_FormPage on livecheck fails with platform error "Unable to save changes for item Contacts_FormPageBusinessRule. It is either created by third-party publisher or installed from the file archive" — the page belongs to the locked installed package CrtDigitalAdsInLeadGen on this env, so its BusinessRule addon can't be saved. Environment lock condition, not a test/name/column defect; the test now uses the correct page name and dynamically resolves real Contact-bound attributes/elements from the bundle.
Files: clio.mcp.e2e/PageBusinessRuleToolE2ETests.cs (Contacts_FormPage), clio.mcp.e2e/EntityBusinessRuleToolE2ETests.cs (apply-filter args wrapper), clio.mcp.e2e/Support/Creatio/BusinessRuleAddonReadback.cs (clear=3, populate=1 comparison fixes).
Impact: Against live env `livecheck` the destructive e2e now: Create_Contact_Rule PASS, Create_SysValue_Condition_Rule PASS, Create_ApplyFilter_Rule_Family PASS, Create_Contact_Page_Rule fails ONLY due to the page being in a locked installed package on this env (environmental). e2e project builds 0 warnings; unit suite unaffected (e2e-only edits).

## 2026-06-08 15:43 – PR #677 review fixes: canonical system-variable casing + catalog test coverage
Context: Reviewer findings on the business-rule condition system-variables feature (branch feature/business-rule-condition-system-variables).
Decision: Backed SupportedSystemVariables with a single ordered SystemVariableCatalog array; the dictionary (OrdinalIgnoreCase) and SupportedSystemVariablesDescription (string.Join over the catalog) now derive from it, so they cannot drift. Added a canonical SysValueName field to SystemVariableDescriptor (record is now (SysValueName, DataValueTypeName, ReferenceSchemaName)). The converter's SysValue branch now persists the canonical catalog name via SupportedSystemVariables.TryGetValue(...).SysValueName instead of the raw user input, so a casing variant that passes the case-insensitive validator (e.g. "currentdate") is stored as "CurrentDate" — the platform resolves sys variables by exact name at runtime.
Decision: Threaded the already-resolved leftDescriptor down ValidateComparisonOperands -> ValidateRightExpression -> ValidateSysValueRightExpression, removing the redundant ResolveAttribute re-lookup in the lookup-reference-schema check.
Discovery: The page (ToPageMetadata) SysValue right side DOES emit ReferenceSchemaName from the left descriptor (line ~172 in BuildRightExpression is not gated by includeAttributeReferenceSchemaName), unlike page AttributeValue expressions which omit it. Pinned by a new page lookup-SysValue test. Data value type codes: Date=8, Time=9, DateTime=7, Lookup=10 (confirmed against existing UnarySupportedTypeCases). The SupportedSystemVariablesDescription order (CurrentDate, CurrentTime, CurrentDateTime, CurrentUser, CurrentUserContact, CurrentUserAccount, CurrentUserRoles) is consumed by ToolContractGetTool.cs and must be preserved; the catalog array preserves it and string.Join yields a byte-identical string, so MCP contract is unchanged.
Tests: Added [TestCaseSource]-driven Validate_Should_Accept/Reject_System_Variable_When_Left_Attribute_Type_Matches/Mismatches covering all 7 catalog entries (7 match + 7 mismatch). Added ToMetadata_Should_Persist_Canonical_SysValue_Name_When_Input_Casing_Differs ("currentdate" -> "CurrentDate") and ToPageMetadata_Should_Map_Lookup_SysValue_Right_Expression. Augmented ToPageMetadata_Should_Map_Date_SysValue_Right_Expression with Value==null + change-trigger assertions.
Files: clio/Command/BusinessRules/BusinessRuleConstants.cs, clio/Command/BusinessRules/BusinessRuleMetadataConverter.cs, clio/Command/BusinessRules/BusinessRuleValidator.cs, clio.tests/Command/BusinessRuleValidatorTests.cs, clio.tests/Command/BusinessRuleMetadataConverterTests.cs.
Impact: All 7 system variables are now regression-guarded against descriptor typos; persisted sysValueName is always canonical. Validated: dotnet test --filter "Category=Unit&(Module=Command|Module=McpServer)" -> 0 failed, 2367 passed, 11 skipped. MCP reviewed, no update required (SupportedSystemVariablesDescription value and order unchanged).
