...
Impact: AI більше не може зберегти сторінку з захардкодженим user-visible текстом — баг закрито на рівні enforcement, не лише настанови. Build 0 warnings/0 new CLIO*. Unit McpServer+Command 2579 passed/0 failed (+10 нових). E2E додано для update-page(dry-run)/sync-pages/validate-page (не в CI). MCP reviewed: tools+prompts(делеговано)+resources+e2e оновлено.
