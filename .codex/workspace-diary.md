...
## 2026-05-14 14:30 - ENG-88580 business rule read MCP tools
Context: User asked to implement the refined ENG-88580 scope: read-only MCP support for both entity and page business rules, using normalized output and the first architecture approach.
Decision: Added `list-business-rules` and `get-business-rule` as read-only MCP tools backed by the core `BusinessRulesManagerService.svc/GetBusinessRules` endpoint. Scope type maps to core names as `entity` -> `Model` and `page` -> `ViewModel`; `get-business-rule` accepts exactly one selector among `ruleUId`, `ruleName`, and `caption`. Normalized output includes identities, conditions, actions, and automatic `unsupportedDetails`; `includeRawMetadata` is the only raw-data opt-in.
Discovery: The business-rule create model types could be reused for normalized read output actions/conditions. Full `Category=Unit` currently has one unrelated failure in `CreateDataBindingCommandTests.Parse_Should_Map_Lowercase_Environment_Alias` due duplicate `--environment` option metadata in `CreateDataBindingOptions`; targeted business-rule/MCP tests and e2e coverage pass.
Files: clio/Command/BusinessRules/BusinessRuleReadModels.cs, clio/Command/BusinessRules/BusinessRuleMetadataReadConverter.cs, clio/Command/BusinessRules/BusinessRuleReadService.cs, clio/Command/BusinessRuleReadCommand.cs, clio/Command/McpServer/Tools/BusinessRuleTool.cs, clio/Command/McpServer/Tools/ToolContractGetTool.cs, clio/BindingsModule.cs, clio.tests/Command/BusinessRuleReadServiceTests.cs, clio.tests/Command/McpServer/BusinessRuleToolTests.cs, clio.tests/Command/McpServer/ToolContractGetToolTests.cs, clio.mcp.e2e/BusinessRuleReadToolE2ETests.cs
Impact: Agents can inspect entity and page business rules without mutating configuration and can request one rule by stable identifier, internal name, or caption while receiving the same normalized shape used by MCP create tools.

## 2026-05-14 16:21 - Removed business-rule read debug fields from MCP schema
Context: After reviewing business and token trade-offs, user decided not to expose `includeRawMetadata` or `unsupportedDetails` in the business-rule read MCP contract.
Decision: Removed `includeRawMetadata` from `list-business-rules` and `get-business-rule` inputs, removed `rawMetadata` and `unsupportedDetails` from read DTO output, and removed the internal unsupported-detail collector. The converter now returns only the normalized rule fields and skips fragments that cannot be represented.
