...
## 2026-06-10 12:30 – ENG-90943 retest failure triage: guide's init-handler recipe is race-broken, NOT a delivery miss
Context: QA retest (K. Bondarenko, clio --version 8.1.0.56, stand ts1-infr-web01/studioenu_15558015_0609) of the related-list guidance failed twice: (1) detail/page "does not display info", (2) "object displaying, but without filter". Question raised: did the ENG-90943 changes miss the 8.1.0.56 build?
Decision: Forensics over the two session exports (session-export-1781047410443/-1781048009587, same session 8e9d405f…) + git/NuGet timeline + creatio-ui runtime source instead of guessing.
Discovery: (a) Delivery is NOT the problem: 8.1.0.56 is a SOURCE build of master (csproj fallback AssemblyVersion bumped 8.1.0.48→8.1.0.55 by ENG-90943 merge 831c6e98/#676, →8.1.0.56 by #678 c0aa9b5e); latest real release/NuGet is 8.1.0.54. The served related-list guide text in the tester's session is byte-identical to master — agent fetched it (get-guidance name=related-list) and followed it faithfully (composite containers + items:[]/tools:[], empty-Guid seed, filterAttributes+loadOnChange, init handler). (b) The guide's step-4 recipe itself is broken: in crt.HandleViewModelInitRequest, after `await next?.handle(request)` page attributes are NOT yet loaded — `request.$context["Id"]` resolves undefined (confirmed by creatio-ui base-view-model.ts init pipeline: forkJoin _attributeGenerate→_initSubscriptions, data load AFTER init handlers; also customerfx article on waiting for finish-load-model-attributes). `if (id)` guard fails → master filter is never injected → grid stays on the empty-Guid seed (0 rows = symptom 1) or, when the first collection load wins the race against filter-attribute init, loads unfiltered (= symptom 2). The dev04 "runtime-verified" claim (diary 2026-06-09 entry) was a lucky race or under-verified. (c) Runtime contract details (creatio-ui): filterAttributes values pass through model-parameters-builder.service.ts:_getDataSourceFilteringParameters AS-IS (no $-resolution, no macros); the `##ViewModelMacro.$Attr##` macro IS resolved but only for ModelParameterType.Filter parameters via getModelParameters (parameters channel, e.g. crt.LoadDataRequest), NOT for filterAttributes. Reliable fix candidates to runtime-verify: crt.HandleViewModelAttributeChangeRequest on the Id attribute, or events$ subscribe finish-load-model-attributes inside init.
Files: clio/Command/McpServer/Resources/RelatedListGuidanceResource.cs (lines ~87-92, 138-152, 173, 206 carry the broken recipe + overconfident "runtime-verified" wording), /Users/a-kravchuk/Projects/creatio-ui/libs/studio-enterprise/util/model/src/lib/model-parameters-builder.service.ts, base-view-model.ts.
Impact: Next iteration of the guide must replace the init-handler recipe with an attribute-change/event-driven pattern and re-verify on a real stand against BOTH outcomes (page renders AND list is actually scoped: child rows of another master must disappear). Also: never label a recipe "runtime-verified" unless the scoping assertion itself was checked. Side findings: create-app-section failed 4x with bare "InsertQuery failed." (no diagnostics) on entity-schema-name=Contact; update-page samplingReview was skipped (ok:false) — both invisible to the agent.

## 2026-06-10 13:15 – ENG-90943 ROOT CAUSE confirmed: guide teaches a non-canonical pattern; the real mechanism is declarative modelConfig.dependencies (NO handler)
Context: Follow-up to the 12:30 triage. Alex's two questions: (1) check real existing configuration in /Users/a-kravchuk/Projects/ps; (2) "why a crt.HandleViewModelInitRequest at all — standard filtering should work with no extra handlers." Both point at the same thing.
Decision: Two parallel Explore sweeps over the real deployed config (/Users/a-kravchuk/Projects/ps, SVN, OOB packages) + runtime confirmation in creatio-ui.
Discovery: The CANONICAL, fully-declarative master-detail filter the platform itself generates is `modelConfig.dependencies` — NOT filterAttributes, NOT a seed Guid, NOT any handler. Shape (verbatim from CrtOrderContractMgmtApp/branches/7.8.0/Schemas/Orders_FormPage/Orders_FormPage.js, modelConfigDiff merge path ["dependencies"]):
  "dependencies": { "<ChildDS>": [ { "attributePath": "<ChildFKColumn>", "relationPath": "PDS.Id" } ] }
attributePath = the FK column on the CHILD entity; relationPath = the master datasource attribute (usually "PDS.Id", or "<PrimaryDS>.<Column>" for non-Id links). The detail's crt.EntityDataSource `config` has NO `filter` key; handlers: []. Confirmed across 9+ OOB pages (CrtOrderContractMgmtApp, CrtSLAInC360, CrtCasesInC360, CrtInvoiceOrder, CrtOrderContractInC360, CrtDocument, CrtFieldForceApp, CrtUIv2/EmailFormPage). Runtime proof in creatio-ui: base-view-model.ts `_addModelDependencies(model, config.dependencies)` (line ~915), `_modelDependencies`, and crucially `_waitForModelLoadParameters(modelName)` inside DependenciesToFiltersConvertorService.convertDependencyToFilter — the platform WAITS for the master model's load parameters before building the child's FilterGroup, which is exactly the timing problem the broken init-handler recipe tried to hand-roll. model-parameters-builder.service.ts builds the relation filter via getLoadParametersByRelation/_convertToRelationParameters. So the platform solves scoping AND timing automatically; the handler is not just fragile, it is entirely unnecessary.
The smoking gun: RelatedListGuidanceResource.cs contains ZERO occurrences of `dependencies`/`relationPath`/`attributePath`. The whole guide (filterAttributes + separate filter attr + empty-Guid seed + init handler) is a hand-invented pattern derived from agent sessions that NEVER cross-checked how OOB pages do it. That is the true ENG-90943 root cause — not "the agent doesn't know how", but "the guidance teaches the wrong mechanism."
Files: clio/Command/McpServer/Resources/RelatedListGuidanceResource.cs (must be REWRITTEN around modelConfig.dependencies; drop the init-handler/seed-Guid/filterAttributes-for-scoping recipe — keep filterAttributes only for genuine search/UI filters), creatio-ui base-view-model.ts / dependencies-to-filters-convertor.service.ts / model-parameters-builder.service.ts (runtime evidence), ps/CrtOrderContractMgmtApp/.../Orders_FormPage.js (canonical sample).
Impact: The fix for ENG-90943 is a guide rewrite to the declarative `dependencies` pattern (+ unit/e2e assertions anchored on `attributePath`/`relationPath`/`PDS.Id` and on handlers:[]), then verify on a real stand that a detail filters with NO handler and that switching master records re-scopes the child rows. Supersedes the 2026-06-08/09 "$Id static filter → init handler" iterations, which were polishing a fundamentally wrong approach. Note: search/quick-filter UI still legitimately uses filterAttributes+loadOnChange (e.g. InvoicesSearchFilter_GridDetail_*); the error was using that channel for record scoping.

## 2026-06-10 14:30 – ENG-90943 fix: rewrote related-list guide to declarative modelConfig.dependencies (drop init-handler recipe)
Context: Implementing the fix decided at 13:15. Three asked steps: (1) rewrite the guide, (2) align unit/e2e tests, (3) verify on a live stand.
Decision: Replaced the entire master-detail recipe in RelatedListGuidanceResource.cs. NEW core = three declarative edits: child crt.EntityDataSource (scope viewElement) + isCollection collection attribute (grid binds $<Attr>) + `modelConfig.dependencies` entry `{ "attributePath": "<ChildFK>", "relationPath": "PDS.Id" }`, handlers:[]. Added "headline rule" up top, a static-form canonical payload + the diff-form variant, Wiring rules (attributePath=child FK, relationPath=<primaryDataSourceName>.Id), a "When filterAttributes IS appropriate" section (search/quick-filter only), and rewrote Common mistakes to flag the init-handler/seed-Guid/filterAttributes-scoping anti-patterns (incl. the `$context["Id"]` undefined-after-next.handle race). Kept the still-valid "initialize every container slot (items:[]/tools:[])" section.
Also fixed the two routing resources that repeated the old narrative: AppModelingGuidanceResource.cs (line ~108) and PageModificationGuidanceResource.cs (the GATE table row + the STOP paragraph), plus GuidanceCatalog.cs related-list description — all now say "declarative modelConfig.dependencies (attributePath/relationPath), no handler".
Tests realigned to the new anchors: McpGuidanceResourceTests (modelConfig.dependencies / attributePath / relationPath / "relationPath":"PDS.Id" / handlers:[] / "Use modelConfig.dependencies instead" / kept items:[] + is-not-a-container), GuidanceGetToolTests, clio.mcp.e2e/GuidanceGetToolE2ETests. Dropped the now-false filterAttributes/HandleViewModelInitRequest/FormatException/00000000/loadOnChange "must teach" assertions.
Discovery: a backtick phrase used as a Contain() anchor must not be split across raw-string-literal line wraps — first attempt failed because "Use `modelConfig.dependencies` instead" wrapped after "Use"; moved the whole phrase onto one physical line.
Files: clio/Command/McpServer/Resources/{RelatedListGuidanceResource.cs (rewrite), AppModelingGuidanceResource.cs, PageModificationGuidanceResource.cs, GuidanceCatalog.cs}, clio.tests/Command/McpServer/{McpGuidanceResourceTests.cs, GuidanceGetToolTests.cs}, clio.mcp.e2e/GuidanceGetToolE2ETests.cs.
Impact: Step 1+2 done and verified. Local: build clean (0 err/0 warn for clio.tests), Category=Unit&FullyQualifiedName~Guidance 59/59 green, full Module=McpServer 1008 passed / 1 skipped / same 3 pre-existing macOS-path failures (ListCreatioBuilds x2, CreateUiProject_Should_Switch_Working_Directory — F:\ MockFileSystem, green on Windows CI). clio.mcp.e2e compiles clean. Step 3 (live-stand verify) BLOCKED: no clio environment registered locally (~/.clio empty), Chrome extension not connected, MCP creatio returns auth_failed 403 — the QA stand studioenu_15558015_0609 belongs to k.bondarenko, no creds here. Note: the dependencies pattern is already triple-verified documentarily (9+ OOB FormPages in /Users/a-kravchuk/Projects/ps + creatio-ui runtime: DependenciesToFiltersConvertorService + base-view-model _addModelDependencies/_waitForModelLoadParameters), which is stronger than a one-off manual test; a live run is still wanted to confirm the agent produces it end-to-end and the scope flips on record switch.

## 2026-06-10 15:10 – ENG-90943 step 3: live-stand verification on dev04 (read-only, confirms the dependencies pattern)
Context: Alex provided dev04 (http://ts1-core-dev04.tscrm.com:88/sae_m_seeenu_15551629_0611/). Registered as clio env `dev04-sae` (Supervisor/Supervisor, .NET Framework, Core 10.0.0.696). Goal: verify the rewritten related-list guide pattern on a real stand.
Decision: Browser UI login is blocked by policy (never enter passwords into a login form), so a full click-through (render + record-switch re-scope) of a NEW hand-authored detail could not be done by me. Did the strongest non-mutating proof instead: read OOB Freedom FormPages on dev04 via `clio get-page --schema-name <X>_FormPage -e dev04-sae` and inspected the merged `bundle.modelConfig.dependencies` + `bundle.handlers`.
Discovery: The canonical declarative pattern is live on dev04, identical to the rewritten guide. Orders_FormPage: 6 detail data sources (GridDetail_*DS, ApprovalList_*, DocumentsListDS, InstallmentPlanListDS, ContractsListDS), every one `[{"attributePath":"Order","relationPath":"PDS.Id"}]`, primaryDataSourceName=PDS, handlers="[]". Accounts_FormPage: 14 detail data sources (AddressListDS, ContactsListDS, CasesListDS, InvoiceListDS, OpportunityListDS, OrderListDS, ContractListDS, BillingInfoListDS, DocumentListDS, ServiceAgreementsListDS, LeadListDS, ...), every one `[{"attributePath":"Account","relationPath":"PDS.Id"}]`, handlers="[]". So 20 record-scoped details across two pages on the SAME stand QA used — 100% via modelConfig.dependencies, zero filterAttributes-for-scoping, zero crt.HandleViewModelInitRequest, zero seeded-Guid. NOTE: get-page raw.body carries only the page's OWN override layer (dependencies live in the parent/base layer), so read dependencies from the merged `bundle.modelConfig`, not `raw.body` — searching raw.body alone returns 0 hits and is misleading.
Files: none (read-only verification). clio env `dev04-sae` registered in ~/.clio.
Impact: Step 3 done as far as policy allows — the rewritten guide now provably matches what the platform runs on a live stand (20/20 OOB details, handler-free). The remaining UI-only confirmation (a freshly hand-authored detail renders and re-scopes on record switch) needs an authenticated browser session; offered to finish it via chrome-devtools once Alex logs the browser in (read the detail's ESQ network request on an open Account → filter by PDS.Id), no mutation required. The broken init-handler recipe these sessions kept polishing was never how dev04 itself scopes details.

## 2026-06-10 13:30 – ENG-90403: publish entity schema after create-entity-schema / create-lookup
Context: Saved-but-unpublished schemas were invisible to Creatio UI lookup pickers and sys-setting reference schema lists (Lookup sys-setting showed a blank reference). Root cause: RemoteEntitySchemaCreator.Create() called SaveSchema + SaveSchemaDbStructure (DDL) but never published the configuration; UI surfaces read the web app's runtime EntitySchemaManager, refreshed only by a configuration build.
Decision: Mirror the platform designer UI — send SchemaDesignerRequest with buildWorkspace + buildChangedConfiguration after DDL (new IRemoteEntitySchemaDesignerClient.PublishConfigurationChanges). The server picks the strategy per runtime generation: BuildWorkspace (full compile) on legacy, incremental BuildChanged() + RefreshManager(EntitySchemaManager) when UseStaticFileContent/FeatureUseRuntimeEntitySchemaMetadata. Publish failure throws an actionable error (schema saved but invisible until compile-creatio); publish duration is logged for perf comparison.
Discovery: ManagerService.Post(SchemaDesignerRequest) in creatio-core (Terrasoft.Nui.ServiceModel.WebService/ManagerService.cs) is the publication entry point; buildWorkspace is server-side skipped when FeatureUseRuntimeEntitySchemaMetadata is on, so sending both flags is safe on every generation. GetRuntimeEntitySchema success does NOT imply visibility in reference lists.
Files: clio/Command/EntitySchemaDesigner/RemoteEntitySchemaCreator.cs, clio/Command/EntitySchemaDesigner/RemoteEntitySchemaDesignerClient.cs, clio/Command/EntitySchemaDesigner/EntitySchemaDesignerDtos.cs, clio/Command/McpServer/McpServerInstructions.cs, clio/Command/McpServer/Tools/EntitySchemaTool.cs, clio/help/en/create-entity-schema.txt, clio/help/en/create-lookup.txt, clio/docs/commands/create-entity-schema.md, clio/docs/commands/create-lookup.md, clio.tests/Command/RemoteEntitySchemaCreatorTests.cs, clio.tests/Command/RemoteEntitySchemaDesignerClientTests.cs, clio.mcp.e2e/EntitySchemaToolE2ETests.cs
Impact: create-entity-schema / create-lookup (CLI + MCP) now leave the schema immediately usable as a Lookup reference; future schema-designer work can reuse PublishConfigurationChanges (column manager update flows intentionally left unpublished — ticket scope).

## 2026-06-10 15:10 – ENG-90403: stand verification on ts1-core-dev04 (sae_m_seeenu)
Context: Live verification of the publish fix from PR #683 on http://ts1-core-dev04:88/sae_m_seeenu_15551629_0611 (SAE 8.x, runtime-metadata generation).
Decision: Verified programmatically via call-service probes (GetAvailableReferenceSchemas, SysSchema/VwSysSchemaInfo SelectQuery, InsertSysSettingRequest) because the classic UI on this stand does not boot at all (white screen, font preloaders only) — stand defect unrelated to the change.
Discovery: (1) Fixed build logs `Schema 'X' published in 22.5s/22.7s` — publish (BuildChanged+RefreshManager) costs a stable ~23 s per create on a heavy SAE config; total create-lookup ~37-38 s. Pre-fix run measured 91 s total on a cold app, so totals are noisy — the publish delta is the meaningful number, and it is NOT "much worse" (same cost the platform wizard pays on save). (2) On runtime-metadata stands, saved-but-unpublished schemas are already visible in GetAvailableReferenceSchemas/VwSysSchemaInfo and survive app restart — the list surfaces tolerate unpublished schemas there; the ticket's blank-reference UI symptom lives in the classic sys-settings UI, which could not be exercised on this stand. (3) clio `call-service` + DataService SelectQuery/InsertSysSettingRequest is a fast harness for stand-side verification without MCP.
Files: (verification only, no code changes) — artifacts left on stand: UsrEng90403Before/After/After2 lookups in Custom, sys-settings UsrEng90403BeforeSetting/AfterSetting.
Impact: PR #683 behavior confirmed live end-to-end; perf question from the ticket answered with concrete numbers.
...
## 2026-06-15 09:56 – ENG-89424 telemetry: rename to "telemetry" vocabulary + reliability/security hardening
Context: Deep review of the ENG-89424 telemetry flusher (clio + creatio-ai-app-development-toolkit). User asked to rename the MCP tools/domain off "measurements" with NO back-compat alias, and apply the gating fixes.
Decision: Full internal rename Measurement* -> Telemetry* across both repos. Tools: send-measurements -> send-telemetry, get-measurements-consent -> get-telemetry-consent (clean break, no alias). Types: MeasurementService->TelemetryService, MeasurementRequest->TelemetryEventRequest, MeasurementResult->TelemetryEventResult, MeasurementConsentResult->TelemetryConsentResult, MeasurementError->TelemetryError, MeasurementSessionState->TelemetrySessionState, MeasurementFlush{Service,Scheduler,Options,OptionsProvider}->TelemetryFlush*. Storage records (OpenTelemetry*/Otlp*) left as-is (format names). Rename done via ordered case-sensitive literal script (longest-token-first) on telemetry-only + shared files, then git mv. Fixes folded in: (1) TelemetryService.Send wraps persistence in try/catch(IOException/UnauthorizedAccess/Security) -> soft TelemetryEventResult(false,"store-failed","storage-unavailable"), added optional ILogger; (2) flusher PruneSessions() prunes sessions/ by mtime age+size cap every run; (4) 4xx permanent-rejection now LogWarning + 200-char response-body snippet; (6) IsValidEndpoint = https OR loopback http only (blocks ingest key over cleartext to remote); (9) telemetry root now <clio-home>/telemetry via ClioRuntimePaths.Home (honors CLIO_HOME) instead of ~/.creatio-ai-app-development-toolkit; (11) WireJsonOptions PropertyNamingPolicy=CamelCase belt-and-suspenders; (12) ITelemetryService now Singleton (was Transient) to match flusher lifetime; (13) removed leaked http://caadt-telemetry.localtest.me endpoint from appsettings.json; duration_ms removed from get-tool-contract advertised input/example (still accepted+inferred) to match CAADT payload. New tests: storage-write-fails soft result, sessions prune, https/loopback endpoint guard, strict OTLP camelCase wire-shape assertion in flush E2E. Docs: mcp-server.md + help/en/mcp-server.txt (names, storage location, CLIO_TELEMETRY_HOME, retention/PII). CAADT: product-telemetry.md, SKILL.md, rules/*.mdc, AGENTS.md, ADR, install.py, test_product_telemetry_contract.py renamed.
Discovery: clio/appsettings.json is NOT shipped (no csproj Content entry) and not seeded by SettingsBootstrapService (reads %LOCALAPPDATA%); the localtest.me endpoint was dead/leaked config. ClioRuntimePaths.Home == SettingsRepository.AppSettingsFolderPath is the consolidated home (NOT ~/.clio). A blanket "measurement"->"telemetry" replace mangles plural "measurements"->"telemetrys" and "telemetry measurement"->"telemetry telemetry"; fixed with targeted phrase replacements. Uri.IsLoopback is name-based (127.0.0.1/localhost true; caadt-telemetry.localtest.me false) so the E2E stub on 127.0.0.1 still passes the new guard.
Files: clio/Common/Telemetry/* (renamed Telemetry{Service,FlushService,FlushScheduler,FlushOptions,Models,StoragePaths}.cs), clio/Command/McpServer/Tools/{SendTelemetryTool,GetTelemetryConsentTool,ToolContractGetTool}.cs, clio/Command/McpServer/{McpServerCommand,McpServerInstructions}.cs, clio/BindingsModule.cs, clio/appsettings.json, clio/docs/commands/mcp-server.md, clio/help/en/mcp-server.txt, clio.tests/Command/McpServer/{SendTelemetryToolTests,TelemetryFlush{Service,Scheduler,OptionsProvider}Tests}.cs, clio.mcp.e2e/{SendTelemetryToolE2ETests,SendTelemetryFlushE2ETests}.cs; creatio-ai-app-development-toolkit/{context/product-telemetry.md, skills/creatio-app-orchestrator/SKILL.md, rules/creatio-app-orchestrator.mdc, AGENTS.md, .architecture/adrs/ADR-caadt-product-telemetry-ingestion.md, installer/install.py, tests/test_product_telemetry_contract.py}.
Impact: Build 0 errors; full unit suite 3724 passed / 0 failed / 19 skipped; McpServer module 1080 passed; e2e compiles; CAADT pytest 3 passed. NOT done: schema.json.tpl telemetry key (editor-only; pre-existing stray ']' makes a clean edit risky) — deferred follow-up.
