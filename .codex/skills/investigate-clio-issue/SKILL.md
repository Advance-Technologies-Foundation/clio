...
description: Investigate a claimed Clio GitHub issue and determine whether the defect belongs to clio, clio-knowledge, or a repository referenced by clio-knowledge. Use after $claim-clio-issue and before implementation, including when downstream issues and GitHub blocking relationships must communicate ownership.
---
...
Confirm that the original issue is open, assigned to the current GitHub user, linked to the expected Development branch, and has a verified `Mitigation stage = Investigating` using the canonical procedure in `$clio-issue-workflow`. Stop on conflicting ownership or an unverified stage.
...
If the Collab MCP server is available, engage Claude after forming the initial diagnosis. Ask for an independent, read-only challenge to the root cause, repository ownership, missing evidence, and smallest sufficient repair. Verify Claude's claims locally and distinguish accepted, rejected, and unresolved findings. If Collab is unavailable, record that and continue; Claude is conditional, not a blocker.
...
Set and verify the original issue's stage using `$clio-issue-workflow`:
...
Return the diagnosis, evidence, affected repositories, existing or created downstream issues, relationships added, proposed repair boundary, Claude's contribution or unavailability, and unresolved questions.
