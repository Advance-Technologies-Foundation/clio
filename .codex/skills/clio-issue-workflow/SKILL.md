...
- For `take`, `triage`, `fix`, `implement`, or `resolve`, start with `$claim-clio-issue`; claiming always precedes investigation.
- Continue with `$investigate-clio-issue` to prove the failure boundary and identify every affected repository.
- Use `$repair-clio-issue` only when the user authorized implementation. A triage-only request stops after publishing the diagnosis and any authorized downstream issue relationships.
