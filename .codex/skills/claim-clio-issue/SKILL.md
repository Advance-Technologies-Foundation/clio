...
1. Run `$clio-issue-workflow`'s read-only `Mitigation stage` readiness check. Stop before any GitHub write when the provisioning contract is not ready.
...
6. Set the original issue's `Mitigation stage` to `Investigating` using the canonical field procedure in `$clio-issue-workflow`, and verify the stored value.
...
9. Fetch the new remote branch before creating an isolated linked worktree under `.codex/worktrees/issue-<number>`. Track the remote branch and preserve the primary checkout and unrelated user changes.
...
Return the issue URL, authenticated login, assignment result, branch name, Development-link result, worktree path, and mitigation stage. `$investigate-clio-issue` may proceed only after the issue is assigned to the current user and the linked branch exists.
